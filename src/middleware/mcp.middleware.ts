...
export const validateMCPRequest = async (
  ...
  try {
    const existingConnection = await getCacheService().get(
      `mcp_connection:${clientId}`,
    );
    if (!existingConnection) {
      await mcpConnectionMonitor.trackConnection(clientId);

      loggingService.info(
        'New MCP client connection tracked during validation',
        {
          component: 'MCPMiddleware',
          operation: 'validateMCPRequest',
          type: 'mcp_validation',
    ...
    loggingService.debug(
      'Failed to track MCP client connection during validation',
      {
        component: 'MCPMiddleware',
        operation: 'validateMCPRequest',
        type: 'mcp_validation',
  ...
  if (
    req.method === 'POST' &&
    ['tools/list', 'resources/list', 'prompts/list'].includes(req.body?.method)
  ) {
    loggingService.info(
      'High-frequency MCP request detected, skipping detailed logging',
      {
        component: 'MCPMiddleware',
        operation: 'validateMCPRequest',
        type: 'mcp_validation',
...
export const mcpResponseTimer = (
  ...
  res.send = function (data: any) {
    if (req.mcpContext) {
      ...
      if (
        !['tools/list', 'resources/list', 'prompts/list'].includes(
          req.body?.method,
        ) ||
        duration > 1000
      ) {
        ...
        loggingService.debug(
          'High-frequency MCP response (timing not logged)',
          {
            component: 'MCPMiddleware',
            operation: 'mcpResponseTimer',
            type: 'mcp_timing',
...
class MCPConnectionMonitor {
  async trackConnection(clientId: string): Promise<void> {
    ...
    try {
      ...
      loggingService.warn(
        'Failed to store MCP connection in cache, using fallback',
        {
          component: 'MCPConnectionMonitor',
          operation: 'trackConnection',
          type: 'mcp_connection_tracking',
  ...
  async updateActivity(clientId: string): Promise<void> {
    ...
    // Update in cache
    try {
      const existingData = await getCacheService().get<{
        errorCount: number;
        requestCount: number;
      }>(`mcp_connection:${clientId}`);
      if (existingData) {
  ...
  async removeConnection(clientId: string): Promise<void> {
    ...
    // Remove from cache
    try {
      await getCacheService().delete(`mcp_connection:${clientId}`);
...
export function mcpRateLimit(
  ...
  return async (
    ...
    try {
      ...
      loggingService.warn(
        'Failed to retrieve MCP rate limit record from cache',
        {
          component: 'MCPMiddleware',
          operation: 'mcpRateLimit',
          type: 'mcp_rate_limit',
    ...
    if (record.count > maxRequests) {
      ...
      loggingService.info(
        '=== MCP RATE LIMIT MIDDLEWARE COMPLETED (LIMIT EXCEEDED) ===',
        {
          component: 'MCPMiddleware',
          operation: 'mcpRateLimit',
          type: 'mcp_rate_limit',
    ...
    loggingService.info(
      'Step 5: Storing updated MCP rate limit record in cache',
      {
        component: 'MCPMiddleware',
        operation: 'mcpRateLimit',
        type: 'mcp_rate_limit',
    ...
    try {
      ...
      loggingService.info(
        'MCP rate limit record stored in cache successfully',
        {
          component: 'MCPMiddleware',
          operation: 'mcpRateLimit',
          type: 'mcp_rate_limit',
