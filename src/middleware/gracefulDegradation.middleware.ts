...
import {
  ...
  DegradationLevel,
  ServiceMode,
} from '../services/gracefulDegradation.service';
    ...
    try {
      ...
      if (requiredFeatures.length > 0) {
        const unavailableFeatures = requiredFeatures.filter(
          (feature) =>
            !gracefulDegradationService.isFeatureEnabled(feature as any),
        );

        if (unavailableFeatures.length > 0) {
          loggingService.info(
            'Required features unavailable due to degradation',
            {
              component: 'GracefulDegradationMiddleware',
              path: req.path,
              unavailableFeatures,
      ...
      if (!shouldProcess.allowed) {
        ...
        if (fallbackResponse.success) {
          ...
          res.setHeader(
            ...
            shouldProcess.reason || 'Request limits exceeded',
          );
      ...
      res.setHeader(
        ...
        Object.entries(status.strategy.features)
...
async function handleFeatureFallback(
  ...
  const featureFallbacks: Record<string, any> = {
    ai_processing: {
      ...
      message:
        'AI processing is disabled due to high system load. Please try again later.',
      fallback_data: { status: 'unavailable', reason: 'system_load' },
    },
    real_time_updates: {
      error: 'Real-time Updates Disabled',
      message:
        'Real-time updates are temporarily disabled. Data may be cached.',
    ...
    notifications: {
      ...
      message:
        'Notifications are temporarily disabled. Check back later for updates.',
      fallback_data: { delivery_mode: 'none' },
    },
    webhooks: {
      error: 'Webhooks Disabled',
      message:
        'Webhook delivery is temporarily disabled. Events will be queued.',
  ...
  for (const feature of unavailableFeatures) {
    if (featureFallbacks[feature]) {
      ...
      res.setHeader(
        'Retry-After',
        calculateRetryAfter(currentLevel).toString(),
      );
  ...
  return gracefulDegradationMiddleware({
    ...
    customFallback: async (req, res, level) => {
      res.status(503).json({
        ...
        message:
          'File upload functionality is temporarily disabled due to system load.',
 ...
 */
export function realtimeDegradationMiddleware(): (
  req: any,
  res: Response,
  next: NextFunction,
) => void {
 ...
 */
export function complexQueryDegradationMiddleware(): (
  req: any,
  res: Response,
  next: NextFunction,
) => void {
  return gracefulDegradationMiddleware({
    ...
    customFallback: async (req, res, level) => {
      res.status(503).json({
        ...
        message:
          'Complex query operations are temporarily disabled. Please use simpler queries.',
 ...
 */
export async function forceDegradationLevel(
