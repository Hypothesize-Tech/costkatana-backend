  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (
        ...
        try {
          const classification =
            await dataClassificationService.classifyContent(content, context);
      ...
      if (
        ...
        try {
          const filterResult = await preTransmissionFilterService.filterContent(
            content,
            {
      ...
      if (
        ...
        try {
          const complianceCheck =
            await complianceEnforcementService.performComplianceCheck(content, {
              userId: req.user?.id || 'anonymous',
              userLocation: req.headers['x-forwarded-for'] || 'unknown',
              processingPurpose: isAIProcessing
                ? 'ai_processing'
                : 'api_processing',
              dataSource: req.path,
        ...
        try {
          const auditEventId = await comprehensiveAuditService.recordEvent(
            ...
            },
            {
              ...
              sensitivity:
                req.dataClassification?.classification.riskScore.toString(),
            },
            {
              severity: blocked
                ? 'critical'
              ...
              context: {
                requestId,
                businessContext: isAIProcessing
                  ? 'ai_processing'
                  : 'api_request',
                technicalContext: `${req.method} ${req.path}`,
                complianceFramework:
                  req.dataClassification?.classification.complianceFrameworks,
              },
              technical: {
      ...
      if (
        ...
        try {
          ...
          const flowId = await realTimeSecurityMonitoringService.trackDataFlow(
            ...
            {
              type:
                req.dataClassification?.classification.categories[0] ||
                'unknown',
              classification:
                req.dataClassification?.classification.level || 'internal',
              size: content.length,
      ...
      if (blocked) {
        ...
        if (
          customComplianceHandler &&
          violations.some((v) => v.type === 'compliance_violation')
        ) {
          const handled = await customComplianceHandler(
            req,
            res,
            violations.filter((v) => v.type === 'compliance_violation'),
          );
          if (handled) return;
        }
        ...
        if (req.dataFlowId) {
          await realTimeSecurityMonitoringService.updateDataFlowStatus(
            req.dataFlowId,
            'blocked',
          );
        }
...
function setSecurityHeaders(
  ...
  if (req.dataClassification) {
    ...
    res.setHeader(
      ...
      req.dataClassification.handling.auditRequired.toString(),
    );
  }
  ...
  if (req.complianceCheck) {
    ...
    res.setHeader(
      'X-Compliance-Frameworks',
      req.complianceCheck.violations.map((v: any) => v.framework).join(','),
    );
  }
 ...
 */
function trackSecurityPerformance(
  req: any,
  res: Response,
  startTime: number,
  securityEvents: string[],
  ...
  const trackCompletion = () => {
    ...
    if (req.dataFlowId) {
      realTimeSecurityMonitoringService.updateDataFlowStatus(
        req.dataFlowId,
        'completed',
        {
          duration,
