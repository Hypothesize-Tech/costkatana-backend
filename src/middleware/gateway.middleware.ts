...
import { GuardrailsService } from '../modules/guardrails/guardrails.service';
import { getCacheService } from '../common/cache/cache.service';
import { AgentIdentityService } from '../modules/governance/services/agent-identity.service';
 ...
 */
async function handleProxyKeyAuth(
  proxyKeyId: string,
  req: Request,
  res: Response,
): Promise<{
...
async function checkRateLimit(
  ...
  loggingService.info(
    'Step 2: Retrieving gateway rate limit record from cache',
    {
      component: 'GatewayMiddleware',
      operation: 'checkRateLimit',
      type: 'gateway_rate_limit',
  ...
  try {
    const cachedRecord = await getCacheService().get(cacheKey);
    if (cachedRecord) {
  ...
  try {
    ...
    await getCacheService().set(cacheKey, record, ttl, {
      type: 'gateway_rate_limit',
      key,
      maxRequests,
      windowMs,
 ...
 */
export const gatewayAuth = async (
  req: any,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  ...
  try {
    ...
    if (authHeader.startsWith('Bearer ')) {
      ...
      if (authValue.startsWith('dak_')) {
        apiKey = authValue;
        loggingService.info(
          'Dashboard API key found in CostKatana-Auth header',
          {
            component: 'GatewayMiddleware',
            operation: 'gatewayAuth',
            type: 'gateway_authentication',
        ...
        loggingService.info(
          'Step 3: Processing gateway headers for proxy key',
          {
            component: 'GatewayMiddleware',
            operation: 'gatewayAuth',
            type: 'gateway_authentication',
        ...
        const agentIdentity =
          await getAgentIdentityService().authenticateAgent(authValue);
        if (!agentIdentity) {
        ...
        req.gatewayContext = {
          ...
          agentId: agentIdentity.agentId,
          agentIdentityId: (
            agentIdentity._id as mongoose.Types.ObjectId
          ).toString(),
    ...
    if (apiKey) {
      ...
      // Parse API key
      const parsedKey = getAuthService().parseApiKey(apiKey);
      if (!parsedKey) {
 ...
 */
export const processGatewayHeaders = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  ...
  if (
    authMethodOverride &&
    (authMethodOverride === 'gateway' || authMethodOverride === 'standard')
  ) {
  ...
  if (failoverPolicy) {
    ...
    if (!targetUrl) {
      res.status(400).json({
        ...
        message:
          'Either CostKatana-Target-Url or CostKatana-Failover-Policy header is required for routing',
      });
      return;
    }
  ...
  context.securityEnabled =
    req.headers['costkatana-llm-security-enabled'] === 'true';
  ...
  context.cortexCoreModel =
    ...
    'anthropic.claude-opus-4-1-20250805-v1:0'; // Claude 4+ for core
  context.cortexEncodingModel =
    (req.headers['costkatana-cortex-encoding-model'] as string) ||
    'amazon.nova-pro-v1:0'; // Nova Pro for encoding
  context.cortexDecodingModel =
    (req.headers['costkatana-cortex-decoding-model'] as string) ||
    'amazon.nova-pro-v1:0'; // Nova Pro for decoding
  (context as any).cortexOperation =
    (req.headers['costkatana-cortex-operation'] as
      | 'optimize'
      | 'compress'
      | 'analyze'
      | 'transform'
      | 'sast') || 'optimize';
  context.cortexOutputStyle =
    (req.headers['costkatana-cortex-output-style'] as
      | 'formal'
      | 'casual'
      | 'technical'
      | 'conversational') || 'conversational';
  context.cortexOutputFormat =
    (req.headers['costkatana-cortex-output-format'] as
      | 'plain'
      | 'markdown'
      | 'structured') || 'plain';
  ...
  context.cortexPriority =
    (req.headers['costkatana-cortex-priority'] as
      | 'cost'
      | 'speed'
      | 'quality'
      | 'balanced') || 'balanced';
  ...
  context.cortexBinaryCompression =
    (req.headers['costkatana-cortex-binary-compression'] as
      | 'basic'
      | 'standard'
      | 'aggressive') || 'standard';
  ...
  const similarityThreshold = parseFloat(
    req.headers['costkatana-similarity-threshold'] as string,
  );
  context.similarityThreshold = !isNaN(similarityThreshold)
    ? similarityThreshold
    : 0.85;
  ...
  context.rateLimitPolicy = req.headers[
    'costkatana-ratelimit-policy'
  ] as string;
  ...
  const cortexContextManagement = req.headers[
    'x-costkatana-cortex-context-management'
  ] as string;
  if (
    cortexContextManagement === 'true' ||
    cortexContextManagement === 'enabled'
  ) {
    (context as any).cortexContextManagement = true;
  } else if (
    cortexContextManagement === 'false' ||
    cortexContextManagement === 'disabled'
  ) {
    (context as any).cortexContextManagement = false;
  }

  const cortexSessionId = req.headers[
    'x-costkatana-cortex-session-id'
  ] as string;
  if (cortexSessionId) {
    (context as any).cortexSessionId = cortexSessionId;
  }

  const cortexContextCompression = req.headers[
    'x-costkatana-cortex-context-compression'
  ] as string;
  if (
    cortexContextCompression === 'true' ||
    cortexContextCompression === 'enabled'
  ) {
    (context as any).cortexContextCompression = true;
  } else if (
    cortexContextCompression === 'false' ||
    cortexContextCompression === 'disabled'
  ) {
 ...
 */
export const addGatewayResponseHeaders = (
  req: Request,
  res: Response,
  next: NextFunction,
): void => {
...
function addResponseMetadata(req: Request, res: Response): void {
  ...
  if (context.cacheEnabled) {
    ...
    res.setHeader(
      'CostKatana-Cache-Status',
      res.getHeader('CostKatana-Cache-Status') || 'MISS',
    );
  }
