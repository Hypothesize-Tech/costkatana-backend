 ...
 */
export function otelBaggageMiddleware(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  try {
    loggingService.info(
      'Step 1: Extracting context from incoming request headers',
      {
        component: 'OTelBaggageMiddleware',
        operation: 'otelBaggageMiddleware',
        type: 'otel_baggage',
    ...
    const tenantRegion =
      (req.headers['x-tenant-region'] as string) ||
    ...
    const baggage =
      propagation.getBaggage(extractedContext) || propagation.createBaggage();
    ...
    context.with(contextWithBaggage, () => {
      ...
      loggingService.info(
        'Step 7: Storing telemetry context in request object',
        {
          component: 'OTelBaggageMiddleware',
          operation: 'otelBaggageMiddleware',
          type: 'otel_baggage',
      ...
      loggingService.info(
        'OpenTelemetry baggage middleware processing completed successfully',
        {
          component: 'OTelBaggageMiddleware',
          operation: 'otelBaggageMiddleware',
          type: 'otel_baggage',
