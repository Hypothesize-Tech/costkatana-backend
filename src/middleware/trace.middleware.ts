...
export const traceInterceptor = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  ...
  if (req.path.includes('/sessions') || req.path.includes('/traces')) {
    loggingService.info(
      'Tracing skipped for trace endpoint to avoid recursion',
      {
        component: 'TraceMiddleware',
        operation: 'traceInterceptor',
        type: 'trace_interceptor',
  ...
  const isSDKRequest =
    req.headers['x-costkatana-sdk'] ?? req.headers['x-api-key'];
  ...
  if (!isSDKRequest) {
    ...
    const isAllowedAIEndpoint = allowedAIEndpoints.some((endpoint) =>
  ...
  loggingService.info('Trace IDs extracted successfully', {
    ...
    sessionId,
    parentId,
    sessionIdSource: req.headers['x-session-id']
      ? 'x-session-id'
      : req.headers['x-trace-session-id']
        ? 'x-trace-session-id'
        : 'generated',
  ...
  try {
    ...
    const userId =
      (req as any).user?.userId ?? (req as any).user?._id?.toString();

    // Start root span for this HTTP request
    const trace = await traceService.startSpan(
      sessionId,
      `${req.method} ${req.path}`,
      { metadata: { method: req.method, path: req.path, userId } },
    );
    ...
    loggingService.info(
      'Step 5: Setting up response interception for span ending',
      {
        component: 'TraceMiddleware',
        operation: 'traceInterceptor',
        type: 'trace_interceptor',
