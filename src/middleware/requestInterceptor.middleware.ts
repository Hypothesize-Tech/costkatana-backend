...
import {
  ...
  type CortexModelRouterInstance,
} from '../services/cortexModelRouter.service';
...
export class RequestInterceptor {
  ...
  private cortexModelRouter: CortexModelRouterInstance;
  ...
  private config = {
    ...
    thresholds: {
      ...
      costSavingsMinimum: 0.1, // Only intervene if saves >$0.10
    },
  };
  ...
  private async findCheaperModel(
    ...
    try {
      ...
      const complexity = await this.cortexModelRouter.analyzePromptComplexity(prompt);
      const routingDecision = await this.cortexModelRouter.makeRoutingDecision(
        complexity,
        {
          priority: 'cost',
          maxCostPerRequest:
            ...
            0.7 *
            Math.ceil(prompt.length / 4), // 30% cheaper
        },
      );
  ...
  private async estimateCompressionSavings(
    ...
    try {
      ...
      const compressed = await GatewayCortexService.processGatewayRequest(req, {
        prompt,
      });
      ...
      const compressedTokens = Math.ceil(
        (compressed.processedBody.prompt?.length || prompt.length) / 4,
      );
