  ...
  try {
    ...
    if (costkatanaAuth) {
      ...
      if (costkatanaAuth.startsWith('Bearer ')) {
        ...
        if (
          bearerValue.startsWith('dak_') ||
          bearerValue.startsWith('ck_user_')
        ) {
          apiKey = bearerValue;
          loggingService.info(
            'API key found in CostKatana-Auth Bearer header',
            {
        ...
        costkatanaAuth.startsWith('ck_user_') ||
        costkatanaAuth.startsWith('dak_')
      ) {
    ...
    if (!token && !apiKey && req.headers.authorization) {
      if (req.headers.authorization.startsWith('Bearer ')) {
        ...
        req.headers.authorization.startsWith('ck_user_') ||
        req.headers.authorization.startsWith('dak_')
      ) {
    ...
    if (!token && !apiKey) {
      ...
      res.status(401).json({
        ...
        message:
          'Please provide either CostKatana-Auth header (API key) or Authorization: Bearer token',
      });
      return;
    }
    ...
    if (apiKey) {
      ...
      try {
        if (userApiKey.encryptedKey) {
          ...
          if (decryptedKey !== apiKey) {
            loggingService.warn(
              'API key validation failed for automation webhook',
              {
                component: 'AutomationMiddleware',
                operation: 'authenticateAutomationWebhook',
                userId: parsedKey.userId,
        ...
        loggingService.error(
          'Error decrypting API key for automation webhook',
          {
            component: 'AutomationMiddleware',
            operation: 'authenticateAutomationWebhook',
      ...
      try {
        ...
        userId = decoded.id;
        user = await getUserModel().findById(userId);
        if (!user) {
