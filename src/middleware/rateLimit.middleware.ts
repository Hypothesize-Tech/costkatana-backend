...
export function rateLimitMiddleware(
  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (record.count >= maxRequests) {
      ...
      loggingService.info(
        '=== RATE LIMIT MIDDLEWARE COMPLETED (LIMIT EXCEEDED) ===',
        {
          component: 'RateLimitMiddleware',
          operation: 'rateLimitMiddleware',
          type: 'rate_limit',
    ...
    if (skipSuccessfulRequests || skipFailedRequests) {
      loggingService.info(
        'Step 8: Setting up skip logic for response handling',
        {
          component: 'RateLimitMiddleware',
          operation: 'rateLimitMiddleware',
          type: 'rate_limit',
        ...
        const handleSkipLogic = async () => {
          if (skipSuccessfulRequests && res.statusCode < 400) {
            ...
            loggingService.debug(
              'Successful request skipped from rate limit count',
              {
                component: 'RateLimitMiddleware',
                operation: 'rateLimitMiddleware',
                type: 'rate_limit',
            ...
            loggingService.debug(
              'Failed request skipped from rate limit count',
              {
                component: 'RateLimitMiddleware',
                operation: 'rateLimitMiddleware',
                type: 'rate_limit',
...
export function apiKeyRateLimit(
  ...
  return rateLimitMiddleware({
    ...
    message:
      'API key rate limit exceeded. Please upgrade your plan or try again later.',
  });
}
