...
import { getAdaptiveRateLimitService } from '../common/services/adaptive-rate-limit.service';
import {
  requestPrioritizationService,
  RequestPriority,
} from '../services/requestPrioritization.service';
...
export function enterpriseTrafficManagementMiddleware(
  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (
        enableServicePrioritization &&
        !bypassSpecificProtections.includes('service_prioritization')
      ) {
        const servicePriority = servicePrioritizationService.getServicePriority(
          req.path,
        );

        if (servicePriority.should_degrade && customOverloadHandler) {
          const handled = await customOverloadHandler(
            req,
            res,
            systemStatus.current_state.service_prioritization?.overload_level ||
              'normal',
          );
          if (handled) return;
        }
        ...
        res.setHeader(
          'X-Service-Priority-Score',
          servicePriority.priority_score.toFixed(3),
        );
      }
      ...
      if (
        ...
        !bypassSpecificProtections.includes('adaptive_rate_limit')
      ) {
        ...
        const rateLimitMetadata = {
          ...
          priority:
            requestMetadata.priority === 'critical'
              ? 'high'
              : requestMetadata.priority === 'background'
                ? 'low'
        ...
        if (!rateLimitDecision.allowed) {
          if (customRateLimitHandler) {
            const handled = await customRateLimitHandler(
              req,
              res,
              rateLimitDecision,
            );
          ...
          res.setHeader(
            'Retry-After',
            rateLimitDecision.retryAfter?.toString() || '60',
          );
      ...
      if (
        enablePreemptiveThrottling &&
        !bypassSpecificProtections.includes('preemptive_throttling')
      ) {
        ...
        const throttlingDecision =
          await preemptiveThrottlingService.checkThrottling(throttlingMetadata);

        if (!throttlingDecision.allowed) {
          res.setHeader('X-Throttling-Phase', throttlingDecision.phase);
          res.setHeader(
            'X-Throttling-Reason',
            throttlingDecision.reasons.join(', '),
          );
          res.setHeader(
            'Retry-After',
            throttlingDecision.retry_after?.toString() || '60',
          );

          res.status(503).json({
            error: 'System Temporarily Overloaded',
            message:
              throttlingDecision.warning_message ||
      ...
      if (
        ...
        !bypassSpecificProtections.includes('graceful_degradation')
      ) {
        ...
        if (degradationStatus.level !== 'none') {
          ...
          if (!shouldProcess.allowed) {
            if (customDegradationHandler) {
              const handled = await customDegradationHandler(
                req,
                res,
                degradationStatus.level,
              );
              if (handled) return;
            }
            ...
            const fallbackResponse =
              await gracefulDegradationService.getFallbackResponse(
                req.path,
                req.method,
            ...
            if (fallbackResponse.success) {
              ...
              res.setHeader(
                ...
                shouldProcess.reason || 'System degraded',
              );
      ...
      if (
        ...
        !bypassSpecificProtections.includes('request_prioritization')
      ) {
        ...
        try {
          ...
          loggingService.warn(
            'Request prioritization failed, continuing without queue',
            {
              component: 'EnterpriseTrafficManagement',
              requestId,
...
function calculateBaseLimit(userTier: string, priority: string): number {
  ...
  const tierMultiplier =
    tierMultipliers[userTier as keyof typeof tierMultipliers] || 1.0;
  const priorityMultiplier =
    priorityMultipliers[priority as keyof typeof priorityMultipliers] || 1.0;
 ...
 */
function setComprehensiveHeaders(
  res: Response,
  systemStatus: any,
  _req: any,
): void {
  ...
  res.setHeader(
    'X-Active-Protections',
    systemStatus.active_protections.join(', '),
  );

  // Performance headers
  res.setHeader(
    'X-System-CPU',
    systemStatus.performance_metrics.cpu_usage.toFixed(1),
  );
  res.setHeader(
    'X-System-Memory',
    systemStatus.performance_metrics.memory_usage.toFixed(1),
  );
  res.setHeader(
    'X-System-Response-Time',
    systemStatus.performance_metrics.average_response_time.toString(),
  );
  res.setHeader(
    'X-System-Error-Rate',
    systemStatus.performance_metrics.error_rate.toFixed(2),
  );
  ...
  if (systemStatus.current_state.graceful_degradation?.level !== 'none') {
    ...
    res.setHeader(
      'X-Degradation-Level',
      systemStatus.current_state.graceful_degradation.level,
    );
  }
  ...
  if (systemStatus.current_state.preemptive_throttling?.phase !== 'normal') {
    ...
    res.setHeader(
      'X-Throttling-Phase',
      systemStatus.current_state.preemptive_throttling.phase,
    );
  }
