...
const MongodbMcpAuditLogModel =
  mongoose.models.MongodbMcpAuditLog ??
  mongoose.model('MongodbMcpAuditLog', MongodbMcpAuditLogSchema);

const MongoDBConnectionModel =
  mongoose.models.MongoDBConnection ??
  mongoose.model(MongoDBConnection.name, MongoDBConnectionSchema);
...
export const mongodbMcpMiddleware = async (
  ...
  try {
    ...
    const connectionId =
      req.body?.params?.connectionId || req.query.connectionId;
    if (!connectionId) {
      res.status(400).json({
        jsonrpc: '2.0',
        error: {
    ...
    const connection = await MongoDBConnectionModel.findOne({
      _id: connectionId,
      userId,
      isActive: true,
    });
...
export const auditMongoDBMCPOperation = async (
  ...
  res.send = function (data: any): Response {
    ...
    if (context) {
      ...
      const collection =
        req.body?.params?.collection || req.body?.params?.arguments?.collection;
      const database =
        req.body?.params?.database || req.body?.params?.arguments?.database;
      ...
      let eventType:
        ...
        | 'schema_accessed'
        | 'data_exported'
        | 'operation_denied' = 'tool_executed';
      if (operation) {
      ...
      loggingService.info('MongoDB MCP operation audit', {
        ...
        toolName,
        toolArgsHash: toolArgs
          ? JSON.stringify(toolArgs).substring(0, 100)
          : undefined,
        duration,
      ...
      MongodbMcpAuditLogModel.create({
        ...
        dangerousOperation: operation === 'delete' || operation === 'update',
        context: {
          userId: new mongoose.Types.ObjectId(context.userId),
          connectionId: context.connectionId
            ? new mongoose.Types.ObjectId(context.connectionId)
            : undefined,
        },
        timestamp: new Date(),
      }).catch((error: unknown) => {
