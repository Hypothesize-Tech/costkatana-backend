...
import { getWebhookService } from '../modules/webhook/webhook.service';
...
export function verifyWebhookSignature(
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    loggingService.info(
      '=== WEBHOOK SIGNATURE VERIFICATION MIDDLEWARE STARTED ===',
      {
    ...
    try {
      loggingService.info(
        'Step 1: Extracting webhook signature and timestamp',
        {
          component: 'WebhookMiddleware',
          operation: 'verifyWebhookSignature',
          type: 'webhook_signature',
      ...
      const isValid = getWebhookService().verifySignature(
        secret,
        payload,
        timestamp,
        signature,
      ...
      loggingService.info(
        'Webhook signature verification completed successfully',
        {
          component: 'WebhookMiddleware',
          operation: 'verifyWebhookSignature',
          type: 'webhook_signature',
      ...
      loggingService.info(
        '=== WEBHOOK SIGNATURE VERIFICATION MIDDLEWARE COMPLETED ===',
        {
          component: 'WebhookMiddleware',
          operation: 'verifyWebhookSignature',
          type: 'webhook_signature',
...
export function webhookRateLimit(
  ...
  return async (
    ...
    loggingService.info(
      'Step 1: Retrieving webhook rate limit record from cache',
      {
        component: 'WebhookMiddleware',
        operation: 'webhookRateLimit',
        type: 'webhook_rate_limit',
    ...
    try {
      ...
      loggingService.warn(
        'Failed to retrieve webhook rate limit record from cache',
        {
          component: 'WebhookMiddleware',
          operation: 'webhookRateLimit',
          type: 'webhook_rate_limit',
    ...
    if (record.count > options.maxRequests) {
      ...
      loggingService.info(
        '=== WEBHOOK RATE LIMIT MIDDLEWARE COMPLETED (LIMIT EXCEEDED) ===',
        {
          component: 'WebhookMiddleware',
          operation: 'webhookRateLimit',
          type: 'webhook_rate_limit',
    ...
    loggingService.info(
      'Step 4: Storing updated webhook rate limit record in cache',
      {
        component: 'WebhookMiddleware',
        operation: 'webhookRateLimit',
        type: 'webhook_rate_limit',
    ...
    try {
      ...
      loggingService.info(
        'Webhook rate limit record stored in cache successfully',
        {
          component: 'WebhookMiddleware',
          operation: 'webhookRateLimit',
          type: 'webhook_rate_limit',
      ...
      loggingService.warn(
        'Failed to store webhook rate limit record in cache',
        {
          component: 'WebhookMiddleware',
          operation: 'webhookRateLimit',
          type: 'webhook_rate_limit',
...
export function parseWebhookPayload(
  ...
  try {
    if (contentType?.includes('application/x-www-form-urlencoded')) {
      ...
      loggingService.info(
        'Step 2c: Using JSON payload (already parsed by express.json())',
        {
          component: 'WebhookMiddleware',
          operation: 'parseWebhookPayload',
          type: 'webhook_payload_parsing',
    ...
    loggingService.info(
      '=== WEBHOOK PAYLOAD PARSING MIDDLEWARE COMPLETED ===',
      {
        component: 'WebhookMiddleware',
        operation: 'parseWebhookPayload',
        type: 'webhook_payload_parsing',
...
export function deduplicateWebhooks(
  ...
  return async (
    ...
    const eventId =
      (req.headers[options.headerName || 'x-event-id'] as string) ||
      req.body.eventId ||
      req.body.id;
    ...
    loggingService.info('Step 1: Extracting event ID for deduplication', {
      ...
      eventId,
      source: req.headers[options.headerName || 'x-event-id']
        ? 'header'
        : req.body.eventId
          ? 'body.eventId'
          : req.body.id
    ...
    loggingService.info(
      'Step 2: Checking if event has already been processed',
      {
        component: 'WebhookMiddleware',
        operation: 'deduplicateWebhooks',
        type: 'webhook_deduplication',
    ...
    try {
      const processedTimestamp = await getCacheService().get<number>(cacheKey);
    ...
    if (isDuplicate) {
      ...
      loggingService.info(
        '=== WEBHOOK DEDUPLICATION MIDDLEWARE COMPLETED (DUPLICATE) ===',
        {
          component: 'WebhookMiddleware',
          operation: 'deduplicateWebhooks',
          type: 'webhook_deduplication',
    ...
    try {
      ...
      await getCacheService().set(cacheKey, now, ttl, {
        type: 'webhook_deduplication',
        eventId,
        ttl: options.ttl,
      });
...
export function validateWebhookPayload(
  ...
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    loggingService.info(
      '=== WEBHOOK PAYLOAD VALIDATION MIDDLEWARE STARTED ===',
      {
        component: 'WebhookMiddleware',
    ...
    loggingService.info(
      '=== WEBHOOK PAYLOAD VALIDATION MIDDLEWARE COMPLETED ===',
      {
        component: 'WebhookMiddleware',
        operation: 'validateWebhookPayload',
        type: 'webhook_payload_validation',
