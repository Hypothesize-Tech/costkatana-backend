...
// Stub for legacy Express middleware - when used with Nest, inject GoogleSearchService via app context
const googleSearchService = {
  ...
  getQuotaStatus: async (): Promise<{
    ...
    limit: number;
    percentage: number;
  }> => ({ count: 0, limit: 100, percentage: 0 }),
};
  ...
  try {
    ...
    if (googleSearchService.isConfigured()) {
      ...
      if (quotaStatus.percentage >= 80) {
        ...
        res.setHeader(
          'X-Search-Quota-Remaining',
          (quotaStatus.limit - quotaStatus.count).toString(),
        );
      }
    }
 ...
 */
export function searchResultsLogger(
  resultCount: number,
...
export async function getQuotaStatus(userId?: string): Promise<{
  ...
  if (!googleSearchService.isConfigured()) {
    loggingService.debug('Google Search service not configured', {
      userId: userId || 'anonymous',
    });
    return {
      count: 0,
