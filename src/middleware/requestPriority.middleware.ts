...
export function requestPriorityMiddleware(
  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const requestPriority = priorityDetector
        ? priorityDetector(req)
        : determinePriority(req, priority);
...
function getRequestType(req: any): 'api' | 'webhook' | 'background' | 'system' {
  ...
  if (req.path.startsWith('/api/system') || req.path.startsWith('/api/health'))
    return 'system';
  if (req.headers['x-background-job'] === 'true' || req.isBackgroundJob)
    return 'background';
  return 'api';
}
...
function isBackgroundJob(req: any): boolean {
  return (
    req.headers['x-background-job'] === 'true' ||
