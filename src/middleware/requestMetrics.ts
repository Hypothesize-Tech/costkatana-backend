...
export function requestMetricsMiddleware(
  ...
  res.end = function (...args: any[]) {
    ...
    if (res.getHeader('content-length')) {
      ...
      if (!isNaN(size)) {
        ...
        loggingService.debug(
          'Invalid content-length header, skipping response size metrics',
          {
            component: 'RequestMetricsMiddleware',
            operation: 'requestMetricsMiddleware',
            type: 'request_metrics',
      ...
      loggingService.debug(
        'No content-length header, skipping response size metrics',
        {
          component: 'RequestMetricsMiddleware',
          operation: 'requestMetricsMiddleware',
          type: 'request_metrics',
    ...
    if (span) {
      ...
      TelemetryService.storeTelemetryData(telemetryData).catch(
        (err: unknown) => {
      ...
      loggingService.warn(
        'No active span found, skipping telemetry data storage',
        {
          component: 'RequestMetricsMiddleware',
          operation: 'requestMetricsMiddleware',
          type: 'request_metrics',
...
export function createBusinessMetrics() {
  ...
  const businessMetrics = {
    ...
    optimizationsPerformed: meter.createCounter(
      'business.optimizations.performed',
      {
