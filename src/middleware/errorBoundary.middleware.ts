...
export class ErrorBoundaryMiddleware {
  ...
  public static errorBoundary(options: ErrorBoundaryOptions = {}) {
    ...
    return (
      ...
      // Check rate limiting
      if (
        config.maxErrorsPerMinute &&
        ErrorBoundaryMiddleware.isRateLimited(
          req.ip || 'unknown',
          config.maxErrorsPerMinute,
        )
      ) {
  ...
  public static serviceErrorBoundary(serviceName: string) {
    return (
      ...
      const processedError = ErrorHandler.processError(error, context);
      ErrorBoundaryMiddleware.sendErrorResponse(
        res,
        processedError,
        ErrorBoundaryMiddleware.DEFAULT_OPTIONS,
      );
    };
  }
  ...
  public static validationErrorHandler() {
    return (error: any, req: Request, res: Response, next: NextFunction) => {
      ...
      if (error.array && typeof error.array === 'function') {
        ...
        return ErrorBoundaryMiddleware.errorBoundary()(
          serviceError,
          req,
          res,
          next,
  ...
  private static isRateLimited(
    ...
    if (!clientErrors || clientErrors.resetTime < minuteAgo) {
      ErrorBoundaryMiddleware.errorCounts.set(clientId, {
        count: 1,
        resetTime: now,
      });
      return false;
