...
export const validateRequest = (
  ...
  if (!errors.isEmpty()) {
    loggingService.debug('Express-validator validation errors found', {
      ...
      errors: errors
        .array()
        .map((e: any) => ({ field: e.path, message: e.msg, value: e.value })),
    });
...
export const validateQuery = (schema: ZodSchema) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    loggingService.info(
      'Step 1: Starting query parameters validation with Zod schema',
      {
        component: 'ValidationMiddleware',
        operation: 'validateQuery',
        type: 'zod_query_validation',
    ...
    try {
      ...
      if (error.errors) {
        ...
        loggingService.info(
          'Query validation errors formatted and sent to client',
          {
            component: 'ValidationMiddleware',
            operation: 'validateQuery',
            type: 'zod_query_validation',
...
export const validateParams = (schema: ZodSchema) => {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    loggingService.info(
      'Step 1: Starting URL parameters validation with Zod schema',
      {
        component: 'ValidationMiddleware',
        operation: 'validateParams',
        type: 'zod_params_validation',
    ...
    try {
      ...
      loggingService.info(
        '=== ZOD PARAMS VALIDATION MIDDLEWARE COMPLETED ===',
        {
          component: 'ValidationMiddleware',
          operation: 'validateParams',
          type: 'zod_params_validation',
      ...
      if (error.errors) {
        ...
        loggingService.info(
          'URL parameters validation errors formatted and sent to client',
          {
            component: 'ValidationMiddleware',
            operation: 'validateParams',
            type: 'zod_params_validation',
      ...
      loggingService.info(
        'Generic URL parameters validation error sent to client',
        {
          component: 'ValidationMiddleware',
          operation: 'validateParams',
          type: 'zod_params_validation',
...
export const sanitizeInput = (
  ...
  loggingService.info(
    'Step 1: Starting input sanitization for XSS prevention',
    {
      component: 'ValidationMiddleware',
      operation: 'sanitizeInput',
      type: 'input_sanitization',
