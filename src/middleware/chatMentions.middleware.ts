...
export function parseMentions(message: string): Mention[] {
  ...
  const mentionPattern =
    /@(mongodb|vercel|github|google|slack|jira|linear|discord)\s*/gi;
  ...
  while ((match = mentionPattern.exec(message)) !== null) {
    ...
    const nextMentionIndex = afterMention.search(
      /@(mongodb|vercel|github|google|slack|jira|linear|discord)/i,
    );
    const commandEnd =
      nextMentionIndex > 0 ? nextMentionIndex : afterMention.length;
 ...
 */
export function chatMentionsMiddleware(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
 ...
 */
export function validateMentionSyntax(mention: Mention): {
  valid: boolean;
  error?: string;
} {
...
export function hasMentions(message: string): boolean {
  return /@(mongodb|vercel|github|google|slack|jira|linear|discord)/i.test(
    message,
  );
}
 ...
 * Get mention by type
 */
export function getMentionByType(
  message: string,
  type: Mention['type'],
): Mention | null {
  const mentions = parseMentions(message);
