...
import {
  getAdaptiveRateLimitService,
  AdaptiveRateLimitConfig,
} from '../common/services/adaptive-rate-limit.service';
...
export function adaptiveRateLimitMiddleware(
  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const decision = await getAdaptiveRateLimitService().checkRateLimit(
        key,
        rateLimitConfig,
        {
          userId: req.user?.id,
      ...
      if (!decision.allowed) {
        ...
        if (enableGracefulDegradation && priority !== 'high') {
          ...
          const degradationResult = await handleGracefulDegradation(
            req,
            res,
            degradationMode,
          );
...
async function tryServeFromCache(
  ...
  try {
    ...
    const cachedData = await getCacheService().get(cacheKey);
...
async function updateUsageCounter(
  ...
  try {
    ...
    let record = await getCacheService().get(cacheKey);
    if (!record) {
    ...
    await getCacheService().set(cacheKey, recordData, ttl);
  } catch (error) {
...
async function getCurrentUsage(cacheKey: string): Promise<number> {
  try {
    const record = await getCacheService().get(cacheKey);
    if (!record) return 0;
      ...
      try {
        ...
        if (skipSuccessfulRequests && res.statusCode < 400) {
          ...
          loggingService.debug(
            'Successful request skipped from adaptive rate limit count',
            {
              component: 'AdaptiveRateLimitMiddleware',
              cacheKey,
          ...
          shouldDecrement = true;
          loggingService.debug(
            'Failed request skipped from adaptive rate limit count',
            {
              component: 'AdaptiveRateLimitMiddleware',
              cacheKey,
              statusCode: res.statusCode,
...
export async function getAdaptiveRateLimitStats(): Promise<any> {
  try {
    return await getAdaptiveRateLimitService().getStatistics();
  } catch (error) {
