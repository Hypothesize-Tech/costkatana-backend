...
import {
  ...
  ThrottlingPhase,
} from '../services/preemptiveThrottling.service';
...
export function preemptiveThrottlingMiddleware(
  ...
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      const decision =
        await preemptiveThrottlingService.checkThrottling(requestMetadata);
...
function setThrottlingHeaders(res: Response, decision: any): void {
  ...
  res.setHeader(
    ...
    decision.metrics.response_time.toString(),
  );
 ...
 */
async function applyThrottlingDelay(
  delayMs: number,
  _decision: any,
  _req: any,
): Promise<void> {
 ...
 */
export function throttlingStatusMiddleware(): (
  req: any,
  res: Response,
  next: NextFunction,
) => void {
  return async (req: any, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      res.setHeader(
        'X-System-Load-Factor',
        status.throttling_factor.toFixed(3),
      );
      res.setHeader(
        'X-System-Phase-Duration',
        status.phase_duration.toString(),
      );
 ...
 */
export function responseTimeTrackingMiddleware(): (
  req: any,
  res: Response,
  next: NextFunction,
) => void {
