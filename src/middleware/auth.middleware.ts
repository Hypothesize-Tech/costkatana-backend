...
import { getUserSessionService } from '../modules/user-session/user-session.service';
import { loggingService } from '../common/services/logging.service';
import { getUserModel } from '../common/utils/get-mongoose-models';
...
export const authenticate = async (
  ...
  try {
    ...
    if (apiKey) {
      ...
      user = await getUserModel().findById(parsedKey.userId);
      if (!user) {
      ...
      try {
        ...
        user = await getUserModel().findById(payload.id);
        ...
        if (payload.jti) {
          ...
          if (userApiKey) {
            ...
            loggingService.debug(
              'JWT token contains userSessionId (not API key)',
              {
                component: 'AuthMiddleware',
                operation: 'authenticate',
                type: 'authentication',
            ...
            try {
              ...
              loggingService.warn(
                'Error validating session in auth middleware',
                {
                  component: 'AuthMiddleware',
                  operation: 'authenticate',
                  type: 'authentication',
        ...
        req.user = {
          ...
          permissions: payload.jti
            ? user.dashboardApiKeys.find(
                (key: any) => key.keyId === payload.jti,
              )?.permissions || ['read']
            : ['read', 'write', 'admin'],
        };
...
export const optionalAuth = async (
  ...
  try {
    ...
    try {
      // Try API key first
      if (authValue.startsWith('dak_')) {
        const parsedKey = getAuthService().parseApiKey(authValue);
        if (parsedKey) {
          const user = await getUserModel().findById(parsedKey.userId);
          if (user) {
            const userApiKey = user.dashboardApiKeys.find(
              (key: any) => key.keyId === parsedKey.keyId,
            );
            if (
              userApiKey &&
              (!userApiKey.expiresAt || new Date() <= userApiKey.expiresAt)
            ) {
              // Validate key
              try {
                ...
                loggingService.debug(
                  'Optional auth: Invalid API key provided',
                  {
                    component: 'AuthMiddleware',
        ...
        // Try JWT token
        const payload = getAuthService().verifyAccessToken(authValue);
