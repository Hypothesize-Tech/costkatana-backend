...
export function parseNaturalDateTime(
  ...
  loggingService.debug('Parsing natural language date/time', {
    text,
...
function parseDate(text: string, now: Date): Date {
  ...
  const dateMatch = text.match(
    /\b(\d{1,2})(st|nd|rd|th)?(?:\s+of\s+)?(?:this\s+month|next\s+month)?\b/i,
  );
  if (dateMatch) {
  ...
  if (monthDayMatch) {
    ...
    const monthIndex =
      monthNames.indexOf(monthName) >= 0
  ...
  const relativeDaysMatch = text.match(
    /\b(?:in\s+)?(\d+)\s+days?(?:\s+from\s+now)?\b/i,
  );
  ...
  const relativeWeeksMatch = text.match(
    /\b(?:in\s+)?(\d+)\s+weeks?(?:\s+from\s+now)?\b/i,
  );
...
function checkIfAllDay(text: string): boolean {
  return (
    /\ball[- ]day\b/i.test(text) ||
    /\bfull day\b/i.test(text) ||
    (/\b(?:on|for)\s+(?:the\s+)?\d{1,2}(?:st|nd|rd|th)?\b/i.test(text) &&
      !/\bat\s+\d/i.test(text))
  );
}
...
function parseRecurrence(
  text: string,
): ParsedDateTime['recurrence'] | undefined {
  ...
  const weeklyMatch = text.match(
    /\bevery\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday)(?:\s+and\s+(monday|tuesday|wednesday|thursday|friday|saturday|sunday))*/i,
  );
  if (weeklyMatch) {
 ...
 */
export function formatRecurrenceAsRRule(
  recurrence: ParsedDateTime['recurrence'],
): string[] {
  ...
  if (recurrence.until) {
    const until =
      recurrence.until.toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
