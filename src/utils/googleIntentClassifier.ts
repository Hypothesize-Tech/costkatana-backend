...
function detectGmailIntent(text: string): GoogleIntent {
  ...
  // Send email patterns
  if (
    /\b(send|email|mail)\b.*\b(to|recipient)\b/.test(text) ||
  ...
  if (
    /\b(search|find|look\s+for)\b.*\b(email|mail|message|inbox)\b/.test(text) ||
  ...
  // List emails
  if (
    /\b(show|list|display)\b.*\b(email|mail|message|inbox)\b/.test(text) ||
...
function detectCalendarIntent(text: string): GoogleIntent {
  ...
  if (
    /\b(create|add|schedule|set\s+up|make)\b.*\b(meeting|event|appointment|calendar)\b/.test(
      text,
    ) ||
    /\b(meeting|event|appointment)\b.*\b(tomorrow|today|next|this)\b/.test(
      text,
    ) ||
  ...
  // List events
  if (
    /\b(show|list|display|what('?s)?)\b.*\b(meeting|event|calendar|schedule|agenda)\b/.test(
      text,
    ) ||
...
function detectDriveIntent(text: string): GoogleIntent {
  // Search files
  if (
    /\b(search|find|look\s+for)\b.*\b(file|drive|document|folder)\b/.test(
      text,
    ) ||
  ...
  // List files
  if (
    /\b(show|list|display)\b.*\b(file|drive|document|folder)\b/.test(text) ||
...
function detectDocsIntent(text: string): GoogleIntent {
  // Read document
  if (
    /\b(read|open|show|display|view)\b.*\b(doc|document|google\s+doc)\b/.test(
      text,
    ) ||
...
function detectSheetsIntent(text: string): GoogleIntent {
  // Read spreadsheet
  if (
    /\b(read|open|show|view)\b.*\b(sheet|spreadsheet|google\s+sheet)\b/.test(
      text,
    )
  ) {
    return {
  ...
  // Create spreadsheet
  if (
    /\b(create|make|new)\b.*\b(sheet|spreadsheet|google\s+sheet)\b/.test(text)
  ) {
    return {
      service: 'sheets',
      action: 'create',
