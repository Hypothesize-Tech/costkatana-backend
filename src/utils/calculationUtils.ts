...
export function calculateUnifiedSavings(
  ...
  const providerStr =
    typeof provider === 'string' ? provider : providerEnumToString(provider);

  // Calculate tokens consistently - estimateTokens expects AIProvider enum or undefined
  const originalPromptTokens = estimateTokens(
    originalPrompt,
    typeof provider === 'string' ? undefined : provider,
    model,
  );
  const optimizedResponseTokens = estimateTokens(
    optimizedResponse,
    typeof provider === 'string' ? undefined : provider,
    model,
  );
  ...
  try {
    ...
    loggingService.warn(
      `No pricing data found for ${providerStr}/${model}, using fallback pricing for original`,
    );
    // Use fallback pricing (GPT-4o-mini rates as default)
    originalCostEstimate = {
      inputCost: (originalPromptTokens / 1_000_000) * 0.15,
      outputCost: (completionTokensEstimate / 1_000_000) * 0.6,
      totalCost:
        (originalPromptTokens / 1_000_000) * 0.15 +
        (completionTokensEstimate / 1_000_000) * 0.6,
    };
  }
  ...
  try {
    ...
    loggingService.warn(
      `No pricing data found for ${providerStr}/${model}, using fallback pricing for optimized`,
    );
    // Use fallback pricing (GPT-4o-mini rates as default)
    optimizedCostEstimate = {
      inputCost: 0,
      outputCost: (optimizedResponseTokens / 1_000_000) * 0.6,
      totalCost: (optimizedResponseTokens / 1_000_000) * 0.6,
    };
  }

  // Calculate actual cost difference (can be negative)
  const actualCostDifference =
    originalCostEstimate.totalCost - optimizedCostEstimate.totalCost;
  const actualCostPercentage =
    originalCostEstimate.totalCost > 0
