...
export function wrapBedrockClient(
  ...
  (client as any).send = async function (command: any): Promise<any> {
    ...
    const requestId =
      context.requestId ||
      `bedrock_${Date.now()}_${Math.random().toString(36).substring(7)}`;
 ...
 */
export function createLoggedBedrockClient(
  context: BedrockLogContext = {},
): BedrockRuntimeClient {
...
function categorizeBedrockError(error: any): string {
  ...
  if (errorName.includes('Throttling') || statusCode === 429)
    return 'throttling';
  if (errorName.includes('AccessDenied') || statusCode === 403)
    return 'auth_error';
  if (errorName.includes('ValidationException') || statusCode === 400)
    return 'validation_error';
  if (errorName.includes('ServiceQuotaExceededException'))
...
export function extractBedrockContext(req: any): BedrockLogContext {
  return {
    ...
    traceId:
      req.body?.traceId || req.query?.traceId || req.gatewayContext?.traceId,
