...
export const initializeCronJobs = () => {
  ...
  cron.schedule('0 * * * *', async () => {
    ...
    try {
      ...
      const { User } = await import('../schemas/user/user.schema');
      const freeUsers = await User.find({
        isActive: true,
      })
        .select('_id usage subscriptionId')
  ...
  cron.schedule('0 4 * * *', async () => {
    ...
    try {
      const { accountClosureService } =
        await import('../services/accountClosure.service');
  ...
  cron.schedule('0 */4 * * *', async () => {
    ...
    try {
      ...
      for (const subscription of subscriptions) {
        await SubscriptionService.checkUsageAlerts(
