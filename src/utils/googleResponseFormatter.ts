 ...
 */
export function formatDriveFilesResponse(
  files: any[],
  action: 'list' | 'search',
...
export function formatDocsListResponse(files: any[]): GoogleFormattedResponse {
  ...
  const docList = files
    .map((file) => `📄 **${file.name || 'Untitled Document'}**`)
    .join('\n');
 ...
 */
export function formatSheetsListResponse(
  files: any[],
): GoogleFormattedResponse {
  if (!files || files.length === 0) {
    return {
  ...
  const sheetList = files
    .map((file) => `📊 **${file.name || 'Untitled Spreadsheet'}**`)
    .join('\n');
 ...
 */
export function formatGmailMessagesResponse(
  messages: any[],
  action: 'list' | 'search',
): GoogleFormattedResponse {
  ...
  const emailList = messages
    .map((msg) => {
      ...
      const subject =
        msg.subject || msg.snippet?.substring(0, 50) || '(No subject)';
 ...
 */
export function formatCalendarEventsResponse(
  events: any[],
): GoogleFormattedResponse {
 ...
 */
export function formatDocumentReadResponse(
  documentId: string,
  content: string,
  name?: string,
): GoogleFormattedResponse {
...
export function formatGoogleServiceResponse(
  ...
  try {
    ...
    if (service === 'gmail' || service === 'email') {
      if (action === 'search' || action === 'find') {
        return formatGmailMessagesResponse(
    ...
    if (service === 'calendar') {
      if (action === 'list' || action === 'events') {
        return formatCalendarEventsResponse(
