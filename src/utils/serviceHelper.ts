...
export class ServiceHelper {
  private static logger = new Logger('ServiceHelper');

  /**
   * Standardize service method responses
   */
  static createSuccessResponse<T>(data: T, metadata?: any) {
    return {
      success: true,
      data,
      ...(metadata && { metadata }),
   ...
   */
  static createErrorResponse(error: string, code?: string) {
    return {
      success: false,
      error,
      ...(code && { code }),
   ...
   */
  static logMethodEntry(methodName: string, params?: any) {
   ...
   */
  static logMethodExit(methodName: string, result?: any, duration?: number) {
    this.logger.debug(`Exiting ${methodName}`, { result, duration });
  }

  /**
   * Log service method error
   */
  static logMethodError(methodName: string, error: Error, params?: any) {
  ...
  static async withErrorHandling<T>(
    ...
    try {
      ...
      this.logger.error(`Operation failed: ${context.operationName}`, {
        ...context,
        error: error.message || String(error),
        stack: error.stack,
      });
  ...
  /**
   * Simple retry operation with exponential backoff
   * Basic retry implementation without external dependencies
   *
  ...
  static async withCircuitBreaker<T>(
    ...
    try {
      ...
      if (breaker.state === 'half-open') {
        ...
        breaker.failureCount = 0;
        this.logger.log(`Circuit breaker recovered`, {
          operationName,
        });
      }
  ...
  static async bulkOperation<T>(
    ...
    this.logger.log(`Starting bulk operation`, {
      operationName,
      totalItems,
      batchSize,
    });
    ...
    for (let i = 0; i < totalItems; i += batchSize) {
      ...
      try {
        ...
        this.logger.error(`Bulk operation batch failed`, {
          operationName,
          batchNumber,
          totalBatches,
          error: error.message || String(error),
  ...
  static async bulkOperationWithErrors<T>(
    ...
    this.logger.log(`Starting bulk operation with error tolerance`, {
      operationName,
      totalItems,
      batchSize,
    });
    ...
    for (let i = 0; i < totalItems; i += batchSize) {
      ...
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') {
          ...
          this.logger.warn(`Bulk operation item failed`, {
            operationName,
            batchNumber,
            itemIndex: i + index,
            error:
    ...
    this.logger.log(`Bulk operation completed with errors`, {
      operationName,
      totalItems,
      successes,
      failures,
