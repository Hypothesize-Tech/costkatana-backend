...
export async function validateTelemetryConfig(): Promise<string[]> {
  ...
  try {
    ...
    if (vendorConfig) {
      loggingService.info(
        'Vendor configuration found, validating requirements',
        {
          component: 'TelemetryValidator',
          operation: 'validateTelemetryConfig',
          type: 'telemetry_validation',
    ...
    loggingService.info(
      '=== TELEMETRY CONFIGURATION VALIDATION COMPLETED ===',
      {
        component: 'TelemetryValidator',
        operation: 'validateTelemetryConfig',
        type: 'telemetry_validation',
 ...
 */
function validateVendorConfig(vendorConfig: {
  vendor: string;
  config: any;
}): string[] {
  const startTime = Date.now();
  ...
  switch (vendor) {
    case 'datadog':
      if (
        !config.apiKey &&
        !process.env.OTEL_EXPORTER_OTLP_HEADERS?.includes('DD-API-KEY')
      ) {
        const issue =
          'Datadog requires DD_API_KEY or API key in OTEL_EXPORTER_OTLP_HEADERS';
        issues.push(issue);
    ...
    case 'newrelic':
      if (
        !config.apiKey &&
        !process.env.OTEL_EXPORTER_OTLP_HEADERS?.includes('Api-Key')
      ) {
        const issue =
          'New Relic requires NEW_RELIC_API_KEY or API key in OTEL_EXPORTER_OTLP_HEADERS';
        issues.push(issue);
    ...
    case 'grafana':
      if (
        !config.apiKey &&
        !process.env.OTEL_EXPORTER_OTLP_HEADERS?.includes('Authorization')
      ) {
        const issue =
          'Grafana Cloud requires GRAFANA_API_KEY or auth in OTEL_EXPORTER_OTLP_HEADERS';
        issues.push(issue);
    ...
    case 'honeycomb':
      if (
        !config.apiKey &&
        !process.env.OTEL_EXPORTER_OTLP_HEADERS?.includes('x-honeycomb-team')
      ) {
        const issue =
          'Honeycomb requires HONEYCOMB_API_KEY or team key in OTEL_EXPORTER_OTLP_HEADERS';
        issues.push(issue);
        ...
        loggingService.warn('Honeycomb configuration issue detected', {
          ...
          hasApiKey: !!config.apiKey,
          hasHeader:
            !!process.env.OTEL_EXPORTER_OTLP_HEADERS?.includes(
              'x-honeycomb-team',
            ),
        });
      }
...
export async function checkTelemetryHealth(): Promise<{
  ...
  try {
    ...
    try {
      const { Telemetry } = await import('../schemas/core/telemetry.schema');
...
export function startCollectorWatchdog(): void {
  ...
  if (!process.env.ENABLE_COLLECTOR_WATCHDOG) {
    loggingService.info(
      'Collector watchdog disabled via ENABLE_COLLECTOR_WATCHDOG=false',
      {
        component: 'TelemetryValidator',
        operation: 'startCollectorWatchdog',
        type: 'collector_watchdog',
  ...
  const checkInterval = parseInt(
    process.env.COLLECTOR_WATCHDOG_INTERVAL || '60000',
    10,
  );
    ...
    try {
      ...
      if (!healthy) {
        ...
        restart.on('exit', (code) => {
          if (code === 0) {
            loggingService.info(
              'Collector restart command executed successfully',
              {
                component: 'TelemetryValidator',
                operation: 'startCollectorWatchdog',
                type: 'collector_watchdog',
            ...
            loggingService.warn(
              'Collector restart command exited with non-zero code',
              {
                component: 'TelemetryValidator',
                operation: 'startCollectorWatchdog',
                type: 'collector_watchdog',
