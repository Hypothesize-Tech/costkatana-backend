...
export class VectorMaintenanceJobs {
    ...
    const job = cron.schedule('0 3 * * 0', async () => {
      ...
      try {
        ...
        if (report.corruptedIndices.length > 0) {
          loggingService.warn(
            'Corrupted indices detected, triggering rebuild',
            {
              component: 'VectorMaintenanceJobs',
              job: 'validation',
  ...
  private async cleanupUnusedIndices(): Promise<void> {
    ...
    for (const userId of userDirs) {
      try {
        ...
        const lastAccess = await Document.findOne({
   ...
   */
  async runJob(
    jobName: 'optimization' | 'validation' | 'backup' | 'cleanup',
  ): Promise<void> {
