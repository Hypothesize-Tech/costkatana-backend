...
export class VectorizationJob {
  ...
  static async processUserMemories(): Promise<void> {
    ...
    try {
      ...
      const stats =
        await backgroundVectorizationService.vectorizeUserMemories();
      ...
      loggingService.logBusiness({
        ...
        metadata: {
          ...
          failed: stats.failed,
          successRate:
            stats.processed > 0 ? stats.success / stats.processed : 0,
        },
      });
    } catch (error) {
  ...
  static async processConversations(): Promise<void> {
    ...
    try {
      ...
      const stats =
        await backgroundVectorizationService.vectorizeConversationMemories();
  ...
  static async processMessages(): Promise<void> {
    ...
    try {
      ...
      const vectorizationStats =
        await backgroundVectorizationService.vectorizeHighValueMessages();
      ...
      const selectedCount = analysisResults.filter(
        (a: { shouldVectorize: boolean }) => a.shouldVectorize,
      ).length;
        ...
        analysis: {
          ...
          selectionRate:
            analysisResults.length > 0
              ? selectedCount / analysisResults.length
              : 0,
        },
        vectorization: {
        ...
        metadata: {
          ...
          selectionRate:
            analysisResults.length > 0
              ...
              : 0,
          successRate:
            vectorizationStats.processed > 0
              ? vectorizationStats.success / vectorizationStats.processed
              : 0,
        },
      });
    } catch (error) {
  ...
  static async performHealthCheck(): Promise<void> {
    ...
    try {
      ...
      const timeEstimates =
        await backgroundVectorizationService.estimateProcessingTime();
      ...
      loggingService.logBusiness({
        ...
        metadata: {
          ...
          userMemoryVectorization:
            healthStats.storageUsage.userMemories.percentage,
          conversationVectorization:
            healthStats.storageUsage.conversations.percentage,
      ...
      if (
        healthStats.embeddingService === 'error' ||
        healthStats.vectorIndexes === 'error'
      ) {
    ...
    return [
      {
        ...
        schedule: VectorizationJob.SCHEDULES.userMemory,
        description:
          'Vectorize user memories and preferences for personalization',
        enabled: true,
        isRunning: false,
      },
      {
        ...
        schedule: VectorizationJob.SCHEDULES.smartSampling,
        description:
          'Analyze messages to identify high-value content for vectorization',
        enabled: true,
        isRunning: false,
      },
      {
        ...
        schedule: VectorizationJob.SCHEDULES.conversations,
        description:
          'Vectorize conversation histories for context-aware responses',
        enabled: true,
        isRunning: false,
      },
      {
        ...
        schedule: VectorizationJob.SCHEDULES.messages,
        description:
          'Vectorize selected high-value messages and update selection criteria',
        enabled: true,
        isRunning: false,
      },
      {
  ...
  static async validateServices(): Promise<{
    ...
    try {
      ...
      errors.push(
        `Background vectorization service: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    try {
      ...
      errors.push(
        `Smart sampling service: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  ...
  static async emergencyStop(): Promise<void> {
    loggingService.warn(
      '🛑 Emergency stop requested for all vectorization jobs',
      {
        component: 'VectorizationJob',
      },
    );
...
export const VECTORIZATION_CRON_SCHEDULES =
  VectorizationJob.getCronExpressions();
