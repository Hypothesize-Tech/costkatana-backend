...
const modelPricingData: ModelPricing[] = [
  ...
  },
  {
    ...
    maxTokens: 1_050_000,
    capabilities: ['text-generation', 'multimodal', 'reasoning', 'premium'],
    inputPrice: 0.03,
    outputPrice: 0.18,
    contextWindow: 1_050_000,
    category: 'premium',
  ...
  },
  {
    ...
    maxTokens: 256_000,
    capabilities: ['text-generation', 'multimodal', 'efficient'],
    inputPrice: 0.00075,
    outputPrice: 0.0045,
    contextWindow: 256_000,
    category: 'balanced',
    features: ['text-generation', 'multimodal', 'efficient'],
  },
  {
    model: 'gpt-5',
    provider: 'openai',
...
export function findCheapestModel(useCase: UseCase): ModelPricing | null {
  ...
  if (useCase.complexity === 'simple') {
    suitableModels = suitableModels.filter(
      (m) =>
  ...
  if (useCase.priority === 'cost') {
    suitableModels = suitableModels.filter(
      (m) => m.inputCostPer1K <= 0.003 && m.outputCostPer1K <= 0.015,
    );
  }
  ...
  return suitableModels.reduce((cheapest, current) =>
    current.inputCostPer1K + current.outputCostPer1K <
...
export function getModelsByUseCase(useCase: UseCase): ModelPricing[] {
  ...
  switch (useCase.type) {
    ...
    case 'content-generation':
      suitableModels = suitableModels.filter(
        (m) =>
          m.capabilities.includes('text-generation') && m.maxTokens >= 8000,
      );
      break;
    case 'data-analysis':
      suitableModels = suitableModels.filter(
    ...
    case 'code-generation':
      suitableModels = suitableModels.filter(
        (m) => m.capabilities.includes('reasoning') && m.maxTokens >= 8000,
      );
      break;
    case 'summarization':
      suitableModels = suitableModels.filter(
        (m) => m.capabilities.includes('summarization') && m.maxTokens >= 4000,
      );
      break;
  }
