...
export class ModelComparisonController {
    ...
    static async getAvailableModels(_req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            ControllerHelper.handleError('getAvailableModels', error, _req as any, res, startTime);
        }
    }
    ...
    static getComparisonTable(_req: Request, res: Response): void {
        ...
        try {
            ...
            loggingService.info('Comparison table retrieved successfully', {
                duration: Date.now() - startTime,
            ...
            ControllerHelper.handleError('getComparisonTable', error, _req as any, res, startTime);
        }
    }
    ...
    static async compareModels(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            const performanceMetrics = await ModelComparisonController.getBedrockPerformanceMetrics(
                model1Pricing, 
                model2Pricing
            );
            ...
            const comparison = {
                models: {
                    model1: {
                        ...
                        description: ModelComparisonController.getModelDescription(model1Pricing),
                        releaseDate: ModelComparisonController.getModelReleaseDate(model1Pricing),
                    ...
                    model2: {
                        ...
                        description: ModelComparisonController.getModelDescription(model2Pricing),
                        releaseDate: ModelComparisonController.getModelReleaseDate(model2Pricing),
                ...
                performanceMetrics,
                benchmarks: await ModelComparisonController.getBedrockBenchmarks(model1Pricing, model2Pricing),
                recommendations: ModelComparisonController.getModelRecommendations(model1Pricing, model2Pricing, model1Cost, model2Cost),
                lastUpdated: new Date()
            };

            loggingService.info('Models compared successfully', {
                duration: Date.now() - startTime,
            ...
            ControllerHelper.handleError('compareModels', error, req as any, res, startTime);
        }
    }
    ...
    static async runPerformanceBenchmark(req: Request, res: Response): Promise<void> {
        ...
        try {
            ...
            for (const modelInfo of models) {
                ...
                try {
                    const benchmarkResults = await ModelComparisonController.runModelBenchmarks(
                        modelPricing, 
                        prompts
                    );
            ...
            loggingService.info('Performance benchmark completed', {
                duration: Date.now() - startTime,
            ...
            ControllerHelper.handleError('runPerformanceBenchmark', error, req as any, res, startTime);
        }
    }
    ...
    private static async getBedrockBenchmarks(model1: any, model2: any): Promise<any> {
        ...
        try {
            ...
            const model1Scores = await ModelComparisonController.runRealBenchmarkTests(client, model1, benchmarkTests);
            const model2Scores = await ModelComparisonController.runRealBenchmarkTests(client, model2, benchmarkTests);
            ...
            for (let i = 0; i < prompts.length; i++) {
                ...
                try {
                    ...
                    if (model.provider === 'AWS Bedrock' || model.modelId.includes('amazon.')) {
                        ...
                        // Other providers - use conservative estimated performance
                        const estimatedLatency = 2500; // Conservative estimate for unknown providers
                        const estimatedResponseLength = 200; // Conservative estimate
                        ...
                        loggingService.info(`📊 Using estimated performance for ${model.provider} ${model.modelId}: ${estimatedLatency}ms`, {
                            provider: model.provider,
                            modelId: model.modelId,
                            estimatedLatency,
                            note: 'Performance testing disabled in production'
    ...
    private static async initializeOptimizations(): Promise<void> {
        try {
            ...
            loggingService.info('Model comparison controller optimizations initialized');
        } catch (error) {
            loggingService.error('Failed to initialize model comparison optimizations:', {
