...
export class DeadLetterQueue {
  ...
  private static handlers: Map<
    string,
    (data: DeadLetterJobData) => Promise<void>
  > = new Map();
  ...
  static registerHandler(
    operation: string,
    handler: (data: DeadLetterJobData) => Promise<void>,
  ): void {
    this.handlers.set(operation, handler);
    loggingService.info(
      `Registered dead letter handler for operation: ${operation}`,
    );
  }
  ...
  static async add(data: DeadLetterJobData): Promise<string | undefined> {
    if (!this.queue) {
      loggingService.warn(
        'Dead Letter Queue not initialized, operation dropped',
        {
    ...
    try {
      const job = await this.queue.add(data.operation, data, {
  ...
  static async cleanup(
    olderThan: number = 30 * 24 * 3600 * 1000,
  ): Promise<void> {
    if (!this.queue) {
      return;
    }
