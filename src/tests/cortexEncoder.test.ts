...
import { CortexEncodingRequest, CortexConfig } from '../types/cortex.types';

// Mock BedrockService for testing
// Create a proper mock for BedrockService
const mockBedrockService = {
  invokeModel: jest.fn() as jest.MockedFunction<
    (prompt: string, model: string) => Promise<any>
  >,
};
...
(mockBedrockService.invokeModel as any).mockResolvedValue({
  content:
    '(query action:action_get target:concept_document aspect:prop_quality)',
...
describe('Cortex Encoder Service', () => {
    ...
    it('should handle conversation history context', async () => {
      const request: CortexEncodingRequest = {
        ...
        conversationHistory: [
          ...
          {
            role: 'assistant',
            content: 'I can help you with document analysis',
            timestamp: new Date(),
          },
        ],
        metadata: {
  ...
  describe('Statistics and Monitoring', () => {
    it('should track encoding statistics', async () => {
      ...
      expect(updatedStats.totalRequests).toBeGreaterThan(
  ...
  describe('Performance Characteristics', () => {
    ...
    it('should handle multiple concurrent encodings', async () => {
      ...
      expect(results.every((r) => r.cortexFrame && r.confidence > 0)).toBe(
        true,
      );
...
describe('Cortex Encoder with Different AI Responses', () => {
  ...
  it('should handle event frame responses', async () => {
    (mockBedrockService.invokeModel as any).mockResolvedValue({
      content:
        '(event action:action_analyze agent:concept_system object:concept_data tense:past)',
  ...
  it('should handle list frame responses', async () => {
    (mockBedrockService.invokeModel as any).mockResolvedValue({
      content:
        '(list name:"Documents" item_1:concept_report item_2:concept_data item_3:concept_analysis)',
      usage: { outputTokens: 15 },
    });
