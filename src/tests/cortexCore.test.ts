...
describe('CortexCoreService', () => {
  ...
  const sampleAnswerFrame: CortexAnswerFrame = {
    ...
    status: 'success',
    content:
      'The document contains valuable insights about AI cost optimization.',
  };
  ...
  beforeEach(() => {
    ...
    (CortexVocabularyService.getInstance as jest.Mock).mockReturnValue(
      mockVocabInstance,
    );
  ...
  describe('Initialization', () => {
    ...
    test('should handle initialization errors gracefully', async () => {
      mockVocabularyService.initialize.mockRejectedValue(
        new Error('Vocabulary init failed'),
      );

      await expect(coreService.initialize()).rejects.toThrow(
        'Vocabulary init failed',
      );
    });
      ...
      result.optimizations.forEach((optimization) => {
        expect(optimization).toMatchObject({
          type: expect.stringMatching(
            /^(semantic_compression|frame_merging|reference_optimization)$/,
          ),
        ...
        expect(optimization.savings.reductionPercentage).toBeGreaterThanOrEqual(
          0,
        );
    ...
    test('should calculate meaningful savings', async () => {
      ...
      if (result.optimizations.length > 0) {
        const totalSavings = result.optimizations.reduce(
          (sum, opt) => sum + opt.savings.tokensSaved,
          0,
        );
  ...
  describe('Statistics Tracking', () => {
    ...
    test('should track processing statistics', async () => {
      ...
      expect(updatedStats.totalProcessed).toBeGreaterThan(
        initialStats.totalProcessed,
      );
      expect(updatedStats.successfulOptimizations).toBeGreaterThanOrEqual(
        initialStats.successfulOptimizations,
      );
  ...
  describe('Integration Scenarios', () => {
    ...
    test('should maintain performance under load', async () => {
      ...
      const promises = Array(10)
        .fill(request)
        .map(() => coreService.process(request));
