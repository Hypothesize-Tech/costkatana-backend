...
describe('Provider Abstraction Layer - Integration Tests', () => {
  ...
  describe('ModelCapabilityRegistry', () => {
    ...
    it('should find models by capability', () => {
      const textModels = modelRegistry.findModelsByCapability([
        ModelCapability.TEXT,
      ]);
      expect(textModels.length).toBeGreaterThan(50);

      const visionModels = modelRegistry.findModelsByCapability([
    ...
    it('should check model capabilities correctly', () => {
      const hasTextCap = modelRegistry.hasCapability(
        'gpt-4o',
        ModelCapability.TEXT,
      );
      expect(hasTextCap).toBe(true);

      const hasVisionCap = modelRegistry.hasCapability(
        'gpt-4o',
        ModelCapability.VISION,
      );
  ...
  describe('Model Selection', () => {
    it('should select cost-optimized model', () => {
      ...
      expect(result.selectionReasoning.strategy).toBe(
    ...
    it('should select quality-optimized model', () => {
      ...
      expect(result).toBeDefined();
      expect(
        result.selectedModel.capabilities.has(ModelCapability.VISION),
      ).toBe(true);
    });
    ...
    it('should respect constraints', () => {
      ...
      expect(result).toBeDefined();
      expect(result.selectedModel.performance.avgLatencyMs).toBeLessThanOrEqual(
        1500,
      );
      expect(
        result.selectedModel.performance.reliabilityScore,
  ...
  describe('Performance and Scalability', () => {
    ...
    it('should handle concurrent model selections', async () => {
      ...
      const results = await Promise.all(
        requests.map((r) => Promise.resolve(r)),
      );
      const duration = Date.now() - startTime;
