...
describe('Cortex Core Infrastructure', () => {
  ...
  describe('Phase 1.1: Core Types and Interfaces', () => {
    ...
    it('should validate default Cortex configuration', () => {
      expect(DEFAULT_CORTEX_CONFIG.encoding.model).toBe(
        'global.anthropic.claude-haiku-4-5-20251001-v1:0',
      );
      expect(DEFAULT_CORTEX_CONFIG.coreProcessing.optimizationLevel).toBe(
        'balanced',
      );
  ...
  describe('Phase 1.2: Cortex Vocabulary Service', () => {
    ...
    it('should initialize vocabulary service successfully', () => {
      ...
      expect(stats.total).toBe(
        stats.actions + stats.concepts + stats.properties + stats.modifiers,
      );
    ...
    it('should detect frame types correctly', () => {
      expect(vocabularyService.detectFrameType('What is the status?')).toBe(
        'query',
      );
      expect(
        vocabularyService.detectFrameType('Who created this document?'),
      ).toBe('query');
      expect(
        vocabularyService.detectFrameType('The system processed the data'),
      ).toBe('event');
      expect(vocabularyService.detectFrameType('The document is ready')).toBe(
        'state',
      );
      expect(vocabularyService.detectFrameType('List all documents')).toBe(
        'list',
      );
      expect(
        vocabularyService.detectFrameType('Error: processing failed'),
      ).toBe('error');
    });

    it('should extract semantic roles from text', () => {
      const roles = vocabularyService.extractRoles(
        'What documents did John create yesterday?',
      );
    ...
    it('should get frame templates', () => {
      const queryTemplate = vocabularyService.getFrameTemplate(
        'information_request',
      );
      const answerTemplate =
        vocabularyService.getFrameTemplate('simple_answer');
    ...
    it('should search primitives by category', () => {
      ...
      const conceptResults = vocabularyService.searchPrimitives(
        'concept',
        'document',
      );
    ...
    it('should suggest similar primitives', () => {
      ...
      const analyzeSuggestion = suggestions.find(
        (s) => s.primitive === 'action_analyze',
      );
    ...
    it('should validate known primitives', () => {
      ...
      expect(vocabularyService.isValidPrimitive('invalid_primitive')).toBe(
        false,
      );
    });
  });
  ...
  describe('Phase 1.3: Cortex Utilities', () => {
    describe('Parsing and Serialization', () => {
      it('should parse simple Cortex string', () => {
        const cortexString =
          '(query action:action_get target:concept_document)';
      ...
      it('should parse Cortex string with nested frames', () => {
        const cortexString =
          '(query target:(entity name:"John Doe" type:concept_person))';
        const frame = parseCortexString(cortexString);
      ...
      it('should parse Cortex string with arrays', () => {
        const cortexString =
          '(state entity:concept_document properties:[prop_name, prop_status, prop_quality])';
        const frame = parseCortexString(cortexString);
      ...
      it('should handle parsing errors gracefully', () => {
        expect(() => parseCortexString('invalid cortex string')).toThrow(
          CortexError,
        );
...
describe('Phase 1 Integration: Core Infrastructure Working Together', () => {
  ...
  it('should process complete Cortex workflow with all components', async () => {
    ...
    console.log(
      `   - Vocabulary loaded: ${vocabularyService.getVocabularyStats().total} primitives`,
    );
  ...
  it('should demonstrate performance characteristics', async () => {
    ...
    const cortexString =
      '(query action:action_get target:concept_document format:"json")';
