...
jest.mock('../schemas/core/optimization.schema');
jest.mock('../schemas/user/user.schema');
jest.mock('../schemas/core/alert.schema');
...
describe('Cortex Optimization Service Integration', () => {
  ...
  const sampleOptimizationRequest = {
    userId: 'user123',
    prompt:
      'Analyze the quarterly financial performance and provide insights on revenue growth patterns.',
    service: 'openai',
    model: 'gpt-4',
  ...
  beforeEach(() => {
    ...
    (CortexEncoderService.getInstance as jest.Mock).mockReturnValue(
      mockEncoderService,
    );
    (CortexCoreService.getInstance as jest.Mock).mockReturnValue(
      mockCoreService,
    );
    (CortexDecoderService.getInstance as jest.Mock).mockReturnValue(
      mockDecoderService,
    );
  ...
  describe('Cortex Processing Integration', () => {
    ...
    test('should fall back to traditional optimization when Cortex is disabled', async () => {
      ...
      const result =
        await OptimizationService.createOptimization(requestWithoutCortex);
    ...
    test('should handle Cortex initialization failure gracefully', async () => {
      mockEncoderService.initialize.mockRejectedValue(
        new Error('Encoder init failed'),
      );
  ...
  describe('Cortex Performance', () => {
    ...
    test('should handle concurrent Cortex requests', async () => {
      const requests = Array(3)
        .fill(sampleOptimizationRequest)
        .map((req, i) => ({
          ...req,
          userId: `user${i + 1}`,
        }));
  ...
  describe('Error Handling', () => {
    test('should handle malformed Cortex configuration', async () => {
      ...
      await expect(
        OptimizationService.createOptimization(malformedRequest),
  ...
  describe('Integration Validation', () => {
    test('should maintain backward compatibility with existing optimization API', async () => {
      ...
      const result =
        await OptimizationService.createOptimization(traditionalRequest);
