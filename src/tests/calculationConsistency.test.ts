...
describe('Calculation Consistency Tests', () => {
  describe('Unified Calculation Function', () => {
    it('should calculate consistent token and cost savings', () => {
      const originalPrompt =
        'This is a very long prompt with lots of unnecessary words that could be optimized to save tokens and reduce costs significantly';
      ...
      expect(result.costSaved).toBeCloseTo(
        result.originalCost - result.optimizedCost,
        6,
      );

      // Check percentage calculations
      const expectedTokenPercentage =
        (result.tokensSaved / result.originalTokens) * 100;
      expect(result.tokensSavedPercentage).toBeCloseTo(
        expectedTokenPercentage,
        2,
      );

      const expectedCostPercentage =
        (result.costSaved / result.originalCost) * 100;
      ...
      expect(result.displayPercentage).toBe(
        Math.abs(result.tokensSavedPercentage),
      );
    });
    ...
    it('should handle cases where optimized response is longer than prompt but still saves overall', () => {
      ...
      const optimizedResponse =
        'This is a much longer response that provides a comprehensive answer with detailed information and context';
      const provider = AIProvider.OpenAI;
      const model = 'gpt-4';
  ...
  describe('Cortex Metrics Conversion', () => {
    it('should convert unified results to cortex metrics format correctly', () => {
      ...
      expect(cortexMetrics.tokenReduction.withoutCortex).toBe(
        unifiedResult.originalTokens,
      );
      expect(cortexMetrics.tokenReduction.withCortex).toBe(
        unifiedResult.optimizedTokens,
      );
      expect(cortexMetrics.tokenReduction.absoluteSavings).toBe(
        unifiedResult.tokensSaved,
      );
      expect(cortexMetrics.tokenReduction.percentageSavings).toBe(
        unifiedResult.tokensSavedPercentage,
      );

      // Check cost impact matches
      expect(cortexMetrics.costImpact.estimatedCostWithoutCortex).toBe(
        unifiedResult.originalCost,
      );
      expect(cortexMetrics.costImpact.actualCostWithCortex).toBe(
        unifiedResult.optimizedCost,
      );
      expect(cortexMetrics.costImpact.costSavings).toBe(
        unifiedResult.costSaved,
      );
      expect(cortexMetrics.costImpact.savingsPercentage).toBe(
        unifiedResult.costSavedPercentage,
      );

      // Check that quality metrics are present
      expect(cortexMetrics.qualityMetrics).toBeDefined();
      expect(cortexMetrics.qualityMetrics.clarityScore).toBeGreaterThanOrEqual(
        0,
      );
      expect(cortexMetrics.qualityMetrics.clarityScore).toBeLessThanOrEqual(
        100,
      );

      // Check performance metrics
      expect(cortexMetrics.performanceMetrics).toBeDefined();
      expect(cortexMetrics.performanceMetrics.processingTime).toBeGreaterThan(
        0,
      );

      // Check justification
      expect(cortexMetrics.justification).toBeDefined();
      expect(cortexMetrics.justification.optimizationTechniques).toBeInstanceOf(
        Array,
      );
      expect(
        cortexMetrics.justification.optimizationTechniques.length,
      ).toBeGreaterThan(0);
    });
  ...
  describe('Token to Dollar Calculation Consistency', () => {
    it('should maintain bidirectional consistency between tokens and cost', () => {
      ...
      const optimizedCostPerToken =
        result.optimizedCost / result.optimizedTokens;
      ...
      if (result.tokensSavedPercentage !== 0) {
        ...
        expect(Math.sign(result.tokensSavedPercentage)).toBe(
          Math.sign(result.costSavedPercentage),
        );
      }
    });
  });
...
export function verifyCalculationConsistency(
  ...
  const tokenMatch =
    optimization.tokensSaved === cortexMetrics.tokenReduction.absoluteSavings;
  const costMatch =
    Math.abs(optimization.costSaved - cortexMetrics.costImpact.costSavings) <
    0.0001;
  const percentageMatch =
    Math.abs(
      optimization.improvementPercentage -
        cortexMetrics.tokenReduction.percentageSavings,
    ) < 0.1;
