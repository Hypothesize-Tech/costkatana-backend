...
import { GenerationDecision } from './generation-decision.service';
...
export class GenerationQueueService {
  ...
  async enqueue(
    ...
    const calculatedPriority =
      priority ?? this.calculatePriority(decision, request);
    ...
    const insertIndex = this.queue.findIndex(
      (item) => item.priority < calculatedPriority,
    );
    if (insertIndex === -1) {
    ...
    this.logger.log('Generation request queued', {
