...
export class LanguagePrioritizerService {
  private readonly logger = new Logger(LanguagePrioritizerService.name);

  // Tooling quality scores (linters, type checkers, test runners)
  private readonly TOOLING_SCORES: Record<string, number> = {
    typescript: 0.95,
    javascript: 0.85,
    python: 0.9,
    java: 0.88,
  ...
  prioritizeLanguages(
    ...
    const priorities: LanguagePriority[] = languages.map((lang) => {
      ...
      const coverage = languageDistribution
        ? (languageDistribution[normalizedLang] || 0) /
          Object.values(languageDistribution).reduce((a, b) => a + b, 0)
        : 1 / languages.length; // Equal if no distribution

      const toolingScore = this.TOOLING_SCORES[normalizedLang] || 0.7;
      ...
      const priority =
        coverage * 0.4 + toolingScore * 0.3 + teamProficiency * 0.3;
    ...
    this.logger.log('Languages prioritized', {
      primaryLanguage,
      secondaryLanguages,
   ...
   */
  detectPrimaryLanguage(fileExtensions: string[]): string {
  ...
  getRecommendedLanguages(
    ...
    return [prioritized.primaryLanguage, ...prioritized.secondaryLanguages]
