...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private costStreamingService: CostStreamingService) {
  ...
  private setupRedisClients(): void {
    try {
      ...
      this.client.on('error', (err: any) => {
        ...
        this.logger.warn(`Redis client error: ${diagnostic}`);
        this.fallbackToInMemory();
      });

      this.readerClient.on('error', (err: any) => {
        const diagnostic = getRedisErrorDiagnostic(err);
        this.logger.warn(`Redis reader client error: ${diagnostic}`);
      });
      ...
      this.readerClient.on('connect', () => {
        this.logger.log('✅ Redis reader client connected');
      });
    } catch (error) {
      const diagnostic = getRedisErrorDiagnostic(error);
      this.logger.warn(`Failed to setup Redis clients: ${diagnostic}`);
  ...
  private fallbackToInMemory(): void {
    if (!this.isLocalDev) {
      ...
      try {
        ...
        this.logger.debug('Error during Redis client cleanup:', {
          error: err instanceof Error ? err.message : String(err),
        });
      }
  ...
  public async connect(): Promise<void> {
    if (this.isLocalDev) {
      this.logger.log('✅ Redis: Using in-memory cache for local development');
      return;
    }
    ...
    try {
      ...
      this.logger.log(`🔌 Connecting to Redis at ${maskedUrl}`);

      // Simplified connection approach - let node-redis handle retries
      const connectIfNeeded = async (client: any, name: string) => {
        if (client.isOpen) {
          this.logger.log(`${name} is already open`);
          return;
        }

        if (client.isReady) {
          this.logger.log(`${name} is already ready`);
          return;
        }

        this.logger.log(`Connecting to ${name}...`);
      ...
      const diagnostic = getRedisErrorDiagnostic(error);
      this.logger.warn(`❌ Redis connection failed: ${diagnostic}`);
      this.logger.warn('Falling back to in-memory cache');
      this.fallbackToInMemory();
    } finally {
  ...
  private async generateEmbedding(text: string): Promise<number[]> {
    try {
      ...
      try {
        ...
        this.logger.warn('AWS Bedrock embedding failed, using fallback:', {
          error:
  ...
  public async checkCache(
    ...
    try {
      ...
      if (exactMatch) {
        ...
        this.logger.log('Cache hit (exact match)', {
      ...
      if (enableDeduplication) {
        ...
        if (dedupData) {
          ...
          this.logger.log('Cache hit (deduplication)', {
      ...
      if (semanticEnabled) {
        ...
        if (semanticResults.length > 0) {
          ...
          this.logger.log('Cache hit (semantic)', {
      ...
      this.logger.log('Cache miss', { prompt: prompt.substring(0, 50) });
  ...
  private async findSemanticMatches(
    ...
    try {
      ...
      this.logger.error('Semantic match search failed:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  ...
  public async clearCache(
    ...
    try {
      ...
      if (keys.length > 0) {
        ...
        this.logger.log(`Cleared ${keys.length} cache entries`, { pattern });
      }
  ...
  private async incrementStat(stat: string, value: number = 1): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to increment stat ${stat}:`, {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  private async getTopModels(
    ...
    try {
      ...
      this.logger.error('Failed to get top models:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  ...
  public async warmupCache(
    ...
    try {
      this.logger.log(`Warming up cache with ${queries.length} entries`);
  ...
  public async exportCache(): Promise<{
    ...
    try {
      ...
      this.logger.error('Failed to export cache:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  public async importCache(data: { entries: CacheEntry[] }): Promise<void> {
    try {
      this.logger.log(`Importing ${data.entries.length} cache entries`);
      ...
      this.logger.error('Failed to import cache:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  public async set(key: string, value: any, ttl: number = 3600): Promise<void> {
    try {
      ...
      this.logger.warn('Redis set failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  public async get(key: string): Promise<any | null> {
    try {
      ...
      this.logger.warn('Redis get failed, trying in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
    ...
    try {
      ...
      this.logger.warn('In-memory fallback also failed:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async del(key: string): Promise<void> {
    try {
      ...
      this.logger.warn('Redis del failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async scanKeys(pattern: string): Promise<string[]> {
    try {
      ...
      this.logger.warn('Redis scan failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async exists(key: string): Promise<boolean> {
    try {
      ...
      this.logger.warn('Redis exists failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async getTTL(key: string): Promise<number> {
    try {
      ...
      this.logger.warn('Redis TTL failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async incr(key: string, amount: number = 1): Promise<number> {
    try {
      ...
      this.logger.warn('Redis incr failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  ...
  public async flushDB(): Promise<void> {
    try {
      ...
      this.logger.warn('Redis flushDB failed, using in-memory fallback:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
