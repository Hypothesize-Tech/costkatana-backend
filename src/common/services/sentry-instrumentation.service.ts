...
export class SentryInstrumentationService {
  private readonly logger = new Logger(SentryInstrumentationService.name);
  ...
  async instrumentAIModelCall<T>(
    ...
    // Add breadcrumb
    addBreadcrumb(`AI ${operation} started with ${modelName}`, 'ai', 'info', {
      model: modelName,
    ...
    try {
      ...
      addBreadcrumb(`AI ${operation} completed successfully`, 'ai', 'info', {
        model: modelName,
      ...
      this.logger.log('AI model call completed', {
      ...
      addBreadcrumb(`AI ${operation} failed`, 'ai', 'error', {
        model: modelName,
        duration: `${duration}ms`,
  ...
  recordBusinessEvent(
    ...
    this.logger.log('Business event recorded', {
