...
export class ErrorHandlerService {
  ...
  public processError(
    ...
    // Determine if it's already a ServiceError or HttpException
    let code: string;
  ...
  public createServiceError(
    ...
    // Return an object that can be thrown and handled by HttpExceptionFilter
    return {
      ...processed,
      toHttpException: () => ({
        ...
        details: {
          ...
          component: processed.context.component,
          operation: processed.context.operation,
          shouldRetry: processed.shouldRetry,
          retryAfter: processed.retryAfter,
        },
      }),
    };
  }
   ...
   */
  private classifyError(error: any): { code: string; statusCode: number } {
  ...
  private categorizeError(message: string): ErrorCategory {
    ...
    for (const [category, patterns] of Object.entries(
      ErrorHandlerService.ERROR_PATTERNS,
    )) {
   ...
   * Determine retry behavior
   */
  private determineRetryBehavior(
    category: ErrorCategory,
    statusCode: number,
    component: string,
    ...
    if (
      ...
      const errorCount = ErrorHandlerService.errorCount.get(errorKey) || 0;
  ...
  private logError(error: ProcessedError): void {
    ...
    switch (error.severity) {
      ...
      case ErrorSeverity.LOW:
        this.logger.log(`LOW SEVERITY: ${error.message}`, logData);
        break;
    }
  }
      ...
      if (
        ...
        Sentry.withScope((scope) => {
          ...
          Sentry.captureException(
            originalError || new Error(processedError.message),
          );
        });
      }
    } catch (reportingError) {
      this.logger.warn('Failed to report error to monitoring systems', {
        errorId: processedError.id,
        reportingError:
   ...
   */
  private trackErrorPattern(error: ProcessedError): void {
    const patternKey = `${error.context.component || 'unknown'}_${error.category}`;
    const currentCount = ErrorHandlerService.errorCount.get(patternKey) || 0;

    ErrorHandlerService.errorCount.set(patternKey, currentCount + 1);
    ErrorHandlerService.lastErrorTime.set(patternKey, Date.now());
  ...
  public getErrorStats(): {
    ...
    const recentErrors = Array.from(ErrorHandlerService.errorCount.entries())
      .map(([pattern, count]) => ({
        pattern,
        count,
        lastOccurrence: new Date(
          ErrorHandlerService.lastErrorTime.get(pattern) || 0,
        ),
      }))
      .sort((a, b) => b.lastOccurrence.getTime() - a.lastOccurrence.getTime());

    return {
      errorCounts: Object.fromEntries(ErrorHandlerService.errorCount),
  ...
  public resetTracking(): void {
    ErrorHandlerService.errorCount.clear();
    ErrorHandlerService.lastErrorTime.clear();
  }
}
