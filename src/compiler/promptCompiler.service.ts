...
export class PromptCompilerService {
    ...
    try {
      ...
      if (optimizationLevel > 0) {
        ...
        if (optimizationLevel >= 3 && !preserveQuality) {
          ...
          const aggressiveResult = await this.aggressiveOptimization(
            optimizedAST,
            targetTokens,
          );
  ...
  private static async parse(prompt: string): Promise<ProgramNode> {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      const priority = this.calculateContextPriority(
        segment.text,
        i,
        segments.length,
      );
   ...
   */
  private static segmentPrompt(
    prompt: string,
  ): Array<{ text: string; type: 'sentence' | 'paragraph' | 'section' }> {
    ...
    for (const paragraph of paragraphs) {
      ...
      if (
        paragraph.match(/^#{1,6}\s+/) ||
        (paragraph.length < 100 && paragraph.match(/^[A-Z][A-Z\s]+$/))
      ) {
    ...
    return segments.length > 0
      ? segments
      : [{ text: prompt, type: 'sentence' }];
  }

  /**
   * Detect instruction patterns (imperative verbs)
   */
  private static detectInstruction(
    text: string,
  ): { directive: string; subject: string; modifiers: string[] } | null {
    ...
    for (const pattern of imperativePatterns) {
      ...
      if (match) {
        ...
        if (text.match(/\b(detailed|comprehensive|thorough|in-depth)\b/i))
          modifiers.push('detailed');
        if (text.match(/\b(creative|original|innovative)\b/i))
          modifiers.push('creative');
        if (text.match(/\b(factual|accurate|precise)\b/i))
          modifiers.push('factual');
        if (text.match(/\b(step-by-step|stepwise|sequential)\b/i))
          modifiers.push('step-by-step');
   ...
   */
  private static detectConstraint(text: string): {
    type: 'length' | 'format' | 'style' | 'tone' | 'custom';
    value: string;
    strict: boolean;
  } | null {
    // Length constraints
    if (
      text.match(
        /\b(within|max|maximum|at most|up to|limit|restrict)\s+(\d+)\s*(words|characters|chars|tokens|sentences|paragraphs)\b/i,
      )
    ) {
    ...
    if (
      text.match(
        /\b(format|structure|organize|arrange)\s+(as|in|into)\s+(json|markdown|code|yaml|xml|csv|table|list|bullets?|numbered)\b/i,
      )
    ) {
      const match = text.match(
        /\b(format|structure|organize|arrange)\s+(as|in|into)\s+(json|markdown|code|yaml|xml|csv|table|list|bullets?|numbered)\b/i,
    ...
    if (
      text.match(
        /\b(style|written|composed|formatted)\s+(as|in|like)\s+(.+?)(?:\.|$)/i,
      )
    ) {
      const match = text.match(
        /\b(style|written|composed|formatted)\s+(as|in|like)\s+(.+?)(?:\.|$)/i,
    ...
    if (
      text.match(
        /\b(tone|mood|voice)\s+(should be|must be|is|be)\s+(professional|casual|formal|informal|friendly|serious|humorous|technical|academic|conversational)\b/i,
      )
    ) {
      const match = text.match(
        /\b(tone|mood|voice)\s+(should be|must be|is|be)\s+(professional|casual|formal|informal|friendly|serious|humorous|technical|academic|conversational)\b/i,
   ...
   * Detect output format requirements
   */
  private static detectOutputFormat(text: string): {
    format: 'json' | 'markdown' | 'code' | 'text' | 'list';
    schema?: any;
  } | null {
    // JSON format
    if (
      text.match(/\b(json|JSON)\b/i) ||
      text.match(/\breturn\s+(a\s+)?json/i)
    ) {
      // Try to extract schema hints
      const schemaMatch = text.match(
        /\b(?:with|including|containing)\s+(?:fields?|properties?|keys?)\s*:?\s*([^.]+)/i,
      );
      return {
        format: 'json',
    ...
    // Code format
    if (
      text.match(/\b(code|source|programming|syntax)\b/i) ||
      text.match(/\b(?:in|using|with)\s+(\w+)\s+code/i)
    ) {
   ...
   */
  private static calculateContextPriority(
    text: string,
    index: number,
    ...
    if (
      text.match(
        /\b(important|critical|essential|required|necessary|must|key|primary)\b/i,
      )
    )
      priority += 2;
    if (text.match(/\b(context|background|information|details?|data)\b/i))
      priority += 1;
    ...
    if (
      text.match(/\b(optional|additional|extra|supplementary|nice to have)\b/i)
    )
      priority -= 2;
    if (text.match(/\b(example|sample|instance|illustration)\b/i))
      priority -= 1;
  ...
  private static isContextRequired(text: string): boolean {
    return (
      text.match(
        /\b(required|necessary|essential|critical|must|mandatory)\b/i,
      ) !== null || text.match(/\b(important|key|primary|vital)\b/i) !== null
    );
  }
   ...
   */
  private static determineContextScope(
    text: string,
    index: number,
  ): 'global' | 'local' {
    if (text.match(/\b(global|general|universal|overall|system-wide)\b/i))
      return 'global';
  ...
  private static extractModifiers(
    ...
    const modifierPatterns = [
      ...
      {
        pattern:
          /\b(very|extremely|highly)\s+(detailed|comprehensive|thorough)\b/i,
        type: 'detailed',
        strength: 0.9,
      },
      {
        pattern: /\b(detailed|comprehensive|thorough|in-depth)\b/i,
        type: 'detailed',
        strength: 0.7,
      },
      {
        pattern: /\b(creative|original|innovative|imaginative)\b/i,
        type: 'creative',
        strength: 0.8,
      },
      {
        pattern: /\b(factual|accurate|precise|exact)\b/i,
        type: 'factual',
        strength: 0.8,
      },
      {
        pattern: /\b(step-by-step|stepwise|sequential)\b/i,
        type: 'step-by-step',
        strength: 0.9,
      },
    ];
  ...
  private static extractDependencies(body: StatementNode[]): string[] {
    ...
    for (let i = 0; i < body.length; i++) {
      ...
      if (i > 0 && node.type === 'Instruction') {
        const instructionNode = node as any;
        const subjectText =
          typeof instructionNode.subject?.value === 'string'
        ...
        for (let j = 0; j < i; j++) {
          ...
          if (prevNode.type === 'Context') {
            const contextText =
              typeof (prevNode as any).content?.value === 'string'
   ...
   */
  private static extractVariableDeclarations(
    text: string,
    ...
    const letPattern =
      /\blet\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+(?:be|is|=)\s+(.+?)(?:\.|$|,)/gi;
    let match;
    ...
    const definePattern =
      /\bdefine\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+as\s+(.+?)(?:\.|$|,)/gi;
    ...
    while ((match = colonPattern.exec(text)) !== null) {
      ...
      if (
        value.length > 3 &&
        !value.match(/^(string|number|boolean|object|array)$/i)
      ) {
        declarations.push({
          name: match[1].toLowerCase(),
          value: value,
  ...
  private static findVariableInContext(
    ...
    for (let i = currentIndex - 1; i >= 0; i--) {
      ...
      if (node.type === 'Context') {
        ...
        const contextText =
          typeof contextNode.content === 'object' &&
          'value' in contextNode.content
        ...
        const varPatterns = [
          new RegExp(
            `\\b${varName}\\s+(?:is|be|equals?|=\\s*)(.+?)(?:\\.|$|,)`,
            'i',
          ),
          new RegExp(
            `\\b(?:let|define)\\s+${varName}\\s+(?:be|as|is|=)\\s+(.+?)(?:\\.|$|,)`,
            'i',
          ),
        ...
        const varIndex = words.findIndex((w) =>
          w.includes(varName.toLowerCase()),
        );
      ...
      if (node.type === 'Instruction') {
        ...
        const instructionText =
          typeof instructionNode.subject === 'object' &&
          'value' in instructionNode.subject
   ...
   */
  private static extractImplicitVariables(
    text: string,
    ...
    const ofPattern =
      /\b(?:of|about|for|from|regarding|concerning)\s+([A-Z][a-zA-Z0-9_]*)\b/g;
    let match;
  ...
  private static createInstructionNode(
    ...
    for (const decl of variableDeclarations) {
      if (!variableMap.has(decl.name)) {
        ...
        loggingService.debug('Variable declared in instruction', {
          variable: decl.name,
          value:
            decl.value.substring(0, 50) + (decl.value.length > 50 ? '...' : ''),
    ...
    for (const pattern of variableRefPatterns) {
      ...
      for (const match of matches) {
        ...
        if (
          normalizedVarName.length < 3 ||
          this.isCommonWord(normalizedVarName)
        ) {
          continue;
        }
        ...
        if (variableMap.has(normalizedVarName)) {
          ...
          const contextMatch = this.findVariableInContext(
            normalizedVarName,
            body,
            index,
          );
    ...
    for (const implicitVar of implicitVariables) {
      if (!variableMap.has(implicitVar.name)) {
        ...
        const contextValue = this.findVariableInContext(
          implicitVar.name,
          body,
          index,
        );
    ...
    const variableDependencies = Array.from(detectedVariables).filter((v) =>
      variableMap.has(v),
    );
  ...
  private static async aggressiveOptimization(
    ...
    if (typeof targetTokens === 'number' && totalTokens > targetTokens) {
      ...
      for (const node of newAst.body) {
        if (node.type === 'Context' || node.type === 'Instruction') {
          ...
          if (node.type === 'Context') {
            ...
            value =
              typeof subject?.value === 'string' ? subject.value : undefined;
          }
          ...
          if (
            ...
            const maxLen = Math.max(
              16,
              Math.floor(
                ((targetTokens - newTokenTotal) / newAst.body.length) * 4,
              ),
            );
            newValue =
    ...
    return {
      ...
      result: {
        ...
        warnings: warnings.length
          ? warnings.concat(['Aggressive optimization may affect quality'])
  ...
  private static async generateIR(
    ...
    for (const node of ast.body) {
      if (node.type === 'Instruction') {
        instructions.push({
          ...
          operands: [{ type: 'constant', value: (node.subject as any).value }],
        ...
        instructions.push({
          ...
          operands: [{ type: 'constant', value: (node.content as any).value }],
          dependencies: [],
          cost: {
            tokens: node.metadata.tokens || 0,
            estimatedMs: 0,
    ...
    return {
      ...
      metadata: {
        ...
        optimizedTokens: instructions.reduce(
          (sum, i) => sum + i.cost.tokens,
          0,
        ),
        optimizationPasses: [],
        parallelGroups: options.analysisResult.parallelizable.map(
  ...
  private static async generateCode(ir: IRProgram): Promise<string> {
    ...
    for (const instruction of ir.instructions) {
      if (
        instruction.opcode === IROpcode.PROMPT ||
        instruction.opcode === IROpcode.CONTEXT
      ) {
