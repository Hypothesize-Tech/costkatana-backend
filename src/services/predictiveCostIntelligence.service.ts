...
export class PredictiveCostIntelligenceService {
    ...
    private static async projectTokenUsage(
        userId: string,
        scopeId: string | undefined,
        targetDate: Date
    ): Promise<{
        ...
        const avgDailyTokens = recentUsage.reduce((sum, d) => sum + d.totalTokens, 0) / recentUsage.length;
        const avgPromptTokens = recentUsage.reduce((sum, d) => sum + d.averagePromptTokens, 0) / recentUsage.length;
        
        // Simple projection - would be more sophisticated in production
        return {
            promptTokens: Math.round(avgPromptTokens),
            completionTokens: Math.round(avgDailyTokens - avgPromptTokens),
            totalTokens: Math.round(avgDailyTokens)
        };
    }

    private static async generateModelRecommendations(
        userId: string,
        scopeId: string | undefined,
     ...
     */
    private static estimateCostFromTokens(totalTokens: number): number {
        // Use average pricing across models (rough estimate)
