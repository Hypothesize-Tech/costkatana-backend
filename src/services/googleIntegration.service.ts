...
export class GoogleIntegrationService {
    ...
    /**
     * Create QBR slides deck
     */
    static async createQBRSlides(
        connection: IGoogleConnection & { decryptToken: () => string; decryptRefreshToken?: () => string | undefined },
        options: CostReportOptions
    ): Promise<{ presentationId: string; presentationUrl: string; audit: IGoogleExportAudit }> {
        try {
            // Fetch usage data for QBR
            const match: any = {
                userId: new mongoose.Types.ObjectId(options.userId)
            };
            ...
            const summary = costSummary[0] || { totalCost: 0, totalRequests: 0 };
            ...
            // Create presentation
            const title = `CostKatana QBR - ${new Date().toISOString().split('T')[0]}`;
            const { presentationId, presentationUrl } = await GoogleService.createPresentation(connection, title);

            // Add summary slide
            await GoogleService.addSlideWithText(
                connection,
                presentationId,
                'Cost Summary',
                `Total Cost: $${summary.totalCost.toFixed(2)}\nTotal Requests: ${summary.totalRequests.toLocaleString()}`
            ...
            // Add top models slide
            const topModelsText = topModels
                .map((m, i) => `${i + 1}. ${m._id}: $${m.totalCost.toFixed(2)} (${m.requests} requests)`)
                .join('\n');
            await GoogleService.addSlideWithText(
                connection,
                presentationId,
            ...
            return { presentationId, presentationUrl, audit };
        } catch (error: any) {
