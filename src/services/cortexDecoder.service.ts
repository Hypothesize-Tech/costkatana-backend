...
🚨 ENHANCED FOR LARGE OUTPUTS: You can handle complex, multi-part responses from Claude Opus 4.1 including:
...
- (answer: ...) → Convert to informative responses - THIS IS THE MAIN TYPE NOW
...
- entity_X property_Y value_Z → "X's Y is Z"
- action_X target_Y → "You should X the Y" or "X the Y"
- details_X → Include X as supporting information
...
Reply ONLY JSON: {"has_information_loss": false, "issues": []}`;
    ...
    private async needsAIDecoding(cortexStructure: CortexFrame): Promise<boolean> {
        ...
        if (cortexStructure.frameType === 'answer' && 
            ...
            loggingService.info('🧩 Detected code response, using specialized decoding', {
