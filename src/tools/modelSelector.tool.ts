...
export class ModelSelectorTool extends Tool {
  ...
  private async compareModels(operation: ModelOperation): Promise<string> {
    try {
      ...
      const comparison = operation.models.map((model) => {
        ...
        const suitability = operation.useCase
          ? this.assessSuitability(model, operation.useCase)
          : null;
        ...
        return {
          ...
          estimatedCost: operation.useCase
            ? this.estimateUseCost(model, operation.useCase)
            : 0,
        };
      });
      ...
      return JSON.stringify(
        {
          ...
          savings:
            comparison.length > 1
              ? (
                  ((comparison[comparison.length - 1].estimatedCost -
                    comparison[0].estimatedCost) /
                    comparison[comparison.length - 1].estimatedCost) *
                  100
                ).toFixed(1) + '% potential savings'
              : null,
        },
        null,
        2,
  ...
  private async testModel(operation: ModelOperation): Promise<string> {
    try {
      ...
      const prompts =
        config.samplePrompts && config.samplePrompts.length > 0
      ...
      for (const prompt of prompts) {
        ...
        try {
          ...
          if (pricing && pricing.length > 0) {
            const p = pricing[0];
            cost =
              (inputTokens * (p.inputPrice ?? 0) +
                outputTokens * (p.outputPrice ?? 0)) /
              1_000_000;
          }
          ...
          const qualityScore =
            responseStr.length > 0
              ? Math.min(
                  100,
                  50 + Math.min(50, Math.floor(responseStr.length / 20)),
                )
              : 0;
      ...
      const avgResponseTime =
        successful.length > 0
          ? successful.reduce((s, r) => s + r.responseTime, 0) /
            successful.length
          : 0;
      const totalCost = testResults.reduce((s, r) => s + r.cost, 0);
      const avgQuality =
        successful.length > 0
          ? successful.reduce((s, r) => s + (r.qualityScore ?? 0), 0) /
            successful.length
          : 0;
      ...
      const config = {
        modelSettings: {
          temperature: this.getOptimalTemperature(
            operation.useCase?.type || 'api-calls',
          ),
          maxTokens: this.getOptimalMaxTokens(
            operation.useCase?.type || 'api-calls',
          ),
          topP: 0.9,
          frequencyPenalty: 0,
        ...
        optimizations: {
          ...
          compressionLevel:
            operation.useCase?.priority === 'cost' ? 'aggressive' : 'moderate',
      ...
      return JSON.stringify(
        {
          ...
          implementation: {
            codeExample: this.generateCodeExample(
              operation.testConfig?.provider || 'anthropic',
            ),
            environmentVariables: this.getRequiredEnvVars(
              operation.testConfig?.provider || 'anthropic',
            ),
      ...
      return JSON.stringify(
        {
          ...
          warnings: !validation.apiAccess
            ? ['API access not verified in current region']
            : !modelExists
    ...
    const modelMap = {
      'api-calls': [
        {
          ...
          score: 90,
          reason: 'Excellent for high-volume, cost-effective API calls',
        },
        {
          model: 'gpt-3.5-turbo',
          score: 85,
          reason: 'Fast and reliable for general API usage',
        },
        {
          model: 'claude-3-sonnet-20240229-v1:0',
          score: 75,
          reason: 'Better quality but higher cost',
        },
      ],
      chatbot: [
        {
          model: 'claude-3-sonnet-20240229-v1:0',
          score: 95,
          reason: 'Superior conversational abilities',
        },
        {
          model: 'gpt-4',
          score: 90,
          reason: 'Excellent understanding and context retention',
        },
        {
          model: 'global.anthropic.claude-haiku-4-5-20251001-v1:0',
          score: 80,
          reason: 'Cost-effective for simple conversations',
        },
      ],
      'content-generation': [
        {
          ...
          score: 95,
          reason: 'Best for creative and long-form content',
        },
        {
          model: 'gpt-4-turbo',
          score: 90,
          reason: 'Excellent creativity and coherence',
        },
        {
          model: 'claude-3-sonnet-20240229-v1:0',
          score: 85,
      ...
      'data-analysis': [
        ...
        },
        {
          model: 'gpt-4',
          score: 88,
          reason: 'Excellent for complex data interpretation',
        },
        {
          model: 'global.anthropic.claude-haiku-4-5-20251001-v1:0',
          score: 75,
          reason: 'Good for simple analysis tasks',
        },
      ],
      'code-generation': [
        {
          model: 'claude-3-sonnet-20240229-v1:0',
          score: 95,
          reason: 'Excellent code generation and debugging',
        },
        {
          model: 'gpt-4',
          score: 90,
      ...
      summarization: [
        {
          model: 'global.anthropic.claude-haiku-4-5-20251001-v1:0',
          score: 95,
          reason: 'Perfect for high-volume summarization',
        },
        {
          model: 'gpt-3.5-turbo',
          score: 88,
        ...
        },
        {
          model: 'claude-3-sonnet-20240229-v1:0',
          score: 80,
          reason: 'Better for complex documents',
        },
      ],
    };

    const models =
      modelMap[useCase.type as keyof typeof modelMap] || modelMap['api-calls'];
    ...
    return models
      .map((model) => {
        ...
        if (useCase.priority === 'cost' && model.model.includes('haiku')) {
          ...
          useCase.priority === 'speed' &&
          (model.model.includes('haiku') || model.model.includes('3.5-turbo'))
        ) {
          adjustedScore += 5;
        }
  ...
  private analyzeCosts(recommendations: any[], useCase: any) {
    ...
    const monthlyRequests =
      volumeMultiplier[useCase.volume as keyof typeof volumeMultiplier] || 1000;
    const avgTokens =
      tokenEstimate[useCase.complexity as keyof typeof tokenEstimate] || 1000;
  ...
  private assessSuitability(model: string, useCase: any) {
    ...
    const modelScores = scores[model as keyof typeof scores] || {
      cost: 50,
      speed: 50,
      quality: 50,
    };
  ...
  private analyzeTestResults(results: any, _config: any) {
    const analysis = {
      performance:
        results.responseTime < 1000
          ? 'excellent'
          : results.responseTime < 2000
            ? 'good'
            : 'slow',
      costEfficiency:
        results.cost < 0.01
    ...
    if (analysis.performance === 'slow') {
      analysis.recommendations.push(
        'Consider using a faster model like Claude 3 Haiku for better response times',
      );
    }
    if (analysis.costEfficiency === 'expensive') {
      analysis.recommendations.push(
        'Consider optimizing prompts or using a more cost-effective model',
      );
    }
    if (analysis.quality === 'needs improvement') {
      analysis.recommendations.push(
        'Consider using a higher-quality model or refining your prompts',
      );
    }
...
const response = await client.chat.completions.create({
  ...
  private isValidOperation(operation: ModelOperation): boolean {
    ...
    if (
      ...
      (!operation.models || operation.models.length < 2)
    )
      return false;
