...
export class OptimizationManagerTool extends Tool {
  ...
  private async analyzeCosts(
    ...
    try {
      ...
      const analysis = {
        summary: {
          totalAnalyzedCost: Number(
            usageAnalysis.reduce((sum, u) => sum + u.totalCost, 0).toFixed(4),
          ),
        ...
        recommendations:
          this.generateOptimizationRecommendations(opportunities),
        timeRange: timeRange,
      };
  ...
  private async recommendModels(
    ...
    try {
      ...
      for (const modelInfo of suitableModels.slice(0, 5)) {
        ...
        if (pricing && pricing.length > 0) {
          ...
          recommendations.push({
            ...
            costVsCurrent: Number(
              (
                ((projectedCost - usage.totalCost) / usage.totalCost) *
                100
              ).toFixed(1),
            ),
      ...
      const modelRecommendations = {
        ...
        summary: {
          topRecommendation:
            recommendations[0]?.model || 'No suitable models found',
          potentialSavings: recommendations[0]
            ? Math.max(
                0,
                usage.totalCost - recommendations[0].projectedMonthlyCost,
              )
            : 0,
          confidenceLevel:
            recommendations[0]?.suitabilityScore > 80
              ? 'high'
              : recommendations[0]?.suitabilityScore > 60
                ? 'medium'
                : 'low',
        },
      };
      ...
      return JSON.stringify(
        {
          ...
          insights:
            this.generateModelRecommendationInsights(modelRecommendations),
        },
        null,
        2,
      );
  ...
  private async createOptimization(
    ...
    try {
      ...
      const dynamicData =
        await this.calculateDynamicOptimizationData(operation);
      ...
      const optimizationData = {
        ...
        metadata: {
          ...
          implementationDetails:
            operation.optimizationData.implementationDetails || {},
        },
        applied: false,
        appliedCount: 0,
      ...
      return JSON.stringify(
        {
          success: true,
          message:
            'Optimization plan created successfully based on your actual usage data',
  ...
  private async calculateDynamicOptimizationData(
    operation: OptimizationOperation,
  ): Promise<any> {
    try {
      const userId = operation.userId;
      ...
      const usageData = await Usage.aggregate([
        {
          $match: {
            userId,
            createdAt: { $gte: timeRange.start, $lte: timeRange.end },
            ...(optimizationData.currentModel && {
              model: optimizationData.currentModel,
            }),
          },
        },
        {
      ...
      const avgCompletionTokens = Math.round(
        currentModelData.avgCompletionTokens || 0,
      );
      ...
      return {
        originalPrompt: this.generateOriginalPromptDescription(
          currentModel,
          avgPromptTokens,
        ),
        optimizedPrompt:
          optimizationData.description || optimizationResult.description,
        techniques: [optimizationData.type || 'model_switch'],
        originalTokens: totalTokens,
        optimizedTokens: optimizationResult.optimizedTokens,
        tokensSaved: Math.max(
          0,
          totalTokens - optimizationResult.optimizedTokens,
        ),
        ...
        improvementPercentage: Number(
          ((optimizationResult.costSaved / totalCost) * 100).toFixed(1),
        ),
        service: currentModelData.providers[0] || 'unknown',
        model: currentModel,
        recommendedModel: optimizationResult.recommendedModel || currentModel,
        category:
          optimizationData.type === 'model_switch'
            ? 'model_selection'
            : optimizationData.type === 'prompt_optimization'
              ? 'prompt_reduction'
              : optimizationData.type === 'request_batching'
                ? 'batch_processing'
  ...
  private async calculateModelSwitchOptimization(
    ...
    if (targetModel) {
      ...
      if (targetPricing && targetPricing.length > 0) {
        const newCost =
          ((avgPromptTokens * targetPricing[0].inputPrice +
            avgCompletionTokens * targetPricing[0].outputPrice) *
            totalRequests) /
          1000000;
        bestAlternative = {
          model: targetModel,
      ...
      if (alternatives.length > 0) {
        ...
        bestSavings =
          totalCurrentCost - bestAlternative.projectedCost * totalRequests;
      }
    }
  ...
  private calculatePromptOptimization(
    ...
    const optimizedPromptTokens = Math.round(
      avgPromptTokens * (1 - reductionPercentage),
    );
  ...
  private calculateBatchingOptimization(
    ...
    const batchingEfficiency =
      totalRequests > 1000 ? 0.15 : totalRequests > 100 ? 0.1 : 0.05;
  ...
  private generateOriginalPromptDescription(
    model: string,
    avgTokens: number,
  ): string {
  ...
  private createFallbackOptimizationData(
    operation: OptimizationOperation,
  ): any {
    ...
    return {
      originalPrompt:
        'No usage data available - creating template optimization',
      optimizedPrompt:
        optimizationData.description ||
        'Template cost optimization recommendation',
      techniques: [optimizationData.type || 'model_switch'],
      originalTokens: 0,
      optimizedTokens: 0,
      tokensSaved: 0,
  ...
  private async listOptimizations(
    operation: OptimizationOperation,
  ): Promise<string> {
    try {
      const userId = operation.userId;
      const projectFilter = operation.projectId
      ...
      const optimizations = await Optimization.find({
        ...
        .limit(20)
        .select(
          'category optimizedPrompt costSaved applied createdAt metadata',
        );
      ...
      const optimizationsList = {
        ...
        optimizations: optimizations.map((opt) => ({
          ...
          hasImplementationDetails:
            !!opt.metadata?.implementationDetails &&
            Object.keys(opt.metadata.implementationDetails).length > 0,
        })),
        summary: {
          totalEstimatedSavings: Number(
            optimizations
              .reduce((sum, opt) => sum + (opt.costSaved || 0), 0)
              .toFixed(4),
          ),
  ...
  private async findCheaperAlternatives(
    ...
    for (const modelId of allModels) {
      ...
      if (pricing && pricing.length > 0 && modelId !== currentModel) {
        const currentCost =
          (avgPromptTokens * currentPricing[0].inputPrice +
            avgCompletionTokens * currentPricing[0].outputPrice) /
          1000000;
        const alternativeCost =
          (avgPromptTokens * pricing[0].inputPrice +
            avgCompletionTokens * pricing[0].outputPrice) /
          1000000;
  ...
  private analyzePromptOptimization(usage: any): any {
    ...
    if (avgPromptTokens > 1000) {
      ...
      return {
        ...
        targetPromptLength: Math.round(
          avgPromptTokens * (1 - potentialReduction),
        ),
        estimatedSavings: Number(estimatedSavings.toFixed(4)),
        savingsPercentage: potentialReduction * 100,
        confidence: 75,
        reasoning:
          'Long prompts detected - optimization through prompt engineering can reduce token usage',
        implementation: {
          steps: [
  ...
  private analyzeBatchingOpportunity(usage: any): any {
    ...
    if (totalRequests > 100) {
      ...
      return {
        ...
        confidence: 80,
        reasoning:
          'High request volume detected - batching can reduce overhead costs',
        implementation: {
          steps: [
  ...
  private generateOptimizationRecommendations(opportunities: any[]): string[] {
    ...
    if (opportunities.length === 0) {
      recommendations.push(
        'Your current setup is well optimized. Continue monitoring for future opportunities.',
      );
    ...
    recommendations.push(
      `Priority optimization: ${topOpportunity.type} could save $${topOpportunity.estimatedSavings} (${topOpportunity.savingsPercentage}%)`,
    );

    const modelSwitches = opportunities.filter(
      (o) => o.type === 'model_switch',
    ).length;
    if (modelSwitches > 0) {
      recommendations.push(
        `${modelSwitches} model switch opportunities identified`,
      );
    }

    const promptOpts = opportunities.filter(
      (o) => o.type === 'prompt_optimization',
    ).length;
    if (promptOpts > 0) {
      recommendations.push(
        `${promptOpts} prompt optimization opportunities found`,
      );
    }
  ...
  private generateAnalysisInsights(analysis: any): string[] {
    ...
    if (
      ...
      analysis.summary.totalAnalyzedCost * 0.2
    ) {
      insights.push(
        `High optimization potential detected: ${analysis.summary.averageSavingsPercentage}% average savings available`,
      );
    }

    if (analysis.opportunities.length > 5) {
      insights.push(
        `Multiple optimization opportunities found - consider implementing in phases`,
      );
    }

    const highImpactOps = analysis.opportunities.filter(
      (op: any) => op.savingsPercentage > 30,
    ).length;
    if (highImpactOps > 0) {
      insights.push(
        `${highImpactOps} high-impact optimizations (>30% savings) identified`,
      );
    }
  ...
  private calculateProjectedCost(
    ...
    const inputCost =
      (avgPromptTokens * pricing[0].inputPrice * totalRequests) / 1000000;
    const outputCost =
      (avgCompletionTokens * pricing[0].outputPrice * totalRequests) / 1000000;
  ...
  private getModelConsiderations(modelId: string, usage: any): string[] {
    ...
    if (pricing && pricing.length > 0) {
      if (
        pricing[0].contextWindow <
        usage.avgPromptTokens + usage.avgCompletionTokens
      ) {
      ...
      if (pricing[0].category === 'premium') {
        considerations.push(
          'Higher cost per request - ensure quality benefits justify expense',
        );
      }
    }
  ...
  private generateModelRecommendationInsights(recommendations: any): string[] {
    ...
    insights.push(
      `Current usage: ${current.useCase} with ${current.volume} volume and ${current.complexity} complexity`,
    );

    if (recommendations.summary.potentialSavings > 0) {
      insights.push(
        `Top recommendation could save $${recommendations.summary.potentialSavings.toFixed(2)} monthly`,
      );
    }

    if (recommendations.summary.confidenceLevel === 'high') {
      insights.push(
        'High confidence in recommendations based on usage patterns',
      );
    }
  ...
  private getOptimizationNextSteps(type: string): string[] {
    ...
    return (
      steps[type] || [
        'Review optimization details and create implementation plan',
      ]
    );
  }
    ...
    try {
      ...
      for (const row of usageAnalysis) {
        ...
        if (avgPrompt > 2000) {
          reductionPct = 0.3;
          suggestion =
            'Remove redundant context and use prompt templates; trim to essential instructions.';
        } else if (avgPrompt > 1000) {
          reductionPct = 0.2;
          suggestion =
            'Audit prompts for redundancy; use variables for repeated blocks.';
        ...
        const estimatedSavings = Number(
          (row.totalCost * reductionPct).toFixed(4),
        );
        recommendations.push({
          model: row._id,
          suggestion,
  ...
  private async applyOptimization(
    ...
    try {
      if (!operation.userId) {
        return JSON.stringify({
          success: false,
          message: 'userId is required to apply optimization.',
        });
      }
      const optimizationId = (operation as unknown as Record<string, unknown>)
        .optimizationId as string | undefined;
      if (!optimizationId) {
        return JSON.stringify(
          {
            success: true,
            ...
            message:
              'Apply optimization requires optimizationId. Use list_optimizations to get IDs.',
            applied: 0,
          },
          null,
          2,
  ...
  private async bulkAnalysis(
    ...
    try {
      ...
      if (!userId) {
        return JSON.stringify({
          success: false,
          message: 'userId is required for bulk analysis.',
        });
      }
      const timeRange = this.getTimeRange(operation.analysisParams?.timeRange);
      const match: any = {
  ...
  private async costForecast(
    ...
    try {
      ...
      if (!userId) {
        return JSON.stringify({
          success: false,
          message: 'userId is required for cost forecast.',
        });
      }
      const timeRange = this.getTimeRange(operation.analysisParams?.timeRange);
      const days = Math.max(
