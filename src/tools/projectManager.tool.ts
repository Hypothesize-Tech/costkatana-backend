...
export class ProjectManagerTool extends Tool {
  ...
  private async createProject(operation: ProjectOperation): Promise<string> {
    try {
      ...
      const smartDefaults = this.generateSmartDefaults(
        operation.projectData.type || 'custom',
      );

      // Get user's workspace
      const userWorkspace = await User.findById(operation.userId).select(
        'workspaceId',
      );
      ...
      const projectData = {
        ...
        description:
          operation.projectData.description ||
          `AI project: ${operation.projectData.name}`,
        ownerId: operation.userId,
        workspaceId: userWorkspace.workspaceId,
        budget: {
          amount:
            operation.projectData.settings?.budgetLimit ||
            smartDefaults.settings.budgetLimit,
          ...
          alerts: [
            {
              threshold:
                operation.projectData.settings?.alertThreshold ||
                smartDefaults.settings.alertThreshold,
              type: 'email' as const,
              recipients: [],
            },
          ],
        ...
        settings: {
          allowedModels:
            operation.projectData.settings?.preferredModels ||
            smartDefaults.settings.preferredModels,
  ...
  private async updateProject(operation: ProjectOperation): Promise<string> {
    try {
      ...
      if (operation.projectData) {
        if (operation.projectData.name)
          project.name = operation.projectData.name;
        if (operation.projectData.description)
          project.description = operation.projectData.description;
        if (operation.projectData.settings) {
          project.settings = {
            ...project.settings,
            ...operation.projectData.settings,
          };
        }
      }
  ...
  private isValidOperation(operation: ProjectOperation): boolean {
    ...
    if (
      ['update', 'get', 'delete', 'configure'].includes(operation.operation) &&
      !operation.projectId
    ) {
      return false;
    }
