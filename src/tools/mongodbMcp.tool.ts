...
import {
  getMongodbMcpService,
  MCPToolResult,
} from '../modules/mongodb-mcp/services/mongodb-mcp.service';
import { getMongoDBConnectionModel } from '../common/utils/get-mongoose-models';
import { MongodbResultFormatterService } from '../modules/mongodb-mcp/services/mongodb-result-formatter.service';
...
abstract class BaseMongoDBTool extends Tool {
  ...
  protected async executeTool(toolName: string, args: any): Promise<string> {
    try {
      ...
      const mcpService = getMongodbMcpService();
      const result: MCPToolResult = await mcpService.executeToolCall(
        this.userId,
        this.connectionId!,
        toolName,
        args,
