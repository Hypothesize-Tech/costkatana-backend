...
import { getGoogleSearchService } from '../modules/utils/services/google-search.service';
...
export class WebSearchTool extends Tool {
  ...
  private async performSearch(
    ...
    if (!getGoogleSearchService().isConfigured()) {
      return {
        success: false,
        operation: 'search',
        data: {},
        error:
          'Google Search API is not configured. Please set GOOGLE_SEARCH_API_KEY and GOOGLE_SEARCH_ENGINE_ID environment variables.',
      };
    }
    ...
    try {
      const searchOptions = {
        ...
        domains: request.options?.costDomains
          ? SEARCH_CONFIG.COST_DOMAINS
          : undefined,
      };

      // Perform search
      const searchResults = await getGoogleSearchService().search(
        request.query,
        searchOptions,
      );
      ...
      // Get quota status
      const quotaStatus = await getGoogleSearchService().getQuotaStatus();
