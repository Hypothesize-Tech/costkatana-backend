...
export class ContextManager {
  ...
  private extractTopics(
    ...
    if (response.metadata?.topics) {
      (response.metadata.topics as string[]).forEach((topic) =>
        topics.add(topic),
      );
    }
   ...
   */
  private async loadContextFromCache(
    sessionId: string,
  ): Promise<ContextFrame | null> {
    try {
      const key = `${this.contextCachePrefix}${sessionId}`;
      const cached = await getCacheService().get(key);
      ...
      loggingService.error('Failed to load context from cache', {
        sessionId,
        error,
      });
      return null;
  ...
  private async saveContextToCache(context: ContextFrame): Promise<void> {
    try {
      ...
      await getCacheService().set(key, cacheable, 3600); // Cache for 1 hour
    } catch (error) {
  ...
  public async clearContext(sessionId: string): Promise<void> {
    ...
    const key = `${this.contextCachePrefix}${sessionId}`;
    await getCacheService().delete(key);
