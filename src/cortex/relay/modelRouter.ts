...
export class ModelRouter {
  ...
  public async selectModel(query: CortexQuery): Promise<ModelSelection> {
    ...
    if (
      query.routingPreferences?.preferredModels &&
      query.routingPreferences.preferredModels.length > 0
    ) {
  ...
  private analyzeComplexity(query: CortexQuery): number {
    ...
    const taskCount = Object.keys(query.roles).filter((k) =>
      k.startsWith('task_'),
    ).length;
    if (taskCount > 1) {
  ...
  private extractRequirements(query: CortexQuery): string[] {
    ...
    if (queryStr.includes('create') || queryStr.includes('generate'))
      requirements.push('creative');
    if (queryStr.includes('translate')) requirements.push('translation');
    if (queryStr.includes('predict') || queryStr.includes('forecast'))
      requirements.push('prediction');
  ...
  private selectByComplexity(
    ...
    const suitable = candidates.filter(
      (p) => complexity <= p.complexityThreshold,
    );
  ...
  private applyCostConstraints(
    ...
    const cheaper = this.modelProfiles
      ...
      .sort(
        (a, b) => b.capabilities.costPerToken - a.capabilities.costPerToken,
      );
  ...
  private createModelSelection(
    ...
    const estimatedCost =
      (estimatedTokens / 1000000) * profile.capabilities.costPerToken;
