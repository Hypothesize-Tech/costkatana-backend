...
export class IntelligentModelSelector {
  ...
  public async getBestReasoningModel(): Promise<string> {
    ...
    // Check cache
    const cached = await getCacheService().get<string>(cacheKey);
    if (cached) {
      loggingService.info('Using cached best reasoning model', {
        modelId: cached,
      });
      return cached;
    }
    ...
    for (const modelId of this.reasoningModelPriority) {
      ...
      if (testResult.accessible) {
        // Cache for 1 hour
        await getCacheService().set(cacheKey, modelId, 3600);

        if (modelId.includes('opus')) {
          loggingService.info(
            '🎉 Claude Opus is accessible! Using most powerful reasoning model',
            {
              modelId,
    ...
    loggingService.warn('No priority models accessible, using fallback', {
      fallback,
    });
    return fallback;
  }
  ...
  public async getBestEncodingModel(): Promise<string> {
    ...
    const cached = await getCacheService().get<string>(cacheKey);
    if (cached) {
      loggingService.info('Using cached best encoding model', {
        modelId: cached,
      });
      return cached;
    }
    ...
    const fallback = 'amazon.nova-pro-v1:0';
    loggingService.warn(
      'No priority encoding models accessible, using fallback',
      { fallback },
    );
    return fallback;
  }
  ...
  private async testModelAccess(modelId: string): Promise<ModelTestResult> {
    ...
    try {
      ...
      if (
        error.message?.includes('on-demand throughput') ||
  ...
  public async executeWithOpusFallback(
    ...
    const opusModels = this.reasoningModelPriority.filter((m) =>
      m.includes('opus'),
    );
    ...
    for (const modelId of opusModels) {
      try {
        ...
        loggingService.info('🎉 Successfully used Claude Opus for reasoning!', {
          modelId,
        });
        ...
        loggingService.debug('Opus model not accessible, trying next', {
          modelId,
        });
      }
    }
  ...
  public async getModelAccessStatus(): Promise<{
    ...
    const opusAvailable = reasoningResults.some(
      (r) => r.modelId.includes('opus') && r.accessible,
    );
