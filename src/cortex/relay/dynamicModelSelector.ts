...
export class DynamicModelSelector {
  ...
  public async getBestReasoningModel(): Promise<string> {
    ...
    await getCacheService().set(cacheKey, bestModel, 3600);
  ...
  public async getBestModelForRequirements(requirements: {
    ...
    for (const [modelId, capabilities] of Object.entries(
      ...
      // Check requirements
      if (
        requirements.minReasoning &&
        capabilities.reasoning < requirements.minReasoning
      )
        continue;
      if (requirements.minSpeed && capabilities.speed < requirements.minSpeed)
        continue;
      if (requirements.maxCost && 1 - capabilities.cost > requirements.maxCost)
        continue;
      if (
      ...
      // Calculate score
      const score =
        (capabilities.reasoning + capabilities.speed + capabilities.cost) / 3;
