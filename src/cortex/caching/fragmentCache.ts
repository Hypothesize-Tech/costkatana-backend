...
export class FragmentCacheManager {
  ...
  private initializeStats(): void {
    Object.values(FragmentType).forEach((type) => {
      this.fragmentStats.set(type as FragmentType, {
        hits: 0,
        misses: 0,
        savings: 0,
      });
    ...
    this.traverseExpression(expression, (expr, path) => {
      ...
      if (fragment) {
        ...
        if (fragment.expression && 'dependencies' in fragment.expression) {
          dependencies.set(
            fragment.key,
            fragment.expression.dependencies as string[],
          );
        }
      }
    });
  ...
  public async getFragments(
    keys: string[],
  ): Promise<Map<string, CachedFragment>> {
  ...
  /**
   * Get all cache keys matching a pattern (uses Redis SCAN via getCacheService().keys)
   */
  private async getAllCacheKeys(pattern: string): Promise<string[]> {
    try {
      const keys = await getCacheService().keys(pattern);
   ...
   */
  private async updateFragmentMetadata(
    key: string,
    action: 'hit' | 'miss',
  ): Promise<void> {
