...
export class PrimitiveLearner {
  ...
  constructor() {
    ...
    this.learningRate = parseFloat(
      process.env.CORTEX_PRIMITIVE_LEARNING_RATE || '0.1',
    );
    this.minConfidenceThreshold = 0.7;
    this.maxVocabularySize = parseInt(
      process.env.CORTEX_MAX_PRIMITIVE_COUNT || '100000',
    );
  ...
  public async analyzeInteraction(
    ...
    if (
      !process.env.CORTEX_DYNAMIC_PRIMITIVE_LEARNING ||
    ...
    try {
      ...
      for (const term of unknownTerms) {
        ...
        const primitive = await this.learnPrimitive(
          term,
          input,
          _query,
          response,
      ...
      loggingService.warn('Failed to analyze interaction for learning', {
        error,
      });
    }
...
examples[2]{example}:
  ...
  example_usage_2`;
      ...
      const modelId =
        process.env.CORTEX_LEARNING_MODEL || 'amazon.nova-lite-v1:0';
      ...
      const request = BedrockModelFormatter.formatRequestBody({
        ...
        systemPrompt:
          'You are a semantic analyzer for the Cortex meta-language. Identify meaningful primitives.',
        maxTokens: 500,
        temperature: 0.3,
      });
      ...
      const responseBody = JSON.parse(
        new TextDecoder().decode(analysisResponse.result.body),
      );
      const analysisText = BedrockModelFormatter.parseResponseBody(
        modelId,
        responseBody,
      );
      ...
      if (
        !analysis.isPrimitive ||
        analysis.confidence < this.minConfidenceThreshold
      ) {
        return null;
      }
  ...
  private async updateLearningMetrics(
    _newPrimitives: LearnedPrimitive[],
  ): Promise<void> {
  ...
  private async loadLearnedPrimitives(): Promise<void> {
    try {
      const cached = await getCacheService().get('cortex:learned:primitives');
...
Format:
{
  "primitives": [
    {
      ...
      const request = BedrockModelFormatter.formatRequestBody({
        ...
        systemPrompt:
          'You are a vocabulary designer for the Cortex meta-language.',
        maxTokens: 1000,
        temperature: 0.7,
      });
      ...
      const responseBody = JSON.parse(
        new TextDecoder().decode(suggestionResponse.result.body),
      );
      const suggestionText = BedrockModelFormatter.parseResponseBody(
        modelId,
        responseBody,
      );
      ...
      loggingService.warn('Failed to suggest vocabulary expansion', {
        domain,
        error,
      });
    }
  ...
  public getMetrics(): LearningMetrics {
    return {
      ...
      learningRate: this.learningRate,
      vocabularyGrowth:
        this.learnedPrimitives.size > 0
          ? this.learnedPrimitives.size /
            Object.keys(CorePrimitives).flat().length
          : 0,
    };
  }
}
