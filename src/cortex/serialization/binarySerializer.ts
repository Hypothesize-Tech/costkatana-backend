  ...
  public static async serialize(
    ...
    try {
      ...
      if (options.encrypt) {
        ...
        if (!encryptionKey) {
          throw new Error(
            'CORTEX_ENCRYPTION_KEY environment variable is required',
          );
        }
...
export class SchemaBasedSerializer {
  ...
  public decode(buffer: Buffer): any {
    ...
    if (version !== this.schema.version) {
      throw new Error(
        `Schema version mismatch: expected ${this.schema.version}, got ${version}`,
      );
    }
    ...
    while (offset < buffer.length) {
      ...
      result[fieldName] = this.decodeValue(
        valueBuffer,
        field.type,
        field.repeated,
      );
