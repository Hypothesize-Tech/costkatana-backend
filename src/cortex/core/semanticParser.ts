...
export class TrueSemanticParser {
  ...
  public parseToTrueCortex(input: string): string {
    ...
    const cortexExpression = this.generateLispFormat(
      taskTrees,
   ...
   */
  private convertRolesToPrimitives(
    roles: Record<string, any>,
  ): Record<string, any> {
  ...
  private formatValue(
    ...
    const refKey = Object.keys(references).find(
      (key) => references[key] === value,
    );
    if (refKey) {
    ...
    if (Array.isArray(value)) {
      const items = value
        .map((item) => this.formatValue(item, references, indent))
        .join(', ');
      return ` [${items}]`;
    }
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      const nested = Object.entries(value)
        .map(
          ([key, val]) =>
            `${indent}    ${key}:${this.formatValue(val, references, indent + '  ')}`,
        )
        .join('\n');
  ...
  private extractAction(text: string): number | null {
    const actionWords = [
      ...
      {
        words: ['get', 'getting', 'retrieve', 'find'],
        id: PrimitiveIds.action_get,
      },
      {
        words: ['analyze', 'analysis', 'examining'],
        id: PrimitiveIds.action_analyze,
      },
      { words: ['summarize', 'summary'], id: PrimitiveIds.action_summarize },
      { words: ['create', 'creating', 'make'], id: PrimitiveIds.action_create },
      {
        words: ['compare', 'comparing', 'comparison'],
        id: PrimitiveIds.action_compare,
      },
      {
        words: ['explain', 'explaining', 'describe'],
        id: PrimitiveIds.action_explain,
      },
    ];
  ...
  private extractTarget(text: string): any {
    ...
    const entityPatterns = [
      {
        pattern:
          /\b(latest|newest|recent)\s+(star\s*wars|starwars)\s+(movie|film)\b/i,
        entity: {
          concept: PrimitiveIds.concept_movie,
          franchise: 'Star Wars',
    ...
    for (const { pattern, agent } of subjectPatterns) {
      ...
      if (match) {
        ...
        return {
          ...agent,
          properties,
          definiteness: this.extractDefiniteness(match[0]),
        };
      }
    }
  ...
  private extractDefiniteness(text: string): number {
    return text.includes('the ')
      ? PrimitiveIds.mod_definite
      : PrimitiveIds.mod_indefinite;
  }
  ...
  private findSharedEntity(task1: string, task2: string): string | null {
    ...
    for (const entity of entities) {
      if (
        task1.toLowerCase().includes(entity) &&
        task2.toLowerCase().includes(entity)
      ) {
        return entity;
      }
    }
  ...
  private formatResponseData(data: any, indent: string): string {
    if (Array.isArray(data)) {
      const items = data
        .map((item, index) => `${indent}item_${index + 1}: "${item}"`)
        .join('\n');
...
export class ReferenceResolver {
  ...
  private formatEntityReference(entity: any): string {
    return `(entity:${entity.concept || 'unknown'} ${Object.entries(entity)
      .map(([k, v]) => `${k}:${v}`)
      .join(' ')})`;
  }
}
