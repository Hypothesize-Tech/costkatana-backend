...
const logger = new Logger('MongoDBMCPConfig');
...
// Validation
if (
  !MongoDBMCPConfig.ENCRYPTION_KEY ||
  MongoDBMCPConfig.ENCRYPTION_KEY === 'default-key-change-me'
) {
  logger.warn(
    'WARNING: MongoDB MCP encryption key not properly configured. Set JWT_SECRET.',
  );
}
