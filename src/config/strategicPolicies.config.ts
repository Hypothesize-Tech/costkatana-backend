...
export const DEFAULT_CORTEX_OPERATION_POLICY: CortexOperationPolicy = {
  ...
  operationConfig: {
    ...
    compress: {
      ...
      tradeoff:
        'High savings: 65% token reduction, 300ms overhead, slight quality loss',
    },
    analyze: {
    ...
    sast: {
      ...
      tradeoff:
        'Semantic focus: 40% token reduction, 250ms overhead, 92% preservation',
    },
    answer: {
      ...
      tradeoff:
        'Legacy support: Same as optimize, maintained for compatibility',
    },
  },
};
 ...
 */
export const FALLBACK_PRICING_POLICIES: Record<
...
> = {
  conservative: {
    ...
    pricingRates: {
      inputCostPer1M: 0.5, // 75th percentile (~GPT-4 level)
      outputCostPer1M: 2.0,
    },
    rationale:
      'Safer for budget planning - better to overestimate than underestimate',
    accuracy: 0.7, // Within 30% of actual
    risk: 'low',
  },
  ...
  optimistic: {
    strategy: 'optimistic',
    pricingRates: {
      inputCostPer1M: 0.1, // 25th percentile (~Gemini Flash level)
      outputCostPer1M: 0.4,
    },
    rationale: 'Lower estimates for cost-sensitive scenarios',
    accuracy: 0.5, // Within 50% of actual
    risk: 'high',
  },
  ...
  balanced: {
    ...
    accuracy: 0.6, // Within 40% of actual
    risk: 'medium',
  },
  ...
  strict: {
    ...
    accuracy: 1.0, // 100% (requires exact pricing)
    risk: 'low',
  },
};
...
export const ROUTING_STRATEGY_POLICIES: Record<string, RoutingStrategyPolicy> =
  {
    cost: {
      name: 'Cost-Optimized',
      weights: { cost: 0.6, latency: 0.15, quality: 0.25 },
    ...
    balanced: {
      ...
      tradeoff:
        'Balance all three: 800ms latency, $0.005/request, 93% satisfaction',
    },
    ...
    quality: {
      ...
      weights: { cost: 0.1, latency: 0.15, quality: 0.75 },
      ...
      tradeoff:
        'Maximum quality: 4s latency acceptable, $0.02/request, 98% satisfaction',
    },
  };
...
export function getStrategicPolicies(): StrategicPoliciesConfig {
  const config: StrategicPoliciesConfig = {
    ...
    fallbackPricing:
      (process.env.FALLBACK_PRICING_STRATEGY as FallbackPricingStrategy) ||
      'balanced',
  ...
  if (!FALLBACK_PRICING_POLICIES[config.fallbackPricing]) {
    loggingService.warn(
      `Invalid fallback pricing strategy: ${config.fallbackPricing}, using 'balanced'`,
    );
  ...
  if (!ROUTING_STRATEGY_POLICIES[config.routingStrategy]) {
    loggingService.warn(
      `Invalid routing strategy: ${config.routingStrategy}, using 'balanced'`,
    );
