...
export const CUSTOMER_KILL_SWITCH_METHODS = {
  DELETE_ROLE: {
    ...
    description:
      'Immediately revokes all access. CostKatana cannot assume a deleted role.',
    reversible: true,
  ...
  REMOVE_TRUST: {
    ...
    description:
      'Edit the role trust policy to remove the CostKatana AWS account.',
    reversible: true,
    timeToEffect: 'Immediate',
  },
  DENY_ASSUME: {
...
export class KillSwitchService implements OnModuleInit, OnModuleDestroy {
   ...
   */
  async activateKillSwitch(activation: KillSwitchActivation): Promise<void> {
    ...
    switch (activation.scope) {
      case 'global':
        ...
        this.logger.error('CRITICAL: Global kill switch activated', {
      ...
      case 'customer':
        if (!activation.id) {
          throw new Error(
            'Customer ID required for customer-scoped kill switch',
          );
        }
        this.state.perCustomer.set(activation.id, entry);
        this.logger.warn('Customer kill switch activated', {
          component: 'KillSwitchService',
          operation: 'activateKillSwitch',
          scope: 'customer',
      ...
      case 'service':
        if (!activation.id) {
          throw new Error(
            'Service name required for service-scoped kill switch',
          );
        }
        this.state.perService.set(activation.id, entry);
        this.logger.warn('Service kill switch activated', {
          component: 'KillSwitchService',
          operation: 'activateKillSwitch',
          scope: 'service',
      ...
      case 'connection':
        if (!activation.id) {
          throw new Error(
            'Connection ID required for connection-scoped kill switch',
          );
        }
        this.state.perConnection.set(activation.id, entry);
        this.logger.warn('Connection kill switch activated', {
          component: 'KillSwitchService',
          operation: 'activateKillSwitch',
          scope: 'connection',
   ...
   */
  enableReadOnlyMode(activatedBy: string, reason: KillSwitchReason): void {
    this.state.readOnlyMode = true;

    this.logger.warn('Read-only mode enabled', {
      component: 'KillSwitchService',
   ...
   */
  disableReadOnlyMode(deactivatedBy: string): void {
    this.state.readOnlyMode = false;

    this.logger.log('Read-only mode disabled', {
      component: 'KillSwitchService',
   ...
   */
  checkKillSwitch(request: ExecutionRequest): KillSwitchCheckResult {
   ...
   */
  getScopeState(scope: KillSwitchScope): KillSwitchEntry[] {
    switch (scope) {
      case 'customer':
   ...
   */
  getCustomerKillSwitchMethods(): typeof CUSTOMER_KILL_SWITCH_METHODS {
   ...
   */
  getEmergencyStopInstructions(roleArn: string): string {
    return `
    ...
    this.expirationCheckerInterval = setInterval(() => {
      ...
      for (const [id, entry] of this.state.perCustomer) {
        if (entry.expiresAt && entry.expiresAt < now) {
          this.state.perCustomer.delete(id);
          this.logger.log('Customer kill switch expired', {
            component: 'KillSwitchService',
            customerId: id,
          });
        }
      ...
      for (const [id, entry] of this.state.perService) {
        if (entry.expiresAt && entry.expiresAt < now) {
          this.state.perService.delete(id);
          this.logger.log('Service kill switch expired', {
            component: 'KillSwitchService',
            service: id,
          });
        }
      ...
      for (const [id, entry] of this.state.perConnection) {
        if (entry.expiresAt && entry.expiresAt < now) {
          this.state.perConnection.delete(id);
          this.logger.log('Connection kill switch expired', {
            component: 'KillSwitchService',
            connectionId: id,
          });
        }
