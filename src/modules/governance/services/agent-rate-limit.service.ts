...
export class AgentRateLimitService {
  private readonly logger = new Logger(AgentRateLimitService.name);
  ...
  constructor(
    ...
    private readonly agentIdentityModel: Model<IAgentIdentity>,
  ...
  async checkRateLimits(
    ...
    try {
      ...
      this.loggingService.error('Rate limit check error', {
  ...
  private async checkAgentLimits(
    ...
    const agentLimits = {
      requestsPerMinute:
        agentIdentity.maxRequestsPerMinute || limits.requestsPerMinute,
      requestsPerHour:
        agentIdentity.maxRequestsPerHour || limits.requestsPerHour,
      maxConcurrent:
        agentIdentity.maxConcurrentExecutions || limits.maxConcurrent,
    };
   ...
   */
  async getRateLimitStatus(agentId: string): Promise<{
    ...
    return {
      minute: {
        current: minute,
        limit: this.DEFAULT_LIMITS.agent!.requestsPerMinute,
      },
      hour: {
        current: hour,
        limit: this.DEFAULT_LIMITS.agent!.requestsPerHour,
      },
      concurrent: {
        current: concurrent,
        limit: this.DEFAULT_LIMITS.agent!.maxConcurrent,
      },
    };
  }
   ...
   */
  async resetRateLimits(agentId: string): Promise<void> {
    const keys = [
    ...
    this.loggingService.warn('Rate limits reset for agent', {
      component: 'AgentRateLimitService',
  ...
  private mergeLimits(
    ...
    return {
      organization: {
        ...this.DEFAULT_LIMITS.organization!,
        ...customLimits?.organization,
      },
  ...
  private async increment(key: string, ttl: number): Promise<void> {
    try {
      ...
      await this.cacheService.set(key, (current + 1).toString(), ttl);
    } catch (error) {
  ...
  private async decrement(key: string): Promise<void> {
    try {
      const current = await this.getCount(key);
      if (current > 0) {
        await this.cacheService.set(key, (current - 1).toString(), 300);
      }
    } catch (error) {
  ...
  private async deleteKey(key: string): Promise<void> {
    try {
      ...
      this.loggingService.error('Failed to delete key', {
        component: 'AgentRateLimitService',
        operation: 'deleteKey',
        key,
        error: error instanceof Error ? error.message : String(error),
