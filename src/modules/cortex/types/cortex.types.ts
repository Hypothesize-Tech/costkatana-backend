...
// ============================================================================
// CORTEX SERVICE / GATEWAY TYPES
// ============================================================================

/** Query type for CortexService.process() (gateway/runtime pipeline) */
...
/** Response from Cortex core (gateway pipeline) */
export interface CortexResponse {
  output: CortexFrame;
  modelUsed?: string;
  processingTime?: number;
  metadata?: { cacheHit?: boolean; [key: string]: unknown };
}

/** Metrics returned with CortexService.process() response */
...
/** Model selection for Cortex pipeline stages */
...
/** Runtime/gateway configuration for CortexService */
export interface CortexServiceConfig {
  enabled: boolean;
  mode?: 'mandatory' | 'optional' | 'disabled';
  optimization?: { tokenReduction: boolean; semanticCaching: boolean };
  models?: { core: string; encoder: string; decoder: string };
  gateway?: {
    headerName?: string;
    queryParam?: string;
    cookieName?: string;
...
export interface CortexGatewayHeaders {
  ...
  'x-costkatana-cortex-optimization-level'?:
    | 'conservative'
    | 'balanced'
    | 'aggressive';
}
 ...
 */
export type FrameRoles<T extends CortexFrameType> = T extends 'query'
  ? keyof CortexQueryFrame
  : T extends 'answer'
    ? keyof CortexAnswerFrame
    : T extends 'event'
      ? keyof CortexEventFrame
      : T extends 'state'
        ? keyof CortexStateFrame
        : T extends 'entity'
          ? keyof CortexEntityFrame
          : T extends 'list'
            ? keyof CortexListFrame
            : T extends 'error'
              ? keyof CortexErrorFrame
              : never;
...
export const isCortexFrame = (value: any): value is CortexFrame => {
  return (
    ...
    value !== null &&
    'frameType' in value &&
    ['query', 'answer', 'event', 'state', 'entity', 'list', 'error'].includes(
      value.frameType,
    )
  );
};
 ...
 */
export const isQueryFrame = (frame: CortexFrame): frame is CortexQueryFrame =>
  frame.frameType === 'query';
export const isAnswerFrame = (frame: CortexFrame): frame is CortexAnswerFrame =>
  frame.frameType === 'answer';
export const isEventFrame = (frame: CortexFrame): frame is CortexEventFrame =>
  frame.frameType === 'event';
export const isStateFrame = (frame: CortexFrame): frame is CortexStateFrame =>
  frame.frameType === 'state';
export const isEntityFrame = (frame: CortexFrame): frame is CortexEntityFrame =>
  frame.frameType === 'entity';
export const isListFrame = (frame: CortexFrame): frame is CortexListFrame =>
  frame.frameType === 'list';
export const isErrorFrame = (frame: CortexFrame): frame is CortexErrorFrame =>
  frame.frameType === 'error';

/**
 * Default Cortex configuration
 */
/** Encoder: Claude 3.5 Haiku - $0.80/$4 per 1M, 8K output. Use v1 (v2 invalid on Bedrock) */
const CORTEX_ENCODER = 'anthropic.claude-3-5-haiku-20241022-v1:0';
/** Decoder: Mistral Large 3 - $0.50/$1.50 per 1M, 8K output. AWS model ID: mistral-large-3-675b-instruct */
const CORTEX_DECODER = 'mistral.mistral-large-3-675b-instruct';
/** Sonnet 4.6 - US cross-region inference profile (required for on-demand; plain model ID not supported) */
const CORTEX_SONNET_46 = 'us.anthropic.claude-sonnet-4-6';

export const DEFAULT_CORTEX_CONFIG: CortexConfig = {
  encoding: {
    model: CORTEX_ENCODER,
    strategy: 'balanced',
  },
  coreProcessing: {
    model: CORTEX_SONNET_46,
    optimizationLevel: 'balanced',
  },
  decoding: {
    model: CORTEX_DECODER,
