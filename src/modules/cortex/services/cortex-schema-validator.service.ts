/**
 * Cortex Schema Validation Service (NestJS)
 *
...
export class CortexSchemaValidatorService {
  private readonly logger = new Logger(CortexSchemaValidatorService.name);
    ...
    try {
      ...
      if (!frame) {
        errors.push({
          ...
          message: 'Frame parameter is required',
          path: 'root',
          severity: 'critical',
        });
      ...
      if (schema.constraints?.customValidations) {
        for (const customValidation of schema.constraints.customValidations) {
          try {
            const customResult = customValidation.validate(frame);
            ...
            errors.push({
              ...
              message: `Custom validation '${customValidation.name}' failed: ${error instanceof Error ? error.message : String(error)}`,
              path: 'root',
              severity: 'error',
            });
          }
      ...
      this.logger.debug(
        `🔍 Cortex schema validation completed for ${frame.frameType}`,
        {
          valid:
            errors.filter(
  ...
  public getSchema(
    frameType: CortexFrameType,
  ): CortexSchemaDefinition | undefined {
  ...
  private validateFrameStructure(
    ...
    if (strict) {
      const allowedRoles = [
        ...schema.requiredRoles,
        ...schema.optionalRoles,
        ...schema.deprecatedRoles,
      ];
      for (const role of Object.keys(frame)) {
        if (role === 'frameType') continue;
  ...
  private validateRoleConstraints(
    ...
    for (const [role, value] of Object.entries(frame)) {
      ...
      // If no schema for this role and strict mode is ON, treat as error. Otherwise, skip.
      if (!roleSchema) {
        if (strict) {
          errors.push({
            code: ValidationErrorCode.INVALID_ROLE_TYPE,
            message: `Role '${role}' is not defined in schema and strict mode is enabled.`,
            path: role,
            severity: 'error',
          });
        }
      ...
      if (roleSchema.constraints) {
        this.validateRoleConstraint(
          role,
          value,
          roleSchema.constraints,
          errors,
          warnings,
        );
      }
  ...
  private validateSchemaConstraints(
    ...
    const roleCount = Object.keys(frame).filter(
      (k) => k !== 'frameType',
    ).length;

    // Strict mode: Always enforce minRoles and maxRoles
    if (
      constraints.minRoles !== undefined &&
      (strict
        ? roleCount < constraints.minRoles
        : constraints.minRoles && roleCount < constraints.minRoles)
    ) {
      errors.push({
        code: ValidationErrorCode.CONSTRAINT_VIOLATION,
    ...
    if (
      constraints.maxRoles !== undefined &&
      (strict
        ? roleCount > constraints.maxRoles
        : constraints.maxRoles && roleCount > constraints.maxRoles)
    ) {
      errors.push({
        code: ValidationErrorCode.CONSTRAINT_VIOLATION,
    ...
    if (constraints.conditionalRequired) {
      for (const condition of constraints.conditionalRequired) {
        if (condition.if in frame) {
          const missingRequired = condition.then.filter(
            (role) => !(role in frame),
          );
          if (missingRequired.length > 0) {
            // In strict mode, treat as error; otherwise warning
            errors.push({
  ...
  private calculateQualityScore(
    ...
    const requiredRolesPresent = schema.requiredRoles.filter(
      (role) => role in frame,
    ).length;
    const requiredRolesRatio =
      requiredRolesPresent / schema.requiredRoles.length;
    score += requiredRolesRatio * 10;

    const optionalRolesPresent = schema.optionalRoles.filter(
      (role) => role in frame,
    ).length;
