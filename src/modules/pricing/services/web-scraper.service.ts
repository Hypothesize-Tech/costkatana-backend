...
export class WebScraperService {
  ...
  private getRandomUserAgent(): string {
    return WebScraperService.USER_AGENTS[
      Math.floor(Math.random() * WebScraperService.USER_AGENTS.length)
    ];
  }

  async scrapeProviderPricing(provider: string): Promise<ScrapedPricingData> {
    const url =
      WebScraperService.PROVIDER_URLS[
        provider as keyof typeof WebScraperService.PROVIDER_URLS
      ];
    ...
    try {
      ...
      if (content && content.length > 100) {
        this.logger.log(
          `Successfully scraped ${content.length} characters for ${provider}`,
        );
        return {
          provider,
          url: fullUrl,
      ...
      this.logger.error(
        `Direct scraping failed for ${provider}: ${errorMessage}`,
      );
  ...
  async scrapeAllProviders(): Promise<ScrapedPricingData[]> {
    const providers = Object.keys(WebScraperService.PROVIDER_URLS);
    const results: ScrapedPricingData[] = [];

    this.logger.log(
      `Starting to scrape pricing data for ${providers.length} providers`,
    );
    ...
    for (const provider of providers) {
      try {
        ...
        results.push(result);
        this.logger.log(
          `Scraped ${provider}: ${result.success ? 'success' : 'failed'}`,
        );
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        this.logger.error(`Failed to scrape ${provider}: ${errorMessage}`);
        results.push({
          provider,
          url:
            WebScraperService.PROVIDER_URLS[
    ...
    this.logger.log(
      `Completed scraping: ${successCount}/${providers.length} providers successful`,
    );

    return results;
  }

  async testScraping(provider: string): Promise<ScrapedPricingData> {
    this.logger.log(`Testing scraping for ${provider}`);
    return await this.scrapeProviderPricing(provider);
  }

  private getFallbackContent(provider: string): string {
