...
export class UserPreferenceService {
  ...
  async getUserPreferences(userId: string): Promise<UserPreferenceData | null> {
    try {
      ...
      this.loggerService.error('❌ Failed to get user preferences:', {
  ...
  async updatePreferences(
    ...
    try {
      this.loggerService.info(`🔧 Updating preferences for user: ${userId}`);
      ...
      this.loggerService.info(`✅ Updated preferences for user: ${userId}`);
    } catch (error) {
      this.loggerService.error('❌ Failed to update user preferences:', {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
    ...
    try {
      ...
      if (context) {
        ...
        if (
          ...
          contextLower.includes('programming')
        ) {
          return 'amazon.nova-pro-v1:0'; // Good for coding
        }

        if (
          contextLower.includes('creative') ||
          contextLower.includes('writing')
        ) {
          return 'anthropic.claude-3-5-sonnet-20240620-v1:0'; // Good for creative tasks
        }

        if (
          contextLower.includes('analysis') ||
      ...
      this.loggerService.error('❌ Failed to get recommended model:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 'amazon.nova-pro-v1:0';
    }
   ...
   */
  async getRecommendedChatMode(
    userId: string,
  ): Promise<'fastest' | 'cheapest' | 'balanced'> {
    try {
      const preferences = await this.getUserPreferences(userId);
      ...
      this.loggerService.error('❌ Failed to get recommended chat mode:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 'balanced';
    }
  ...
  async learnFromInteraction(
    ...
    try {
      ...
      if (
        ...
        interaction.userSatisfaction >= 4
      ) {
        await this.updatePreferences(userId, {
      ...
      this.loggerService.info(
        `📚 Learned from interaction for user: ${userId}`,
      );
    } catch (error) {
      this.loggerService.error('❌ Failed to learn from interaction:', {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
  ...
  async getPreferenceSummary(userId: string): Promise<string> {
    try {
      ...
      this.loggerService.error('❌ Failed to get preference summary:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async resetPreferences(userId: string): Promise<void> {
    try {
      this.loggerService.info(`🔄 Resetting preferences for user: ${userId}`);

      await this.userPreferenceModel.deleteOne({ userId });
      this.preferenceCache.delete(userId);

      this.loggerService.info(`✅ Reset preferences for user: ${userId}`);
    } catch (error) {
      this.loggerService.error('❌ Failed to reset user preferences:', {
        error: error instanceof Error ? error.message : String(error),
      });
      throw error;
    }
  ...
  async exportPreferences(userId: string): Promise<any> {
    try {
      ...
      this.loggerService.error('❌ Failed to export preferences:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return {};
    }
