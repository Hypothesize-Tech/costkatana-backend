...
/**
 * CKQLService (Cost Katana Query Language)
 *
 * Natural language to MongoDB query conversion service with AI-powered parsing,
 * vector search capabilities, and fallback pattern matching.
 */
@Injectable()
export class CKQLService {
  private readonly logger = new Logger(CKQLService.name);
  ...
  async parseQuery(
    ...
    try {
      ...
      try {
        ...
        this.logger.log(
          `CKQL query parsed with AI in ${Date.now() - startTime}ms`,
        );
        return result;
      } catch (aiError) {
        this.logger.warn('AI parsing failed or timed out, using fallback:', {
          error: aiError instanceof Error ? aiError.message : String(aiError),
        });
        return this.parseWithFallback(naturalLanguage, context);
      }
    } catch (error) {
      this.logger.error('Failed to parse CKQL query:', {
  ...
  private async parseWithAI(
    ...
    const embedding =
      await this.embeddingsService.generateEmbedding(naturalLanguage);
  ...
  private generateFallbackQuery(naturalLanguage: string, context?: any): any {
    ...
    if (lowerQuery.includes('similar') && lowerQuery.includes('cost')) {
      ...
      query.operation_name = {
        $in: ['gen_ai.chat.completions', 'http.get', 'http.post'],
      };
      if (!query.timestamp) {
        query.timestamp = { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) }; // Last 24 hours
      }
    ...
    if (
      ...
      (Object.keys(query).length === 2 && query.tenant_id && query.workspace_id)
    ) {
  ...
  async executeQuery(
    ...
    try {
      ...
      const needsSemanticSearch = this.needsSemanticSearch(
        query.naturalLanguage,
      );

      if (needsSemanticSearch && query.vectorSearch) {
        const vectorResults = await this.executeVectorSearchOptimized(
          query,
          limit,
          offset,
        );
        ...
        if (results.length === 0) {
          this.logger.log(
            'Vector search returned no results, trying fallback query',
          );
      ...
      const insights = await this.generateInsightsOptimized(
        query.naturalLanguage,
        results,
      );
      ...
      this.logger.error('Failed to execute CKQL query:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private async executeSingleAggregationQuery(
    ...
    try {
      ...
      const [aggregationResult] = await this.telemetryModel.aggregate(pipeline);
      ...
      this.debugLog(
        'Single aggregation query failed, falling back to separate queries:',
        { error },
      );
      ...
      const totalCount = await this.telemetryModel.countDocuments(matchQuery);
...
4. NEVER use aggregation math operators: $avg, $sum, $max, $min, $add, $multiply, $subtract, $divide, $mod, $abs, $ceil, $floor, $round, $sqrt, $pow, $log, $log10, $ln, $exp
...
Return ONLY the JSON query object:`;
    ...
    try {
      ...
      if (cleanedQueryText.startsWith('```json')) {
        cleanedQueryText = cleanedQueryText
          .replace(/^```json\s*/, '')
          .replace(/\s*```$/, '');
      ...
      try {
        ...
        this.logger.error('JSON parse failed for cleaned query text:', {
        ...
        try {
          ...
          this.logger.log('Emergency cleaning succeeded');
        } catch (emergencyError) {
          this.logger.error('Emergency cleaning also failed, using fallback');
      ...
      this.logger.error('Failed to generate MongoDB query:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private async executeVectorSearchOptimized(
    ...
    try {
      ...
      const optimalCandidates = this.calculateOptimalCandidates(
        limit,
        queryComplexity,
      );
      ...
      const results = aggregationResult?.results || [];
      const totalCount =
        aggregationResult?.totalCount?.[0]?.count || results.length;

      return { results, totalCount };
    } catch (error) {
      this.logger.error(
        'Optimized vector search failed, falling back to regular search:',
        {
          error: error instanceof Error ? error.message : String(error),
        },
      );
   ...
   */
  private calculateOptimalCandidates(
    limit: number,
...
Provide a 1-sentence explanation of what this query will find.`;
   ...
   */
  private async generateInsightsOptimized(
    naturalLanguage: string,
    results: any[],
  ): Promise<string[]> {
    if (results.length === 0) {
      return [
        'No data found matching your query. Try adjusting the time range or search terms.',
      ];
    }
    ...
    try {
      ...
      try {
        const aiInsightPromise = this.generateAIInsight(
          naturalLanguage,
          results,
        );
        ...
        this.debugLog(
          'AI insight generation failed or timed out, using basic insights only:',
          {
            error: aiError instanceof Error ? aiError.message : String(aiError),
          },
        );
  ...
  private generateBasicInsights(results: any[]): string[] {
    ...
    const uniqueModels = Array.from(
      new Set(results.filter((r) => r.gen_ai_model).map((r) => r.gen_ai_model)),
    ).length;

    // Cost insights
    if (totalCost > 0) {
      insights.push(
        `Total cost: $${totalCost.toFixed(4)} across ${results.length} operations`,
      );

      if (totalCost > 1) {
        insights.push(
          '⚠️ High cost detected - consider optimizing expensive operations',
        );
      }
    }
    ...
    if (avgDuration > 0) {
      ...
      if (avgDuration > 5000) {
        insights.push(
          '🐌 Slow operations detected - investigate performance bottlenecks',
        );
      }
    }
    ...
    try {
      const summary = {
        ...
        avgDuration:
          results.reduce((sum, r) => sum + (r.duration_ms || 0), 0) /
          results.length,
        operations: [...new Set(results.map((r) => r.operation_name))].slice(
          0,
          3,
        ),
...
Provide a single, specific recommendation for optimization or investigation.`;
  ...
  private fixMalformedQuery(query: any): any {
    try {
      ...
      this.logger.warn('Failed to fix malformed query, using original:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return query;
    }
  ...
  private cleanMongoShellSyntax(queryText: string): string {
    try {
      this.debugLog('Original query text before shell cleaning:', {
        value: queryText,
      });
      ...
      queryText = queryText.replace(
        /ISODate\s*\(\s*([^)]*new Date[^)]*)\s*\)/g,
        `"${new Date().toISOString()}"`,
      );
      queryText = queryText.replace(
        /ISODate\s*\(\s*([^)]*Date\.now[^)]*)\s*\)/g,
        `"${new Date().toISOString()}"`,
      );
      ...
      this.debugLog('Query text after shell cleaning:', { value: queryText });
      return queryText;
    } catch (error) {
      this.logger.warn('Failed to clean MongoDB shell syntax:', {
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private emergencyQueryCleaning(queryText: string): string {
    try {
      this.debugLog('Emergency cleaning input:', { value: queryText });
      ...
      this.debugLog('Emergency cleaning output:', { value: queryText });
      return queryText;
    } catch (error) {
      this.logger.error('Emergency cleaning failed:', {
        error: error instanceof Error ? error.message : String(error),
      });
      return '{"timestamp": {"$gte": "HOUR_AGO"}}';
    }
  ...
  private aggressiveQueryCleaning(queryText: string): string {
    try {
      this.debugLog('Query before aggressive cleaning:', { value: queryText });
      ...
      this.debugLog('Query after aggressive cleaning:', { value: queryText });
      return queryText;
    } catch (error) {
      this.logger.warn('Failed to perform aggressive query cleaning:', {
        error: error instanceof Error ? error.message : String(error),
      });
        ...
        if (typeof value === 'object' && value !== null) {
          ...
          for (const opKey in value) {
            if (opKey === '$in' && Array.isArray(value[opKey])) {
              ...
              fixedOperators[opKey] =
                typeof value[opKey] === 'string'
                  ? value[opKey]
                  : String(value[opKey]);
            }
          }
  ...
  private ensureSafeFindQuery(query: any): any {
    if (!query || typeof query !== 'object') {
      this.debugLog('Query is null or not object, using default safe query');
      return { timestamp: { $gte: new Date(Date.now() - 3600000) } };
    }

    this.debugLog('Original query before safety check:', {
    ...
    const cleanObject = (obj: any): any => {
      ...
      for (const key in obj) {
        if (dangerousOperators.includes(key)) {
          this.logger.warn(
            `Removing dangerous operator ${key} from find query`,
          );
          continue;
        }
    ...
    if (Object.keys(cleanedQuery).length === 0) {
      this.debugLog(
        'Query became empty after cleaning, using default safe query',
      );
      return { timestamp: { $gte: new Date(Date.now() - 3600000) } };
    }

    this.debugLog('Final safe query:', { query: JSON.stringify(cleanedQuery) });
    return cleanedQuery;
  }
   ...
   */
  private removeProblematicOperators(
    obj: any,
    ...
    for (const key in obj) {
      if (key.startsWith('$')) {
        ...
        if (validOperators.includes(key)) {
          cleaned[key] = this.cleanQueryObject(
            obj[key],
            validOperators,
            validFields,
          );
