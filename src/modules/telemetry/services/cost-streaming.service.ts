 ...
 */
@Injectable()
export class CostStreamingService implements OnModuleDestroy {
  private readonly logger = new Logger(CostStreamingService.name);
  ...
  constructor() {
    ...
    this.logger.log(
      '🌊 Cost Streaming Service initialized with anomaly detection',
    );
  }
  ...
  registerClient(
    ...
    try {
      ...
      this.logger.log('Client registered for cost streaming', {
        clientId,
        userId,
        workspaceId,
      ...
      this.logger.error('Failed to register streaming client', {
  ...
  unregisterClient(clientId: string): void {
    ...
    if (client) {
      ...
      this.logger.debug('Client unregistered from cost streaming', {
        clientId,
        totalClients: this.clients.size,
      });
    }
  ...
  emitCostEvent(event: CostTelemetryEvent): void {
    try {
      ...
      const clientsToNotify = Array.from(this.clients.values()).filter(
        (client) => this.shouldNotifyClient(client, event),
      );
      ...
      this.logger.debug('Cost event emitted to clients', {
        eventType: event.eventType,
        clientsNotified: clientsToNotify.length,
        cost: event.data.cost,
      });
    } catch (error) {
      this.logger.error('Failed to emit cost event', {
        error: error instanceof Error ? error.message : String(error),
        eventType: event.eventType,
      });
    }
  ...
  private detectCostAnomaly(event: CostTelemetryEvent): void {
    ...
    if (currentCost > avgCost * this.SPIKE_THRESHOLD) {
      ...
      this.logger.warn('💸 Cost spike detected!', {
        userId: event.userId,
  ...
  private sendToClient(client: StreamClient, event: CostTelemetryEvent): void {
    try {
      ...
      this.logger.warn('Failed to send to client, removing', {
        clientId: client.id,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private startHeartbeat(): void {
    this.heartbeatInterval = setInterval(() => {
      ...
      if (clientsToRemove.length > 0) {
        this.logger.debug('Cleaned up inactive streaming clients', {
  ...
  getStats(): {
    ...
    this.clients.forEach((client) => {
      if (client.userId) {
        clientsByUser.set(
          client.userId,
          (clientsByUser.get(client.userId) || 0) + 1,
        );
      }
      if (client.workspaceId) {
        clientsByWorkspace.set(
          client.workspaceId,
          (clientsByWorkspace.get(client.workspaceId) || 0) + 1,
        );
      }
    ...
    const bufferedEvents = Array.from(this.eventBuffer.values()).reduce(
  ...
  onModuleDestroy(): void {
    ...
    this.logger.log('Cost Streaming Service shut down');
  }
}
