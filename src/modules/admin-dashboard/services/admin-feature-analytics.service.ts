...
export class AdminFeatureAnalyticsService {
  private readonly logger = new Logger(AdminFeatureAnalyticsService.name);
  ...
  async getFeatureUsageStats(
    ...
    try {
      ...
      const featureStats = await this.usageModel.aggregate([
        { $match: matchStage },
        {
          $group: {
            _id: {
                    ...
                    branches: [
                      {
                        case: {
                          $or: [
                            {
                              $regexMatch: {
                                input: { $toLower: '$metadata.endpoint' },
                                regex: 'chat|agent',
                              },
                            },
                            {
                          ...
                          $or: [
                            {
                              $regexMatch: {
                                input: { $toLower: '$metadata.endpoint' },
                                regex: 'experimentation|what-if',
                              },
                            },
                            {
                              $regexMatch: {
                                input: { $toLower: '$metadata.endpoint' },
                                regex: '/api/experimentation',
                              },
                            },
                          ],
                        },
      ...
      this.logger.error('Error getting feature usage stats:', {
  ...
  async getFeatureAdoptionRates(
    ...
    try {
      ...
      // Get total users
      const totalUsers = await this.usageModel
        .distinct('userId', matchStage)
        .then((ids) => ids.length);
      ...
      const getFeatureFromEndpoint = (
        ...
        if (normalized.includes('/gateway') || normalized.includes('gateway'))
          return 'Gateway';
        if (
          ...
          return 'Integration';
        if (normalized.includes('/workflow') || normalized.includes('workflow'))
          return 'Workflow';
        if (
        ...
        if (
          ...
          return 'Analytics';
        if (normalized.includes('/notebook') || normalized.includes('notebook'))
          return 'Notebook';
      ...
      const currentData = await this.usageModel
        .find({
          ...matchStage,
          createdAt: {
            $gte:
      ...
      for (const usage of currentData) {
        const feature = getFeatureFromEndpoint(
          usage.metadata?.endpoint as string,
        );
      ...
      const previousData = await this.usageModel
        .find({
          createdAt: {
      ...
      const currentFeatureUsers = Array.from(currentFeatureMap.entries()).map(
        ([feature, userIds]) => ({
          _id: feature,
      ...
      const previousFeatureUsers = Array.from(previousFeatureMap.entries()).map(
        ([feature, userIds]) => ({
          _id: feature,
          userIds: Array.from(userIds),
        }),
      ...
      const currentMap = new Map(
        currentFeatureUsers.map((f: any) => [f._id, f.userIds.length]),
      );
      const previousMap = new Map(
        previousFeatureUsers.map((f: any) => [f._id, f.userIds.length]),
      );
      ...
      this.logger.error('Error getting feature adoption rates:', {
        error: error instanceof Error ? error.message : String(error),
        component: 'AdminFeatureAnalyticsService',
  ...
  async getFeatureCostAnalysis(
    ...
    try {
      ...
      const previousStart = new Date(
        (filters.startDate?.getTime() || now.getTime()) - periodDuration,
      );
      const previousEnd =
        filters.startDate || new Date(now.getTime() - periodDuration);
      ...
      const previousCostMap = new Map(
        previousStats.map((s) => [s.feature, s.totalCost]),
      );
      ...
      const analysis: FeatureCostAnalysis[] = usageStats.map((stat) => {
        ...
        if (previousCost > 0) {
          const changePercent =
            ((stat.totalCost - previousCost) / previousCost) * 100;
        ...
        return {
          ...
          averageCostPerUser:
            stat.uniqueUsers > 0 ? stat.totalCost / stat.uniqueUsers : 0,
          trend,
        };
      });
      ...
      this.logger.error('Error getting feature cost analysis:', {
        error: error instanceof Error ? error.message : String(error),
        component: 'AdminFeatureAnalyticsService',
