...
export class UserSessionService {
  ...
  async createUserSession(
    ...
    try {
      ...
      const parsedDeviceInfo = await this.detectDeviceInfo(
      ...
      expiresAt.setDate(
        expiresAt.getDate() + UserSessionService.SESSION_EXPIRY_DAYS,
      );
      ...
      // Create session
      const userSession = await this.userSessionModel.create({
        userSessionId,
        userId,
      ...
      this.logger.log('User session created', {
    ...
    try {
      ...
      if (currentUserSessionId) {
        sessions.forEach((session) => {
          const sessionWithFlag = session as UserSession & {
            isCurrentSession?: boolean;
          };
          sessionWithFlag.isCurrentSession =
      ...
      this.logger.error('Error getting active user sessions', {
        component: 'UserSessionService',
  ...
  async revokeUserSession(
    ...
    try {
      ...
      this.logger.error('Error revoking user session', {
        component: 'UserSessionService',
        operation: 'revokeUserSession',
        userId,
        userSessionId,
   ...
   */
  async revokeUserSessionByToken(
    revokeToken: string,
  ): Promise<{ userId: string; userSessionId: string }> {
    try {
      const session = await this.userSessionModel.findOne({
        revokeToken,
        isActive: true,
      ...
      const maxAge =
        UserSessionService.REVOKE_TOKEN_EXPIRY_HOURS * 60 * 60 * 1000;
      ...
      this.logger.log('User session revoked via email token', {
        component: 'UserSessionService',
      ...
      this.logger.error('Error revoking user session by token', {
        component: 'UserSessionService',
        operation: 'revokeUserSessionByToken',
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  async revokeAllOtherUserSessions(
    ...
    try {
      ...
      this.logger.log('All other user sessions revoked', {
        component: 'UserSessionService',
      ...
      this.logger.error('Error revoking all other user sessions', {
        component: 'UserSessionService',
        operation: 'revokeAllOtherUserSessions',
        userId,
        currentUserSessionId,
  ...
  async validateSessionForRefresh(
    ...
    try {
      ...
      const session = await this.userSessionModel.findOne({ userSessionId });

      if (!session) {
        this.logger.debug('Session not found for refresh validation', {
          component: 'UserSessionService',
      ...
      if (!session.isActive) {
        this.logger.log('Session is revoked, refresh token invalid', {
          component: 'UserSessionService',
          operation: 'validateSessionForRefresh',
          userId: session.userId,
          userSessionId,
      ...
      this.logger.error('Error validating session for refresh', {
        component: 'UserSessionService',
        operation: 'validateSessionForRefresh',
        userSessionId,
        error: error instanceof Error ? error.message : String(error),
   ...
   */
  async validateSession(userSessionId: string): Promise<boolean> {
    try {
      if (!userSessionId) {
        return false;
      }
   ...
   */
  async updateUserSessionActivity(userSessionId: string): Promise<void> {
    try {
      ...
      this.logger.debug('Error updating user session activity', {
        component: 'UserSessionService',
  ...
  async cleanupExpiredUserSessions(): Promise<number> {
    try {
      ...
      this.logger.log('Expired user sessions cleaned up', {
        component: 'UserSessionService',
      ...
      this.logger.error('Error cleaning up expired user sessions', {
        component: 'UserSessionService',
        operation: 'cleanupExpiredUserSessions',
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  static getCurrentUserSessionId(request: any): string | undefined {
    // Extract from JWT token jti field (set in auth service)
    const user = request.user as { jti?: string; userSessionId?: string };
    ...
    try {
      ...
      if (isLocalhost) {
        ...
        try {
          ...
          this.logger.debug(
            'Failed to get location from external API for localhost',
            {
              component: 'UserSessionService',
      ...
      this.logger.debug('Error getting location from IP', {
        component: 'UserSessionService',
        operation: 'getLocationFromIP',
        ipAddress,
        error: error instanceof Error ? error.message : String(error),
  ...
  async isNewDevice(
    ...
    try {
      ...
      const existingSessions = await this.userSessionModel
        .find({
          userId,
      ...
      this.logger.error('Error checking if new device', {
        component: 'UserSessionService',
  ...
  async enforceConcurrentSessionLimit(userId: string): Promise<void> {
    try {
      ...
      if (activeSessions.length >= maxSessions) {
        ...
        this.logger.log('Concurrent session limit enforced', {
          component: 'UserSessionService',
      ...
      this.logger.error('Error enforcing concurrent session limit', {
        component: 'UserSessionService',
        operation: 'enforceConcurrentSessionLimit',
        userId,
        error: error instanceof Error ? error.message : String(error),
