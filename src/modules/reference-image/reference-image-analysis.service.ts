...
export class ReferenceImageAnalysisService {
  private readonly logger = new Logger(ReferenceImageAnalysisService.name);
  private readonly MODEL_ID = 'us.anthropic.claude-3-5-sonnet-20241022-v2:0';
  private readonly extractionEmitter = new EventEmitter();
   ...
   */
  getExtractionEmitter(): EventEmitter {
...
Important: Ensure all fields are present and properly formatted. Return ONLY the JSON, no markdown formatting or additional text.`;
  ...
  private validateExtraction(
    ...
    if (!response.structuredData) {
      ...
      if (!response.structuredData.colors)
        missing.push('structuredData.colors');
      if (!response.structuredData.layout)
        missing.push('structuredData.layout');
      if (!response.structuredData.objects)
        missing.push('structuredData.objects');
      if (!response.structuredData.text) missing.push('structuredData.text');
      if (!response.structuredData.lighting)
        missing.push('structuredData.lighting');
      if (!response.structuredData.quality)
        missing.push('structuredData.quality');
    }

    if (
      !response.criteriaAnalysis ||
      !Array.isArray(response.criteriaAnalysis)
    ) {
      missing.push('criteriaAnalysis');
    } else if (response.criteriaAnalysis.length !== expectedCriteriaCount) {
      missing.push(
        `criteriaAnalysis (expected ${expectedCriteriaCount}, got ${response.criteriaAnalysis.length})`,
      );
    } else {
      // Validate each criterion analysis
      response.criteriaAnalysis.forEach((ca: any, index: number) => {
        if (!ca.criterionId)
          missing.push(`criteriaAnalysis[${index}].criterionId`);
        if (!ca.criterionText)
          missing.push(`criteriaAnalysis[${index}].criterionText`);
        if (!ca.referenceState)
          missing.push(`criteriaAnalysis[${index}].referenceState`);
        if (!ca.comparisonInstructions)
          missing.push(`criteriaAnalysis[${index}].comparisonInstructions`);
        if (typeof ca.confidence !== 'number')
          missing.push(`criteriaAnalysis[${index}].confidence`);
      });
    }
...
Return ONLY valid JSON with the missing fields populated. No markdown or additional text.`;
   ...
   */
  private mergeResponses(initial: any, followUp: any): any {
    return {
      ...initial,
      ...followUp,
      structuredData: {
      ...
      criteriaAnalysis:
        followUp.criteriaAnalysis || initial.criteriaAnalysis || [],
    };
  }
   ...
   */
  async extractReferenceFeatures(
    imageUrl: string,
    criteria: CriterionInput[],
    industry: string,
    templateId: string,
    ...
    try {
      ...
      this.logger.log('Starting reference image feature extraction', {
      ...
      // Step 2: Convert S3 URL to key and download as base64 for Bedrock
      const s3Key = ReferenceImageS3Service.s3UrlToKey(imageUrl);
      const base64Image =
        await this.referenceImageS3Service.downloadImageAsBase64(s3Key);

      // Step 3: Make initial LLM call with image
      ...
      // Step 4: Validate response completeness
      const validation = this.validateExtraction(
        initialResponse,
        criteria.length,
      );

      // Step 5: If validation fails, make follow-up call
      if (!validation.valid) {
        this.logger.warn(
          'Initial extraction incomplete, making follow-up call',
          {
            component: 'ReferenceImageAnalysisService',
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            this.logger.warn(
              'Could not parse follow-up response, using initial data',
            );
            followUpResponse = {};
          }
        }
        ...
        // Merge responses
        initialResponse = this.mergeResponses(
          initialResponse,
          followUpResponse,
        );
      }
      ...
      // Step 8: Update template with extracted features
      ...
      await this.activityModel.create({
        ...
        description:
          'Successfully extracted features from reference image for template',
        metadata: {
      ...
      this.logger.log('Reference image feature extraction completed', {
        component: 'ReferenceImageAnalysisService',
        operation: 'extractReferenceFeatures',
        templateId,
        totalTokens,
      ...
      this.logger.error('Error extracting reference image features', {
        component: 'ReferenceImageAnalysisService',
        operation: 'extractReferenceFeatures',
  ...
  async updateExtractionStatus(
    ...
    await this.promptTemplateModel.findByIdAndUpdate(templateId, update);

    this.logger.log('Reference image extraction status updated', {
      component: 'ReferenceImageAnalysisService',
    ...
    this.logger.log('Emitted extraction status update event', {
      component: 'ReferenceImageAnalysisService',
      operation: 'updateExtractionStatus',
      templateId,
      status,
  ...
  async retryExtraction(
    ...
    const s3Key = ReferenceImageS3Service.s3UrlToKey(
      template.referenceImage.s3Url,
    );
    const imageUrl = await this.referenceImageS3Service.generatePresignedUrl(
      s3Key,
      3600,
    );
   ...
   */
  async reExtractForCriteriaChange(
    templateId: string,
