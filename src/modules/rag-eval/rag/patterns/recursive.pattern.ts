 ...
 * Decomposes complex questions into sub-questions and synthesizes answers (Recursive RAG)
 */
...
export class RecursiveRAGPattern extends BaseRAGPattern {
  ...
  protected async executePattern(
    ...
    try {
      ...
      this.logger.log('Recursive RAG: Query decomposed', {
        component: 'RecursiveRAGPattern',
        subQuestionsCount: state.subQuestions.length,
      });

      // Step 2: Process each sub-question
      const subQuestionPromises = state.subQuestions.map((sq) =>
        this.processSubQuestion(sq, state, context),
      );

      const subResults = await Promise.all(subQuestionPromises);

      // Store results
      for (const result of subResults) {
        state.results.set(result.questionId, result);
        totalRetrievalDuration += (result as any).retrievalDuration || 0;
        totalGenerationDuration += (result as any).generationDuration || 0;
      }
      ...
      totalGenerationDuration += Date.now() - synthesisStart;

      // Collect all documents from sub-questions
      const allDocuments = subResults.flatMap((result) => result.documents);
      const uniqueDocuments = this.deduplicateDocuments(allDocuments);
      const sources = this.extractSources(uniqueDocuments);

      this.logger.log('Recursive RAG: Synthesis completed', {
        component: 'RecursiveRAGPattern',
        subQuestionsProcessed: state.subQuestions.length,
      ...
      this.logger.error('Recursive RAG pattern failed', {
        component: 'RecursiveRAGPattern',
...
...`;
  ...
  private async processSubQuestion(
    ...
    this.logger.log('Recursive RAG: Processing sub-question', {
      component: 'RecursiveRAGPattern',
