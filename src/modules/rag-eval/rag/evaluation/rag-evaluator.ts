...
export class RAGEvaluator {
  ...
  private logger = new Logger(RAGEvaluator.name);
...
Respond with only a number between 0.0 and 1.0.`;
      ...
      this.logger.warn('Context relevance evaluation failed', {
...
Respond with only a number between 0.0 and 1.0.`;
      ...
      this.logger.warn('Answer faithfulness evaluation failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 0.5;
    }
...
Respond with only a number between 0.0 and 1.0.`;
      ...
      this.logger.warn('Answer relevance evaluation failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 0.5;
    }
...
Answer with only "yes" or "no".`;
      ...
      this.logger.warn('Retrieval precision evaluation failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 0.5;
    }
...
Respond with only a number between 0.0 and 1.0.`;
      ...
      this.logger.warn('Retrieval recall evaluation failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 0.5;
    }
  ...
  private calculateOverallScore(metrics: EvaluationMetrics): number {
    ...
    return (
      (metrics.contextRelevance ?? 0) * weights.contextRelevance +
      (metrics.answerFaithfulness ?? 0) * weights.answerFaithfulness +
      (metrics.answerRelevance ?? 0) * weights.answerRelevance +
      (metrics.retrievalPrecision ?? 0) * weights.retrievalPrecision +
      (metrics.retrievalRecall ?? 0) * weights.retrievalRecall
    );
  }
  ...
  calculateAggregateStats(metrics: EvaluationMetrics[]): {
    ...
    keys.forEach((key) => {
      ...
      const variance =
        values.reduce(
          (sum, val) => (sum ?? 0) + Math.pow((val ?? 0) - (mean[key] ?? 0), 2),
          0,
        ) / values.length;
      std[key] = Math.sqrt(variance ?? 0);
    });
