...
export interface ReadConfig extends ModuleConfig {
  ...
  extractionStrategy?: 'full' | 'summary' | 'key-points' | 'hybrid';
}
...
export interface FusionConfig extends ModuleConfig {
  ...
  /**
   * Optional: when strategy is 'llm-based', call this to rank/select documents via an LLM.
   * Receives query and document snippets; returns ordered list of document keys (same order as snippets).
   * If not provided, query-aware relevance scoring is used instead.
   */
  llmRankDocuments?: (
    query: string,
    snippets: Array<{ key: string; content: string; index: number }>,
  ) => Promise<string[]>;
}
...
// ============================================================================
// RAG Evaluation Types (from metrics.ts)
// ============================================================================
