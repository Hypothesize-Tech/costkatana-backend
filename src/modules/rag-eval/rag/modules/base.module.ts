...
export abstract class BaseRAGModule implements IRAGModule {
  ...
  protected logger = new Logger(BaseRAGModule.name);
  ...
  async execute(input: RAGModuleInput): Promise<RAGModuleOutput> {
    ...
    try {
      ...
      this.logger.log(`RAG Module [${this.name}] executing`, {
        component: 'RAGModule',
        module: this.name,
        type: this.type,
      ...
      this.logger.log(`RAG Module [${this.name}] completed`, {
        component: 'RAGModule',
        module: this.name,
        type: this.type,
        duration,
      ...
      this.logger.error(`RAG Module [${this.name}] failed`, {
        component: 'RAGModule',
        module: this.name,
        type: this.type,
  ...
  private createSkippedOutput(
    ...
    this.logger.log(`RAG Module [${this.name}] skipped (disabled)`, {
      component: 'RAGModule',
      module: this.name,
      type: this.type,
    });
  ...
  updateConfig(newConfig: Partial<ModuleConfig>): void {
    ...
    this.logger.log(`RAG Module [${this.name}] configuration updated`, {
      component: 'RAGModule',
      module: this.name,
      type: this.type,
      config: this.config,
