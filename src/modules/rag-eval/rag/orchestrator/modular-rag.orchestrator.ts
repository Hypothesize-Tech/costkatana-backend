...
export class ModularRAGOrchestrator {
  private logger = new Logger(ModularRAGOrchestrator.name);
  ...
  async execute(input: OrchestratorInput): Promise<RAGResult> {
    ...
    try {
      this.logger.log('RAG Orchestrator: Execution started', {
      ...
      this.logger.log('RAG Orchestrator: Pattern selected', {
        component: 'ModularRAGOrchestrator',
        selectedPattern,
      });
      ...
      this.logger.log('RAG Orchestrator: Execution completed', {
        component: 'ModularRAGOrchestrator',
      ...
      this.logger.error('RAG Orchestrator: Execution failed', {
        component: 'ModularRAGOrchestrator',
      ...
      if (this.config.fallbackPattern) {
        try {
          this.logger.log('RAG Orchestrator: Attempting fallback pattern', {
            component: 'ModularRAGOrchestrator',
          ...
          this.logger.error('RAG Orchestrator: Fallback also failed', {
            component: 'ModularRAGOrchestrator',
            error:
  ...
  private async runEvaluationIfEnabled(
    ...
    const { ragEvaluator } = await import('../evaluation/rag-evaluator');
    ...
    try {
      ...
      if (input.config.evaluation.logResults) {
        this.logger.log('RAG evaluation metrics', {
          component: 'ModularRAGOrchestrator',
          evaluation,
        });
      }
      ...
      this.logger.warn('RAG evaluation failed, continuing without metrics', {
        component: 'ModularRAGOrchestrator',
        error: error instanceof Error ? error.message : String(error),
      });
      return result;
  ...
  private async selectPattern(
    ...
    try {
      const cached =
        await RagServiceLocator.getCacheService().get<string>(cacheKey);
      if (cached && typeof cached === 'string') {
        this.logger.log('RAG Orchestrator: Using cached pattern selection', {
          component: 'ModularRAGOrchestrator',
          pattern: cached,
        });
    ...
    this.logger.log('RAG Orchestrator: Query analysis completed', {
      component: 'ModularRAGOrchestrator',
    ...
    // Cache the selection
    try {
      await RagServiceLocator.getCacheService().set(
        cacheKey,
...
Reasoning: [brief explanation]`;
      ...
      this.logger.warn('LLM query analysis failed, using heuristics', {
        component: 'ModularRAGOrchestrator',
        error: error instanceof Error ? error.message : String(error),
      });
      return this.heuristicAnalysis(query);
  ...
  private async getPattern(
    ...
    switch (patternType) {
      ...
      default:
        this.logger.warn('Unknown pattern type, using adaptive', {
          component: 'ModularRAGOrchestrator',
  ...
  updateConfig(newConfig: Partial<OrchestratorConfig>): void {
    ...
    this.logger.log('RAG Orchestrator configuration updated', {
      component: 'ModularRAGOrchestrator',
      config: this.config,
    });
  }
