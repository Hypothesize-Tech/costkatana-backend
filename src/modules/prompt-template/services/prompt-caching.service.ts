...
export class PromptCachingService {
  ...
  private readonly providerSupport: Map<string, ProviderCacheSupport> =
    new Map();
  private readonly activeCacheConfigs: Map<string, PromptCachingConfig> =
    new Map();
  private readonly cache: LRUCache<string, CachedPrompt>;

  // Cache hit/miss tracking for health monitoring
  private cacheHits: number = 0;
  private cacheMisses: number = 0;
  private readonly lastHitRateReset: number = Date.now();
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.logger.log('🧠 Prompt Caching Service initialized');
  }
  ...
  private initializeProviderSupport(): void {
    ...
    this.providerSupport.set('anthropic', {
      ...
      cachePricing: {
        writePrice: 0.3, // $0.30 per 1M tokens (10% of base)
        readPrice: 0.3, // $0.30 per 1M tokens (90% discount)
      },
    });
    ...
    this.providerSupport.set('openai', {
      ...
      cachePricing: {
        writePrice: 1.5, // $1.50 per 1M tokens (50% discount)
    ...
    this.providerSupport.set('google', {
      ...
      cachePricing: {
        writePrice: 1.0, // $1.00 per 1M tokens per hour (storage)
   ...
   */
  getProviderSupport(provider: string): ProviderCacheSupport | undefined {
   ...
   */
  isProviderSupported(provider: string, model?: string): boolean {
    ...
    if (model) {
      ...
      return support.supportedModels.some((supportedModel) =>
        normalizedModel.includes(
          supportedModel.toLowerCase().replace(/[-_]/g, ''),
        ),
      );
    }
   ...
   */
  getDefaultConfig(provider: string): PromptCachingConfig {
    const support = this.getProviderSupport(provider);
    if (!support) {
   ...
   */
  setConfig(key: string, config: PromptCachingConfig): void {
    this.activeCacheConfigs.set(key, config);
    this.cache.set(`config:${key}`, config as any);
    this.logger.log(`Prompt caching configuration updated for ${key}`);
  }

  /**
   * Get configuration for user/project
   */
  getConfig(key: string): PromptCachingConfig | undefined {
    // Try cache first
    let config = this.cache.get(`config:${key}`) as
      | PromptCachingConfig
      | undefined;
    if (!config) {
  ...
  analyzePrompt(
    ...
    try {
      ...
      this.logger.debug(
        `Prompt analysis completed for ${provider}/${model}: ${tokenCount} tokens, cacheable: ${result.isCacheable}`,
      );

      return result;
    } catch (error) {
      this.logger.error('Error analyzing prompt for caching', {
  ...
  async processPrompt(
    ...
    try {
      ...
      if (
        support.cacheType === 'explicit' &&
        request.config?.breakpointsEnabled
      ) {
        modifiedMessages = this.applyExplicitCaching(
          request.messages,
      ...
      const cacheHeaders = {
        ...
        'x-cache-min-tokens': (request.config?.minTokens ?? 0).toString(),
      };
      ...
      this.logger.log(
        `Prompt processed for caching: ${request.provider}/${request.model}, ${analysis.tokenCount} tokens, ${cacheType} mode`,
      );
      ...
      return {
        ...
        metrics: {
          cacheCreationTokens:
            analysis.breakpoints.length > 0 ? analysis.tokenCount : 0,
      ...
      this.logger.error('Error processing prompt for caching', {
        error: error instanceof Error ? error.message : String(error),
  ...
  private estimatePromptTokens(messages: any[], provider: string): number {
    try {
      ...
      this.logger.warn('Error estimating prompt tokens', {
        error: error instanceof Error ? error.message : String(error),
      });
      return 0;
    }
  ...
  private analyzePromptStructure(
    ...
    if (support.cacheType === 'explicit') {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (
          messageTokens >= support.minTokens &&
    ...
    if (support.cacheType === 'automatic') {
      ...
      if (hasStaticContent && messages.length > 3) {
        ...
        const firstDynamicIndex = messages.findIndex(
          (msg) => msg.role !== 'system' && !this.isLikelyStaticContent(msg),
        );
  ...
  private calculateEstimatedSavings(
    ...
    const regularCost =
      (tokenCount * support.cachePricing.writePrice) / 1_000_000;
    const cachedCost =
      (cacheableTokens * support.cachePricing.readPrice) / 1_000_000;
  ...
  private detectContentType(
    message: any,
  ): 'system' | 'tools' | 'documents' | 'history' {
