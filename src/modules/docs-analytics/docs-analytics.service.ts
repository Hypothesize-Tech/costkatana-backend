...
import {
  DocsPageRating,
  DocsPageRatingDocument,
} from '../../schemas/docs-analytics/docs-page-rating.schema';
import {
  DocsPageFeedback,
  DocsPageFeedbackDocument,
} from '../../schemas/docs-analytics/docs-page-feedback.schema';
import {
  DocsPageView,
  DocsPageViewDocument,
} from '../../schemas/docs-analytics/docs-page-view.schema';
import {
  DocsUserPreference,
  DocsUserPreferenceDocument,
} from '../../schemas/docs-analytics/docs-user-preference.schema';
...
export class DocsAnalyticsService {
  constructor(
    @InjectModel(DocsPageRating.name)
    private readonly docsPageRatingModel: Model<DocsPageRatingDocument>,
    @InjectModel(DocsPageFeedback.name)
    private readonly docsPageFeedbackModel: Model<DocsPageFeedbackDocument>,
    @InjectModel(DocsPageView.name)
    private readonly docsPageViewModel: Model<DocsPageViewDocument>,
    @InjectModel(DocsUserPreference.name)
    private readonly docsUserPreferenceModel: Model<DocsUserPreferenceDocument>,
  ...
  async submitRating(data: {
    ...
    ip?: string;
    userAgent?: string;
  }): Promise<DocsPageRating> {
    const ipHash = data.ip ? this.hashIP(data.ip) : undefined;

    // Upsert to allow changing rating
    const rating = await this.docsPageRatingModel.findOneAndUpdate(
    ...
    this.logger.log('Page rating submitted', {
      pageId: data.pageId,
      rating: data.rating,
      component: 'DocsAnalyticsService',
  ...
  async getRatingStats(pageId: string): Promise<RatingStats> {
    const ratings = await this.docsPageRatingModel.find({ pageId });
  ...
  async submitFeedback(data: {
    ...
    ip?: string;
    userAgent?: string;
  }): Promise<DocsPageFeedback> {
    const ipHash = data.ip ? this.hashIP(data.ip) : undefined;

    const feedback = await this.docsPageFeedbackModel.create({
      pageId: data.pageId,
      pagePath: data.pagePath,
    ...
    this.logger.log('Page feedback submitted', {
      pageId: data.pageId,
  ...
  async getFeedbackForPage(pageId: string): Promise<DocsPageFeedback[]> {
    return this.docsPageFeedbackModel
      .find({ pageId })
      .sort({ createdAt: -1 })
      .limit(50);
  }
  ...
  }): Promise<DocsPageView> {
    const ipHash = data.ip ? this.hashIP(data.ip) : undefined;

    // Create or update view for this session
    const view = await this.docsPageViewModel.findOneAndUpdate(
      { pageId: data.pageId, sessionId: data.sessionId },
      {
        $setOnInsert: {
          pageId: data.pageId,
  ...
  async updatePageViewMetrics(data: {
    ...
    return this.docsPageViewModel.findOneAndUpdate(
      { pageId: data.pageId, sessionId: data.sessionId },
      updateData,
      { new: true },
    );
  ...
  async getPageViewStats(pageId: string): Promise<PageViewStats> {
    const views = await this.docsPageViewModel.find({ pageId });

    const uniqueSessions = new Set(views.map((v) => v.sessionId)).size;
    const timesOnPage = views
  ...
  private async updateUserPreference(
    ...
    await this.docsUserPreferenceModel.findOneAndUpdate(
      { sessionId },
      {
  ...
  async getUserPreference(sessionId: string) {
    return this.docsUserPreferenceModel.findOne({ sessionId });
  }
  ...
  async getRecommendations(
    ...
    const userPref = await this.docsUserPreferenceModel.findOne({ sessionId });
  ...
  private async getPopularPages(): Promise<ContentRecommendation[]> {
    const popularPages = await this.docsPageViewModel.aggregate([
      {
        $group: {
          _id: '$pageId',
  ...
  private async generateAIRecommendations(
    ...
    try {
      const visitedPaths = userPref.visitedPages
  ...
  "recommendations": [
    {
      ...
      this.logger.error('AI recommendation generation failed', {
        ...
        component: 'DocsAnalyticsService',
        operation: 'generateAIRecommendations',
      });
    }
      ...
      const availablePages = [
        {
          ...
          title: 'Introduction',
          content:
            'Learn what Cost Katana is and how it can help optimize your AI costs',
        },
        {
          path: '/getting-started/quick-start',
          title: 'Quick Start',
          content: 'Get up and running with Cost Katana in minutes',
        },
        {
          path: '/getting-started/installation',
          title: 'Installation',
          content: 'Detailed installation instructions for all platforms',
        },
        {
          path: '/features/dashboard',
          title: 'Dashboard',
          content: 'Real-time monitoring and insights dashboard',
        },
        {
          path: '/features/usage-tracking',
          title: 'Usage Tracking',
          content: 'Track your AI usage across all providers',
        },
        {
          path: '/features/analytics',
          title: 'Cost Analytics',
          content: 'Advanced cost analysis and reporting',
        },
        {
          path: '/features/optimization',
          title: 'AI Optimization',
          content: 'Intelligent cost reduction strategies',
        },
        {
          path: '/features/predictive-intelligence',
          title: 'Predictive Intelligence',
          content: 'AI-powered cost forecasting',
        },
        {
          path: '/features/projects',
          title: 'Projects',
          content: 'Organize and track projects',
        },
        {
          path: '/features/templates',
          title: 'Prompt Templates',
          content: 'Reusable optimized prompts',
        },
        {
          path: '/features/workflows',
          title: 'Workflows',
          content: 'Multi-step operation monitoring',
        },
        {
          path: '/features/gateway',
          title: 'Gateway & Proxy',
          content: 'Unified API gateway for all providers',
        },
        {
          path: '/features/key-vault',
          title: 'Key Vault',
          content: 'Secure API key management',
        },
        {
          path: '/features/alerts',
          title: 'Alerts',
          content: 'Proactive cost monitoring and alerts',
        },
        {
          path: '/api',
          title: 'API Overview',
        ...
        },
        {
          ...
          content: 'JWT and API key authentication',
        },
        {
          path: '/api/usage',
          title: 'Usage API',
          content: 'Track AI usage programmatically',
        },
        {
          path: '/api/analytics',
          title: 'Analytics API',
          content: 'Retrieve analytics data via API',
        },
        {
          path: '/integrations/nodejs',
          title: 'Node.js SDK',
          content: 'Integrate Cost Katana with Node.js applications',
        },
        {
          path: '/integrations/python',
          title: 'Python SDK',
          content: 'Integrate Cost Katana with Python applications',
        },
        {
          path: '/integrations/cli',
          title: 'CLI Tool',
          content: 'Command-line interface for AI cost optimization',
        },
      ];
...
${availablePages.map((page) => `- ${page.path}: ${page.title} - ${page.content}`).join('\n')}
...
RESPONSE FORMAT - ONLY JSON:
{
  ...
  async getOverallStats(): Promise<{
    ...
    const [totalViews, totalRatings, totalFeedback, topPages] =
      await Promise.all([
        this.docsPageViewModel.countDocuments(),
        this.docsPageRatingModel.countDocuments(),
        this.docsPageFeedbackModel.countDocuments(),
        this.docsPageViewModel.aggregate([
    ...
    const allRatings = await this.docsPageRatingModel.find();
    const upvotes = allRatings.filter((r) => r.rating === 'up').length;
    const avgHelpfulness =
      allRatings.length > 0 ? (upvotes / allRatings.length) * 100 : 0;
