...
export class AgentBehaviorAnalyticsService {
  private readonly logger = new Logger(AgentBehaviorAnalyticsService.name);
  private static readonly ANOMALY_THRESHOLDS = {
    highCost: 0.1, // $0.10 per session
  ...
  constructor(
    @InjectModel(AgentDecisionLog.name)
    private agentDecisionLogModel: Model<AgentDecisionLogDocument>,
  ) {}

  /**
   * Log an agent decision session
   */
  async logAgentSession(params: {
    agentId: string;
    ...
    outcome: any;
    environment?: string;
  }): Promise<AgentDecisionLogDocument> {
    try {
      ...
      const log = new this.agentDecisionLogModel({
      ...
      if (criticalAnomalies.length > 0) {
        this.logger.warn('🚨 Critical agent anomalies detected', {
          agentSessionId: params.agentSessionId,
          anomalies: criticalAnomalies.map((a) => a.anomalyType),
        });
      }

      this.logger.log('✅ Logged agent session', {
        agentSessionId: params.agentSessionId,
      ...
      this.logger.error(`❌ Failed to log agent session`, {
  ...
  private calculatePerformance(
    ...
    for (let i = 1; i < actions.length; i++) {
      if (
        actions[i].actionName === actions[i - 1].actionName &&
        !actions[i - 1].success &&
        actions[i].timestamp.getTime() - actions[i - 1].timestamp.getTime() <
          5000
      ) {
        actionsRetried++;
      }
    ...
    const avgTokensPerAction =
      actions.length > 0 ? totalTokens / actions.length : 0;
   ...
   */
  private calculateCostBreakdown(actions: any[]): any {
    let modelCosts = 0;
    let toolCosts = 0;
    let apiCosts = 0;
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      if (
        i > 0 &&
        action.actionName === actions[i - 1].actionName &&
        !actions[i - 1].success
      ) {
    ...
    const abandonedPathsCost = actions
      .filter((a) => !a.success)
  ...
  private detectAnomalies(
    ...
    if (loopDetection) {
      anomalies.push({
        ...
        suggestedFix:
          'Review agent logic and add loop detection/breaking mechanisms',
        autoFixable: false,
      });
    }
    ...
    if (
      performance.actionsRetried >
      AgentBehaviorAnalyticsService.ANOMALY_THRESHOLDS.excessiveRetries
    ) {
      anomalies.push({
    ...
    if (
      ...
      AgentBehaviorAnalyticsService.ANOMALY_THRESHOLDS.highCost
    ) {
      anomalies.push({
    ...
    const successRate =
      performance.actionsAttempted > 0
        ? performance.actionsSuccessful / performance.actionsAttempted
        : 0;

    if (
      successRate <
        AgentBehaviorAnalyticsService.ANOMALY_THRESHOLDS.lowSuccessRate &&
      performance.actionsAttempted > 5
    ) {
      anomalies.push({
    ...
    const redundantRatio =
      costBreakdown.redundantCallsCost /
      Math.max(0.001, costBreakdown.totalCost);
    if (
      redundantRatio >
      AgentBehaviorAnalyticsService.ANOMALY_THRESHOLDS.redundantActions
    ) {
      anomalies.push({
    ...
    if (
      ...
      AgentBehaviorAnalyticsService.ANOMALY_THRESHOLDS.longDuration
    ) {
      anomalies.push({
        anomalyType: 'timeout',
   ...
   */
  private detectLoops(actions: any[]): {
    description: string;
    indices: number[];
    costImpact: number;
    timeImpact: number;
    ...
    for (let patternLen = 3; patternLen <= 5; patternLen++) {
      for (let i = 0; i <= actions.length - patternLen * 3; i++) {
        ...
        while (j <= actions.length - patternLen) {
          const nextPattern = actions
            .slice(j, j + patternLen)
            .map((a) => a.actionName)
            .join('|');
          if (nextPattern === pattern) {
            repeatCount++;
            indices.push(
              ...Array.from({ length: patternLen }, (_, k) => j + k),
            );
            j += patternLen;
          } else {
            break;
        ...
        if (repeatCount >= 2) {
          ...
          const costImpact = loopActions.reduce(
            (sum, a) => sum + (a.cost || 0),
            0,
          );
          const timeImpact = loopActions.reduce(
            (sum, a) => sum + a.latencyMs,
            0,
          );
  ...
  private calculateLearningSignals(
    ...
    const successRate =
      performance.actionsAttempted > 0
        ? performance.actionsSuccessful / performance.actionsAttempted
        : 0;
    const retryPenalty = Math.max(
      0,
      1 - performance.actionsRetried / performance.actionsAttempted,
    );
    const efficiencyScore = successRate * retryPenalty;

    // Cost efficiency: low cost relative to value delivered
    const valueDelivered = performance.goalAchieved
      ? 1.0
      : outcome.completeness || 0;
    const costEfficiencyScore = Math.max(
      0,
      Math.min(
        1,
        valueDelivered / Math.max(0.001, costBreakdown.totalCost * 10),
      ),
    );
    ...
    if (costBreakdown.redundantCallsCost > 0.01)
      improvementAreas.push('deduplication');
   ...
   */
  async getAgentEfficiencyMetrics(params: {
    agentId?: string;
    agentType?: string;
    userId?: string;
    startDate: Date;
    ...
    try {
      ...
      const logs = await this.agentDecisionLogModel.find(query).lean();

      // Group by agent
      const agentGroups = new Map<string, AgentDecisionLogDocument[]>();
      for (const log of logs) {
        const key = `${log.agentId}:${log.configuration.agentType}`;
        if (!agentGroups.has(key)) {
          agentGroups.set(key, []);
        }
        agentGroups.get(key)!.push(log as unknown as AgentDecisionLogDocument);
      }
      ...
      for (const [key, agentLogs] of agentGroups) {
        ...
        const totalDuration = agentLogs.reduce(
          (sum, l) => sum + l.performance.totalDurationMs,
          0,
        );
        const totalActions = agentLogs.reduce(
          (sum, l) => sum + l.performance.actionsAttempted,
          0,
        );
        const successfulActions = agentLogs.reduce(
          (sum, l) => sum + l.performance.actionsSuccessful,
          0,
        );
        const totalCost = agentLogs.reduce(
          (sum, l) => sum + l.costBreakdown.totalCost,
          0,
        );
        const goalsAchieved = agentLogs.filter(
          (l) => l.performance.goalAchieved,
        ).length;

        const sessionsWithFeedback = agentLogs.filter(
          (l) => l.userFeedback?.rating,
        );
        const avgUserSatisfaction =
          sessionsWithFeedback.length > 0
            ? sessionsWithFeedback.reduce(
                (sum, l) => sum + l.userFeedback!.rating!,
                0,
        ...
        const avgEfficiency =
          agentLogs.reduce(
            (sum, l) => sum + l.learningSignals.efficiencyScore,
            0,
          ) / agentLogs.length;
        const avgCostEfficiency =
          agentLogs.reduce(
            (sum, l) => sum + l.learningSignals.costEfficiencyScore,
            0,
          ) / agentLogs.length;
        const avgQuality =
          agentLogs.reduce(
            (sum, l) => sum + l.learningSignals.qualityScore,
            0,
          ) / agentLogs.length;
        const avgReliability =
          agentLogs.reduce(
            (sum, l) => sum + l.learningSignals.reliabilityScore,
            0,
          ) / agentLogs.length;

        const estimatedSavings = agentLogs.reduce(
          (sum, l) => sum + (l.learningSignals.estimatedSavingsPotential || 0),
          0,
        );
        ...
        const recentAvgCost =
          recentLogs.reduce((sum, l) => sum + l.costBreakdown.totalCost, 0) /
          recentLogs.length;
        const olderAvgCost =
          olderLogs.reduce((sum, l) => sum + l.costBreakdown.totalCost, 0) /
          olderLogs.length;

        let costTrend: 'increasing' | 'stable' | 'decreasing' = 'stable';
        if (Math.abs(recentAvgCost - olderAvgCost) > olderAvgCost * 0.1) {
          costTrend =
            recentAvgCost > olderAvgCost ? 'increasing' : 'decreasing';
        }
        ...
        for (const log of agentLogs) {
          for (const area of log.learningSignals.improvementAreas) {
            improvementAreasMap.set(
              area,
              (improvementAreasMap.get(area) || 0) + 1,
            );
          }
        }
      ...
      this.logger.error('Failed to get agent efficiency metrics', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
    ...
    try {
      ...
      for (const [patternType, group] of anomalyGroups) {
        ...
        patterns.push({
          ...
          description: this.getPatternDescription(
            patternType,
  ...
  private getPatternDescription(
    ...
    return (
      descriptions[patternType] ||
      `Pattern "${patternType}" occurred ${frequency} times`
    );
  }
  ...
  private getPatternRecommendation(
    ...
    const recommendations: Record<string, string> = {
      ...
      low_success_rate:
        'Improve action validation and add better error recovery',
      ...
      resource_waste:
        'Review agent configuration and reduce unnecessary operations',
    };
  ...
  async getTopInefficientAgents(
    ...
    try {
      ...
      this.logger.error('Failed to get top inefficient agents', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
