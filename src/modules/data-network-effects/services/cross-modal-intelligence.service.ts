import { Injectable, Logger } from '@nestjs/common';
import { InjectModel } from '@nestjs/mongoose';
import { Model } from 'mongoose';
import { ChatBedrockConverse } from '@langchain/aws';
import { HumanMessage } from '@langchain/core/messages';
import { CacheService } from '../../../common/cache/cache.service';
import {
  UserMemory,
  UserMemoryDocument,
} from '../../../schemas/agent/memory.schema';
import {
  ConversationMemory,
  ConversationMemoryDocument,
} from '../../../schemas/agent/memory.schema';
import {
  ChatMessage,
  ChatMessageDocument,
} from '../../../schemas/chat/chat-message.schema';
import { VectorMemoryService } from '../../memory/services/vector-memory.service';

export interface UserBehaviorPattern {
  userId: string;
  patterns: {
    preferredTopics: string[];
    queryComplexity: 'simple' | 'moderate' | 'complex';
    responsePreference: 'concise' | 'detailed' | 'comprehensive';
    technicalLevel: 'beginner' | 'intermediate' | 'expert';
    costSensitivity: 'low' | 'medium' | 'high';
    peakUsageHours: number[];
    commonQuestionTypes: string[];
  };
  confidence: number;
  lastAnalyzed: Date;
  sampleSize: number;
}

export interface PredictiveInsight {
  type:
    | 'cost_optimization'
    | 'usage_pattern'
    | 'behavior_prediction'
    | 'recommendation';
  insight: string;
  confidence: number;
  actionable: boolean;
  estimatedImpact: 'low' | 'medium' | 'high';
  category: string;
  metadata: Record<string, unknown>;
}

export interface CrossDomainConnection {
  sourceType: 'memory' | 'conversation' | 'message';
  targetType: 'memory' | 'conversation' | 'message';
  connectionType: 'causal' | 'temporal' | 'semantic' | 'behavioral';
  strength: number;
  description: string;
  examples: string[];
}

/**
 * Cross-Modal Intelligence Engine
 * Generates insights by connecting patterns across UserMemory, ConversationMemory, and Messages
 * Runs in background to provide contextual intelligence for enhanced user experiences
 */
@Injectable()
export class CrossModalIntelligenceService {
  private readonly logger = new Logger(CrossModalIntelligenceService.name);
  private readonly intelligenceAgent: ChatBedrockConverse;
  private readonly ANALYSIS_CACHE_TTL = 6 * 60 * 60; // 6 hours in seconds
  private readonly MIN_SAMPLE_SIZE = 5;

  constructor(
    private readonly cache: CacheService,
    private readonly vectorMemoryService: VectorMemoryService,
    @InjectModel(UserMemory.name)
    private readonly userMemoryModel: Model<UserMemoryDocument>,
    @InjectModel(ConversationMemory.name)
    private readonly conversationMemoryModel: Model<ConversationMemoryDocument>,
    @InjectModel(ChatMessage.name)
    private readonly chatMessageModel: Model<ChatMessageDocument>,
  ) {
    this.intelligenceAgent = new ChatBedrockConverse({
      model: 'amazon.nova-pro-v1:0',
      region: process.env.AWS_REGION ?? 'us-east-1',
      temperature: 0.2,
      maxTokens: 3000,
    });
  }

  async analyzeUserBehaviorPatterns(
    userId: string,
    forceRefresh = false,
  ): Promise<UserBehaviorPattern> {
    try {
      const cacheKey = `behavior_pattern:${userId}`;

      if (!forceRefresh) {
        const cached = await this.cache.get<UserBehaviorPattern>(cacheKey);
        if (cached) {
          return cached;
        }
      }

      this.logger.log('Analyzing user behavior patterns', { userId });

      const [memories, conversations, messages] = await Promise.all([
        this.userMemoryModel
          .find({ userId, isActive: true })
          .limit(50)
          .lean()
          .exec(),
        this.conversationMemoryModel
          .find({ userId, isArchived: false })
          .sort({ createdAt: -1 })
          .limit(30)
          .lean()
          .exec(),
        this.chatMessageModel
          .find({ userId, isVectorized: true })
          .sort({ createdAt: -1 })
          .limit(20)
          .lean()
          .exec(),
      ]);

      const totalSamples =
        memories.length + conversations.length + messages.length;

      if (totalSamples < this.MIN_SAMPLE_SIZE) {
        const defaultPattern = this.createDefaultBehaviorPattern(userId);
        await this.cache.set(cacheKey, defaultPattern, this.ANALYSIS_CACHE_TTL);
        return defaultPattern;
      }

      const behaviorAnalysis = await this.generateBehaviorAnalysis(
        memories,
        conversations,
        messages,
      );

      const behaviorPattern: UserBehaviorPattern = {
        userId,
        patterns: behaviorAnalysis.patterns,
        confidence: Math.min(0.95, totalSamples / 50),
        lastAnalyzed: new Date(),
        sampleSize: totalSamples,
      };

      await this.cache.set(cacheKey, behaviorPattern, this.ANALYSIS_CACHE_TTL);

      this.logger.log('Behavior pattern analysis completed', {
        userId,
        confidence: behaviorPattern.confidence,
        sampleSize: totalSamples,
      });

      return behaviorPattern;
    } catch (error) {
      this.logger.error('Behavior pattern analysis failed', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
      return this.createDefaultBehaviorPattern(userId);
    }
  }

  async generatePredictiveInsights(
    userId: string,
    currentContext?: string,
  ): Promise<PredictiveInsight[]> {
    try {
      this.logger.log('Generating predictive insights', { userId });

      const behaviorPattern = await this.analyzeUserBehaviorPatterns(userId);

      let contextualData: {
        totalResults?: number;
        memories?: unknown[];
        conversations?: unknown[];
        messages?: unknown[];
      } = {};
      if (currentContext) {
        contextualData = await this.vectorMemoryService.crossModelSearch(
          currentContext,
          userId,
          {
            includeConversations: true,
            includeMemories: true,
            includeMessages: true,
            limit: 5,
          },
        );
      }

      const insights = await this.generateAIPredictiveInsights(
        behaviorPattern,
        contextualData,
        currentContext,
      );

      this.logger.log('Predictive insights generated', {
        userId,
        insightCount: insights.length,
        categories: insights.map((i) => i.category),
      });

      return insights;
    } catch (error) {
      this.logger.error('Predictive insights generation failed', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
      return [];
    }
  }

  async discoverCrossDomainConnections(
    userId: string,
  ): Promise<CrossDomainConnection[]> {
    try {
      this.logger.log('Discovering cross-domain connections', { userId });

      const [memories, conversations, messages] = await Promise.all([
        this.userMemoryModel
          .find({
            userId,
            isActive: true,
            semanticEmbedding: { $exists: true },
          })
          .limit(10)
          .lean()
          .exec(),
        this.conversationMemoryModel
          .find({
            userId,
            isArchived: false,
            queryEmbedding: { $exists: true },
          })
          .limit(10)
          .lean()
          .exec(),
        this.chatMessageModel
          .find({
            userId,
            isVectorized: true,
            semanticEmbedding: { $exists: true },
          })
          .limit(5)
          .lean()
          .exec(),
      ]);

      const connections: CrossDomainConnection[] = [];

      connections.push(
        ...(await this.findSemanticConnections(
          memories,
          conversations,
          messages,
        )),
      );
      connections.push(
        ...(await this.findTemporalConnections(
          memories,
          conversations,
          messages,
        )),
      );
      connections.push(
        ...(await this.findBehavioralConnections(
          memories,
          conversations,
          messages,
        )),
      );

      this.logger.log('Cross-domain connections discovered', {
        userId,
        connectionCount: connections.length,
        types: connections.map((c) => c.connectionType),
      });

      return connections;
    } catch (error) {
      this.logger.error('Cross-domain connection discovery failed', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
      return [];
    }
  }

  async updateUserContextModel(
    userId: string,
    newInteraction: {
      type: 'memory' | 'conversation' | 'message';
      content: string;
      metadata?: Record<string, unknown>;
    },
  ): Promise<void> {
    try {
      this.logger.log('Updating user context model', {
        userId,
        type: newInteraction.type,
      });

      const currentPattern = await this.analyzeUserBehaviorPatterns(userId);
      const impactAnalysis = this.analyzeInteractionImpact(
        newInteraction,
        currentPattern,
      );

      if (impactAnalysis.significantChange) {
        await this.analyzeUserBehaviorPatterns(userId, true);
        await this.generatePredictiveInsights(userId);
        this.logger.log('Context model updated with significant changes', {
          userId,
          impactScore: impactAnalysis.impactScore,
        });
      } else {
        this.logger.log('Context model updated incrementally', {
          userId,
          impactScore: impactAnalysis.impactScore,
        });
      }
    } catch (error) {
      this.logger.error('Context model update failed', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
    }
  }

  async getEnhancedMemoryContext(
    userId: string,
    query: string,
  ): Promise<{
    behaviorPattern: UserBehaviorPattern;
    predictiveInsights: PredictiveInsight[];
    crossDomainConnections: CrossDomainConnection[];
    contextualRecommendations: string[];
  }> {
    try {
      this.logger.log('Getting enhanced memory context', { userId });

      const [behaviorPattern, predictiveInsights, crossDomainConnections] =
        await Promise.all([
          this.analyzeUserBehaviorPatterns(userId),
          this.generatePredictiveInsights(userId, query),
          this.discoverCrossDomainConnections(userId),
        ]);

      const contextualRecommendations =
        await this.generateContextualRecommendations(
          behaviorPattern,
          predictiveInsights,
          crossDomainConnections,
          query,
        );

      return {
        behaviorPattern,
        predictiveInsights,
        crossDomainConnections,
        contextualRecommendations,
      };
    } catch (error) {
      this.logger.error('Enhanced memory context retrieval failed', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
      return {
        behaviorPattern: this.createDefaultBehaviorPattern(userId),
        predictiveInsights: [],
        crossDomainConnections: [],
        contextualRecommendations: [],
      };
    }
  }

  private async generateBehaviorAnalysis(
    memories: Array<{ memoryType: string; content: string }>,
    conversations: Array<{ query: string; response: string }>,
    messages: Array<{ role: string; content: string }>,
  ): Promise<{ patterns: UserBehaviorPattern['patterns'] }> {
    try {
      const analysisPrompt = `Analyze user behavior patterns from this data:

MEMORIES (${memories.length} items):
${memories
  .map((m, i) => `${i + 1}. [${m.memoryType}]: ${m.content.substring(0, 100)}`)
  .join('\n')}

CONVERSATIONS (${conversations.length} items):
${conversations
  .map(
    (c, i) =>
      `${i + 1}. Q: "${c.query.substring(0, 80)}" A: "${c.response.substring(0, 80)}"`,
  )
  .join('\n')}

MESSAGES (${messages.length} items):
${messages
  .map(
    (msg, i) =>
      `${i + 1}. [${msg.role}]: ${(msg.content || '').substring(0, 80) || 'No preview'}`,
  )
  .join('\n')}

Analyze patterns and respond with JSON:
{
  "patterns": {
    "preferredTopics": ["topic1", "topic2"],
    "queryComplexity": "simple|moderate|complex",
    "responsePreference": "concise|detailed|comprehensive",
    "technicalLevel": "beginner|intermediate|expert",
    "costSensitivity": "low|medium|high",
    "peakUsageHours": [hour1, hour2],
    "commonQuestionTypes": ["type1", "type2"]
  }
}

Focus on AI cost optimization, technical preferences, and usage patterns.`;

      const response = await this.intelligenceAgent.invoke([
        new HumanMessage(analysisPrompt),
      ]);
      const analysis = this.parseAIResponse(response.content.toString()) as {
        patterns: UserBehaviorPattern['patterns'];
      };

      return analysis ?? this.getDefaultPatterns();
    } catch (error) {
      this.logger.error('Failed to generate behavior analysis', {
        error: error instanceof Error ? error.message : String(error),
      });
      return { patterns: this.getDefaultPatterns().patterns };
    }
  }

  private async generateAIPredictiveInsights(
    behaviorPattern: UserBehaviorPattern,
    contextualData: {
      totalResults?: number;
      memories?: unknown[];
      conversations?: unknown[];
      messages?: unknown[];
    },
    currentContext?: string,
  ): Promise<PredictiveInsight[]> {
    try {
      const insightsPrompt = `Based on user behavior patterns and context, generate predictive insights:

USER BEHAVIOR PATTERN:
- Preferred Topics: ${behaviorPattern.patterns.preferredTopics.join(', ')}
- Technical Level: ${behaviorPattern.patterns.technicalLevel}
- Cost Sensitivity: ${behaviorPattern.patterns.costSensitivity}
- Response Preference: ${behaviorPattern.patterns.responsePreference}
- Common Question Types: ${behaviorPattern.patterns.commonQuestionTypes.join(', ')}

${currentContext ? `CURRENT CONTEXT: ${currentContext}` : ''}

${
  contextualData.totalResults
    ? `
CONTEXTUAL DATA:
- Memories: ${contextualData.memories?.length ?? 0} relevant items
- Conversations: ${contextualData.conversations?.length ?? 0} similar discussions
- Messages: ${contextualData.messages?.length ?? 0} related messages
`
    : ''
}

Generate 3-5 predictive insights as JSON array:
[{
  "type": "cost_optimization|usage_pattern|behavior_prediction|recommendation",
  "insight": "Specific insight text",
  "confidence": 0.0-1.0,
  "actionable": true/false,
  "estimatedImpact": "low|medium|high",
  "category": "category_name",
  "metadata": {}
}]

Focus on actionable cost optimization and usage improvement insights.`;

      const response = await this.intelligenceAgent.invoke([
        new HumanMessage(insightsPrompt),
      ]);
      const insights = this.parseAIResponse(
        response.content.toString(),
      ) as PredictiveInsight[];

      return Array.isArray(insights) ? insights : [];
    } catch (error) {
      this.logger.error('Failed to generate AI predictive insights', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  }

  private async findSemanticConnections(
    memories: Array<{ content: string; semanticEmbedding?: number[] }>,
    conversations: Array<{ query: string; queryEmbedding?: number[] }>,
    _messages: unknown[],
  ): Promise<CrossDomainConnection[]> {
    const connections: CrossDomainConnection[] = [];
    try {
      for (const memory of memories.slice(0, 5)) {
        if (!memory.semanticEmbedding) continue;
        for (const conv of conversations.slice(0, 5)) {
          if (!conv.queryEmbedding) continue;
          const similarity = this.calculateCosineSimilarity(
            memory.semanticEmbedding,
            conv.queryEmbedding,
          );
          if (similarity > 0.8) {
            connections.push({
              sourceType: 'memory',
              targetType: 'conversation',
              connectionType: 'semantic',
              strength: similarity,
              description: `Memory about "${memory.content.substring(0, 50)}" relates to conversation "${conv.query.substring(0, 50)}"`,
              examples: [
                memory.content.substring(0, 100),
                conv.query.substring(0, 100),
              ],
            });
          }
        }
      }
      return connections;
    } catch (error) {
      this.logger.error('Failed to find semantic connections', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  }

  private async findTemporalConnections(
    memories: Array<{ content: string; createdAt: Date }>,
    conversations: Array<{ query: string; createdAt: Date }>,
    messages: Array<{ content: string; createdAt: Date }>,
  ): Promise<CrossDomainConnection[]> {
    const connections: CrossDomainConnection[] = [];
    try {
      const sortedData = [
        ...memories.map((m) => ({ ...m, type: 'memory' as const })),
        ...conversations.map((c) => ({ ...c, type: 'conversation' as const })),
        ...messages.map((msg) => ({ ...msg, type: 'message' as const })),
      ].sort(
        (a, b) =>
          new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(),
      );

      for (let i = 0; i < sortedData.length - 1; i++) {
        const current = sortedData[i];
        const next = sortedData[i + 1];
        const timeDiff =
          new Date(next.createdAt).getTime() -
          new Date(current.createdAt).getTime();
        const hoursDiff = timeDiff / (1000 * 60 * 60);

        if (hoursDiff <= 24 && current.type !== next.type) {
          connections.push({
            sourceType: current.type,
            targetType: next.type,
            connectionType: 'temporal',
            strength: Math.max(0.1, 1 - hoursDiff / 24),
            description: `${current.type} at ${new Date(current.createdAt).toLocaleString()} followed by ${next.type}`,
            examples: [
              (current as { content?: string; query?: string }).content ||
                (current as { content?: string; query?: string }).query ||
                'No content',
              (next as { content?: string; query?: string }).content ||
                (next as { content?: string; query?: string }).query ||
                'No content',
            ],
          });
        }
      }
      return connections;
    } catch (error) {
      this.logger.error('Failed to find temporal connections', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  }

  private async findBehavioralConnections(
    memories: Array<{ content: string }>,
    conversations: Array<{ query: string }>,
    messages: Array<{ content?: string }>,
  ): Promise<CrossDomainConnection[]> {
    try {
      const behaviorPrompt = `Analyze behavioral connections between these data points:

MEMORIES: ${memories.map((m) => m.content.substring(0, 80)).join(' | ')}
CONVERSATIONS: ${conversations.map((c) => c.query.substring(0, 80)).join(' | ')}
MESSAGES: ${messages.map((msg) => (msg.content ?? '').substring(0, 80) || 'No preview').join(' | ')}

Find behavioral patterns and causal relationships. Return JSON array:
[{
  "sourceType": "memory|conversation|message",
  "targetType": "memory|conversation|message",
  "connectionType": "causal|behavioral",
  "strength": 0.0-1.0,
  "description": "Connection description",
  "examples": ["example1", "example2"]
}]`;

      const response = await this.intelligenceAgent.invoke([
        new HumanMessage(behaviorPrompt),
      ]);
      const conns = this.parseAIResponse(
        response.content.toString(),
      ) as CrossDomainConnection[];

      return Array.isArray(conns) ? conns : [];
    } catch (error) {
      this.logger.error('Failed to find behavioral connections', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  }

  private analyzeInteractionImpact(
    interaction: { type: string; content: string },
    currentPattern: UserBehaviorPattern,
  ): { significantChange: boolean; impactScore: number } {
    try {
      let impactScore = 0.1;
      const currentTopics = currentPattern.patterns.preferredTopics
        .join(' ')
        .toLowerCase();
      if (
        !currentTopics.includes(
          interaction.content.toLowerCase().substring(0, 50),
        )
      ) {
        impactScore += 0.3;
      }
      if (currentPattern.sampleSize < 10) {
        impactScore += 0.4;
      }
      return {
        significantChange: impactScore > 0.5,
        impactScore,
      };
    } catch (error) {
      this.logger.error('Failed to analyze interaction impact', {
        error: error instanceof Error ? error.message : String(error),
      });
      return { significantChange: false, impactScore: 0.1 };
    }
  }

  private async generateContextualRecommendations(
    behaviorPattern: UserBehaviorPattern,
    insights: PredictiveInsight[],
    connections: CrossDomainConnection[],
    query: string,
  ): Promise<string[]> {
    try {
      const recPrompt = `Generate contextual recommendations based on:

BEHAVIOR PATTERN:
- Technical Level: ${behaviorPattern.patterns.technicalLevel}
- Cost Sensitivity: ${behaviorPattern.patterns.costSensitivity}
- Preferred Topics: ${behaviorPattern.patterns.preferredTopics.join(', ')}

CURRENT QUERY: ${query}

INSIGHTS: ${insights.map((i) => i.insight).join(' | ')}

CONNECTION PATTERNS: ${connections.length} cross-domain connections found

Generate 3-5 specific, actionable recommendations as JSON array of strings:
["recommendation1", "recommendation2", "recommendation3"]

Focus on cost optimization and technical improvements relevant to the user's level and interests.`;

      const response = await this.intelligenceAgent.invoke([
        new HumanMessage(recPrompt),
      ]);
      const recommendations = this.parseAIResponse(
        response.content.toString(),
      ) as string[];

      return Array.isArray(recommendations)
        ? recommendations
        : [
            'Consider optimizing your Claude model usage based on query complexity',
            'Review your peak usage patterns for potential cost savings',
            'Explore batch processing for similar queries to reduce costs',
          ];
    } catch (error) {
      this.logger.error('Failed to generate contextual recommendations', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
  }

  private createDefaultBehaviorPattern(userId: string): UserBehaviorPattern {
    return {
      userId,
      patterns: {
        preferredTopics: ['cost optimization', 'ai models', 'general'],
        queryComplexity: 'moderate',
        responsePreference: 'detailed',
        technicalLevel: 'intermediate',
        costSensitivity: 'medium',
        peakUsageHours: [9, 14, 16],
        commonQuestionTypes: ['how-to', 'optimization', 'troubleshooting'],
      },
      confidence: 0.3,
      lastAnalyzed: new Date(),
      sampleSize: 0,
    };
  }

  private getDefaultPatterns(): {
    patterns: UserBehaviorPattern['patterns'];
  } {
    return {
      patterns: {
        preferredTopics: ['cost optimization', 'ai models'],
        queryComplexity: 'moderate',
        responsePreference: 'detailed',
        technicalLevel: 'intermediate',
        costSensitivity: 'medium',
        peakUsageHours: [9, 14, 16],
        commonQuestionTypes: ['how-to', 'optimization'],
      },
    };
  }

  private parseAIResponse(content: string): unknown {
    try {
      const jsonMatch = content.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
      if (jsonMatch) {
        return JSON.parse(jsonMatch[0]);
      }
      throw new Error('No JSON found in response');
    } catch (error) {
      this.logger.error('Failed to parse AI response', {
        error: error instanceof Error ? error.message : String(error),
        content: content.substring(0, 200),
      });
      return null;
    }
  }

  private calculateCosineSimilarity(
    vectorA: number[],
    vectorB: number[],
  ): number {
    try {
      if (vectorA.length !== vectorB.length) return 0;

      let dotProduct = 0;
      let normA = 0;
      let normB = 0;

      for (let i = 0; i < vectorA.length; i++) {
        dotProduct += vectorA[i] * vectorB[i];
        normA += vectorA[i] * vectorA[i];
        normB += vectorB[i] * vectorB[i];
      }

      const denominator = Math.sqrt(normA) * Math.sqrt(normB);
      return denominator === 0 ? 0 : dotProduct / denominator;
    } catch {
      return 0;
    }
  }
}
