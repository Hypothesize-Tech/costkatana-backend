...
export class AiQueryRouterService {
  private readonly logger = new Logger(AiQueryRouterService.name);
  ...
  constructor() {
    ...
    this.logger.log('🧠 AI Query Router initialized');
  }
  ...
  async routeQuery(
    ...
    try {
      this.logger.log('🔀 AI Router analyzing query', {
      ...
      this.logger.error('❌ AI Router failed, using fallback', {
        error: errorMessage,
  ...
  private buildRoutingPrompt(query: string, context: RouterContext): string {
    ...
    if (context.hasVercelConnection)
      integrationStatus.push('Vercel: CONNECTED');
    if (context.hasGithubConnection)
      integrationStatus.push('GitHub: CONNECTED');
    if (context.hasGoogleConnection)
      integrationStatus.push('Google: CONNECTED');
...
Respond with JSON only:
{
  ...
  private fallbackRouting(
    ...
    if (
      lowerQuery.includes('costkatana') ||
    ...
    if (
      lowerQuery.includes('about that') ||
      ...
      return {
        ...
        confidence: 0.6,
        reasoning:
          'Fallback: Ambiguous query - needs clarification or direct response',
        suggestedTools: [],
        extractedParams: {},
        requiresIntegration: false,
      };
    ...
    return {
      ...
      confidence: 0.5,
      reasoning:
        'Fallback: No specific pattern detected, using direct response',
      suggestedTools: [],
      extractedParams: {},
      requiresIntegration: false,
    };
   ...
   */
  requiresIntegration(query: string): {
    required: boolean;
    integration?: string;
  } {
    const lowerQuery = query.toLowerCase();
