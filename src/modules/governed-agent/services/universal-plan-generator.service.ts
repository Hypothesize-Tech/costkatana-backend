...
Respond with a JSON array of strings, each being a concise, actionable finding.`;
   ...
   */
  private async getPlaceholderResearch(
    userRequest: string,
    searchQueries: string[],
  ): Promise<ResearchResult> {
    try {
      // Generate AI-based research as fallback when web search fails
      const researchPrompt = `Based on your knowledge, provide research insights for this task:

Task: "${userRequest}"

Generate research findings including:
1. Best practices and approaches
...
{
  "synthesis": "Brief summary of key insights",
  ...
  "recommendations": ["rec1", "rec2", "rec3", ...]
      ...
      try {
        ...
        researchData = {
          synthesis:
            'AI-generated research insights based on best practices and common patterns.',
          keyFindings: [
          ...
          recommendations: [
            'Start with a clear requirements analysis',
      ...
      return {
        query: userRequest,
        sources: searchQueries.map((query, index) => ({
          title: `AI Research: ${query}`,
          url: `https://ai-research.example.com/insight-${index + 1}`,
          snippet:
            researchData.keyFindings[index] ||
            `AI-generated insight for: ${query}`,
          relevance: 0.8,
        })),
      ...
      this.logger.error('AI research fallback failed', {
        userRequest,
        error: error instanceof Error ? error.message : String(error),
      });

      // Final fallback to static research
      return {
        query: userRequest,
        sources: searchQueries.map((query) => ({
          title: `Fallback Research: ${query}`,
          url: 'https://fallback-research.example.com',
          snippet: `General best practices for: ${query}`,
          relevance: 0.6,
        })),
        synthesis: 'Using fallback research due to AI service unavailability.',
        keyFindings: [
