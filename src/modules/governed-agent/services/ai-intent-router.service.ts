...
export class AiIntentRouterService {
  ...
  private cache: Map<string, { result: IntentAnalysis; timestamp: number }> =
    new Map();
  ...
  constructor() {
    ...
    this.logger.log('🧭 AI Intent Router initialized');
  }
  ...
  async analyzeIntent(
    ...
    if (cached && Date.now() - cached.timestamp < this.CACHE_TTL) {
      this.logger.log('🧭 Router cache hit', { route: cached.result.route });
    ...
    try {
      ...
      this.logger.log('🧭 AI Intent analysis complete', {
        route: result.route,
      ...
      this.logger.error('AI Intent analysis failed, using fallback', {
...
Respond with ONLY valid JSON (no markdown, no code fences):
{
  ...
  "suggestedTools": ["tool1", "tool2"]
    ...
    try {
      ...
      this.logger.warn('Failed to parse AI router response', {
  ...
  private fallbackAnalysis(
    ...
    const integrationMatch = query.match(
      /@(vercel|github|linear|slack|discord|jira|gmail|calendar|drive|docs|sheets|slides|forms|google):?/i,
    );
    if (integrationMatch) {
      ...
      return {
        ...
        suggestedTools:
          integration === 'vercel'
            ? ['vercel_list_projects', 'vercel_list_domains']
            : [],
      };
    }
  ...
  private extractActualQuery(message: string): string {
    ...
    if (
      ...
      lastNonEmptyLine.length < 500
    ) {
      // Check if this looks like a query (not context)
      if (
        !lastNonEmptyLine.toLowerCase().startsWith('intent:') &&
  ...
  isKnowledgeBaseQuery(message: string): boolean {
    ...
    return (
      /cost\s*katana/i.test(actualQuery) ||
