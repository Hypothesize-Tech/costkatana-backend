...
export class HtmlSecurityService {
  private readonly logger = new Logger(HtmlSecurityService.name);
   ...
   */
  extractTextFromHTML(htmlContent: string): HTMLSecurityResult {
    try {
            ...
            if (attr.startsWith('data-')) {
              ...
              if (dataValue) {
                ...
                if (this.detectObfuscation(dataValue)) {
                  obfuscatedContent.push(
                    `data-${attr}:${dataValue.substring(0, 100)}`,
                  );
                }
              }
            }
      ...
      this.logger.error('Error extracting text from HTML', {
  ...
  private detectObfuscation(content: string): boolean {
    ...
    if (
      entityPattern.test(content) &&
      content.match(entityPattern)!.length > content.length * 0.2
    ) {
      return true;
    }
  ...
  private decodeHTML(content: string): string {
    try {
      ...
      this.logger.warn('Error decoding HTML content', {
        error: error instanceof Error ? error.message : String(error),
      });
      return content;
    }
   ...
   */
  prepareContentForScanning(content: string): {
    textToScan: string;
    isHTML: boolean;
