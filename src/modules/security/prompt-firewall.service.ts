 ...
 *
 * Uses BedrockService.invokeModelDirectly() for all Bedrock calls so the app has a single
 * place for AWS Bedrock invocation; the firewall supplies model-specific payloads and
 * parses threat-detection responses itself.
 */
...
export class PromptFirewallService {
  private readonly logger = new Logger(PromptFirewallService.name);
  ...
  constructor(
    ...
    private threatLogModel: Model<ThreatLogDocument>,
   ...
   */
  private isIntegrationCommand(prompt: string): boolean {
  ...
  async checkPrompt(
    ...
    try {
      ...
      if (this.isIntegrationCommand(prompt)) {
        this.logger.debug('Integration command detected - whitelisted', {
          requestId,
      ...
      const preparedContent =
        this.htmlSecurityService.prepareContentForScanning(prompt);
      ...
      if (isHTML) {
        this.logger.debug(
          'HTML content detected, extracted text for scanning',
          {
            requestId,
      ...
      if (config.enableBasicFirewall) {
        const promptGuardResult = await this.runPromptGuard(
          textToScan,
          config.promptGuardThreshold,
        );
      ...
      if (config.enableAdvancedFirewall) {
        const aiDetectionResult = await this.runAIDetection(
          textToScan,
          config.openaiSafeguardThreshold,
          isHTML,
        ...
        if (
          aiDetectionResult.threatCategory &&
          aiDetectionResult.confidence > 0.3
        ) {
          await this.logThreatDetection(
            requestId,
            aiDetectionResult,
      ...
      return {
        ...
        stage: config.enableAdvancedFirewall
          ? 'openai-safeguard'
          : 'prompt-guard',
        containmentAction: 'allow',
      };
    } catch (error) {
      this.logger.error('Error in prompt firewall check', {
   ...
   */
  private async runPromptGuard(
    prompt: string,
    threshold: number,
  ): Promise<ThreatDetectionResult> {
    const useBedrockModels = process.env.ENABLE_BEDROCK_FIREWALL === 'true';
    ...
    if (useBedrockModels && !this.isServiceCircuitBreakerOpen()) {
      try {
        ...
        this.logger.warn('Bedrock Prompt Guard failed, using fallback', {
  ...
  private async runAIDetection(
    ...
    htmlMetadata?: HTMLSecurityResult['htmlMetadata'],
  ): Promise<ThreatDetectionResult> {
    const useBedrockModels = process.env.ENABLE_BEDROCK_FIREWALL === 'true';
...
JSON Response:`;
      ...
      try {
        ...
        if (shouldEscalate && initialResult.isBlocked) {
          this.logger.log('Escalating threat detection to Safeguard 120B', {
          ...
          try {
            ...
            this.logger.warn('120B escalation failed, using 20B result', {
              error: error instanceof Error ? error.message : String(error),
            });
        ...
        this.logger.warn('Safeguard models failed, falling back to Nova Pro', {
          error: error instanceof Error ? error.message : String(error),
        });
        ...
        try {
          ...
          this.recordServiceFailure();
          this.logger.warn(
            'All AI detection failed, using pattern-based fallback',
            {
              error:
   ...
   */
  private isHighRiskThreat(category: string): boolean {
    const highRisk = [
      'prompt_injection',
      'data_exfiltration',
      'criminal_planning',
   ...
   */
  private async invokeThreatDetectionModel(
    modelId: string,
    requestBody: any,
    prompt: string,
    threshold: number,
    ...
    const result = (await ServiceHelper.withRetry(
      () => BedrockService.invokeModelDirectly(modelId, requestBody),
    ...
    try {
      ...
      this.logger.warn(
        'Failed to parse AI detection response, using fallback',
        {
          error:
    ...
    if (detectionResult?.isThreat) {
      ...
      const normalizedCategory = this.normalizeThreatCategory(
        detectionResult.threatCategory ?? 'harmful_content',
      );
      ...
      return {
        ...
        containmentAction:
          (detectionResult.confidence ?? 0.9) > 0.8 ? 'block' : 'sandbox',
        details: {
          method: modelId.includes('120b')
            ? 'safeguard_120b'
            : modelId.includes('20b')
              ? 'safeguard_20b'
   ...
   */
  private fallbackPromptGuardCheck(
    prompt: string,
    threshold: number,
   ...
   */
  private fallbackContentCheck(prompt: string): ThreatDetectionResult {
    const matchedPatterns: string[] = [];
    ...
    for (const pattern of this.HARMFUL_PATTERNS) {
      if (pattern.test(prompt)) {
        ...
        return {
          ...
          reason:
            'Content contains potentially harmful patterns (optimized check)',
          stage: 'openai-safeguard',
          matchedPatterns,
  ...
  private async logThreatDetection(
    ...
    try {
      const threatLog = new this.threatLogModel({
        requestId,
        userId: userId
          ? new (await import('mongoose')).Types.ObjectId(userId)
          : undefined,
        ...
        timestamp: new Date(),
        promptHash: originalPrompt
          ? this.hashPrompt(originalPrompt)
          : undefined,
        promptPreview: originalPrompt
          ? this.sanitizePromptForPreview(originalPrompt)
          : undefined,
      ...
      this.logger.log('Threat detected and logged to database', {
        requestId,
      ...
      this.logger.error('Error logging threat detection', {
        error: error instanceof Error ? error.message : String(error),
        requestId,
        threatCategory: result.threatCategory,
      });
   ...
   */
  private normalizeThreatCategory(category: string | null | undefined): string {
    if (!category) {
  ...
  async getFirewallAnalytics(
    ...
    try {
      ...
      result.savingsByThreatType.forEach((item: any) => {
        savingsByThreatType[item.category] =
          (savingsByThreatType[item.category] || 0) + item.cost;
      });
      ...
      this.logger.error('Error getting firewall analytics', {
        error: error instanceof Error ? error.message : String(error),
        userId,
      });
   ...
   */
  getDefaultConfig(): FirewallConfig {
    return {
  ...
  parseConfigFromHeaders(
    ...
    if (headers['costkatana-firewall-rag-threshold']) {
      const threshold = parseFloat(
        headers['costkatana-firewall-rag-threshold'],
      );
      if (!isNaN(threshold) && threshold >= 0 && threshold <= 1) {
    ...
    if (headers['costkatana-firewall-tool-threshold']) {
      const threshold = parseFloat(
        headers['costkatana-firewall-tool-threshold'],
      );
      if (!isNaN(threshold) && threshold >= 0 && threshold <= 1) {
   ...
   */
  private sanitizePromptForPreview(prompt: string): string {
    if (!prompt) return '';
