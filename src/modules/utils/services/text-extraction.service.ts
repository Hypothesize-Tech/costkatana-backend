...
export class TextExtractionService {
  ...
  private async extractFromXLSX(buffer: Buffer): Promise<ExtractionResult> {
    try {
      ...
      return {
        ...
        success: false,
        error:
          error instanceof Error ? error.message : 'XLSX extraction failed',
      };
    }
  }
  ...
  async extractTextFromS3(
    ...
    try {
      const buffer = await this.storageService.getFileBuffer(s3Key);
      return await this.extractText(buffer, fileType, filename);
    } catch (error) {
      this.logger.error('Failed to extract text from S3', {
        error,
        s3Key,
        fileType,
        filename,
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : 'Failed to extract text from S3',
      };
    }
  }
