...
export class WebSearchToolService {
  ...
  private readonly summarizer: ChatBedrockConverse;
  ...
  /**
   * Main entry point for LangChain compatibility
   */
    ...
    try {
      ...
      try {
        ...
        this.loggerService.warn(
          'WebSearchTool received non-JSON input, treating as search query',
          {
      ...
      if (!request.operation) {
        this.loggerService.warn(
          'WebSearchTool operation missing, defaulting to search',
          {
            input: input.substring(0, 200),
          },
        );
      ...
      this.loggerService.info('Web search operation initiated', {
      ...
      this.loggerService.info('Web search operation completed', {
        operation: request.operation,
      ...
      this.loggerService.error('Web search operation failed', {
  ...
  private async performSearch(
    ...
    try {
      const searchOptions = {
        maxResults: request.options?.maxResults || this.MAX_RESULTS,
        deepContent: request.options?.deepContent || false,
        domains: request.options?.costDomains ? this.COST_DOMAINS : undefined,
      };
      ...
      let extractedText = searchResults
        .map((r) => `${r.title}\n${r.snippet}\n`)
        .join('\n');
      ...
      if (request.options?.deepContent) {
        searchResults.forEach((result) => {
          const contentResult = (result as any).content;
          if (contentResult) {
            extractedText += `\n\n--- ${contentResult.title} ---\n${contentResult.cleanedText.substring(0, 2000)}\n`;
          }
        });
      }

      // Get quota status
      const quotaStatus = await this.googleSearchService.getQuotaStatus();
      ...
      this.loggerService.error('Search operation failed', {
        query: request.query,
        error: error instanceof Error ? error.message : String(error),
      });
  ...
  private async scrapeUrl(request: WebSearchRequest): Promise<WebSearchResult> {
    ...
    try {
      this.loggerService.info('Scraping URL directly', { url: request.url });
      ...
      this.loggerService.error('URL scraping failed', {
        url: request.url,
        error: error instanceof Error ? error.message : String(error),
      });
   ...
   */
  private async extractWithSearch(
    request: WebSearchRequest,
  ): Promise<WebSearchResult> {
    if (!request.query) {
