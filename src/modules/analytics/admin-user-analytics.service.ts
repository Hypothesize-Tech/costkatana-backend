...
export class AdminUserAnalyticsService {
  ...
  async getAllUsersSpending(
    ...
    try {
      ...
      const userUsageData = await this.usageModel.aggregate([
        { $match: matchStage },
        {
          $group: {
            _id: '$userId',
      ...
      const processedData = userUsageData.map((userData: any) => {
        ...
        userData.services.forEach((s: any) => {
          const existing = servicesMap.get(s.service) || {
            cost: 0,
            tokens: 0,
            requests: 0,
          };
        ...
        userData.models.forEach((m: any) => {
          if (m.model) {
            const existing = modelsMap.get(m.model) || {
              cost: 0,
              tokens: 0,
              requests: 0,
            };
        ...
        userData.workflows.forEach((w: any) => {
          if (w.workflowId) {
            const existing = workflowsMap.get(w.workflowId) || {
              cost: 0,
              tokens: 0,
              requests: 0,
        ...
        userData.features?.forEach((f: any) => {
          if (f.endpoint) {
            ...
            const existing = featuresMap.get(featureName) || {
              cost: 0,
              tokens: 0,
              requests: 0,
            };
        ...
        return {
          ...
          services: Array.from(servicesMap.entries()).map(
            ([service, data]) => ({
              service,
              ...data,
            }),
          ...
          projects: Array.from(projectsMap.entries()).map(
            ([projectId, data]) => ({
              projectId,
          ...
          workflows: Array.from(workflowsMap.entries()).map(
            ([workflowId, data]) => ({
              workflowId,
          ...
          features: Array.from(featuresMap.entries()).map(
            ([feature, data]) => ({
              feature,
              ...data,
            }),
      ...
      const projectIdArray = Array.from(projectIds).map(
        (id) => new mongoose.Types.ObjectId(id),
      );
      const projects = await this.projectModel
        .find({ _id: { $in: projectIdArray } }, { name: 1 })
        .lean();
      ...
      // Get user details
      const userIds = processedData.map(
      ...
      this.loggingService.error('Error getting all users spending:', {
  ...
  async getUserDetailedSpending(
    ...
    try {
      ...
      this.loggingService.error('Error getting user detailed spending:', {
        error: error instanceof Error ? error.message : String(error),
        component: 'AdminUserAnalyticsService',
  ...
  async getSpendingTrends(
    ...
    try {
      ...
      this.loggingService.error('Error getting spending trends:', {
        error: error instanceof Error ? error.message : String(error),
        component: 'AdminUserAnalyticsService',
   ...
   */
  async getPlatformSummary(filters: AdminUserAnalyticsFilters = {}): Promise<{
    totalUsers: number;
    totalCost: number;
    totalTokens: number;
    totalRequests: number;
    ...
    try {
      ...
      const totalCost = usersSpending.reduce(
        (sum, user) => sum + user.totalCost,
        0,
      );
      const totalTokens = usersSpending.reduce(
        (sum, user) => sum + user.totalTokens,
        0,
      );
      const totalRequests = usersSpending.reduce(
      ...
      return {
        ...
        totalRequests,
        averageCostPerUser:
          usersSpending.length > 0 ? totalCost / usersSpending.length : 0,
        topSpendingUsers: usersSpending.slice(0, 10).map((user) => ({
          userId: user.userId,
      ...
      this.loggingService.error('Error getting platform summary:', {
        error: error instanceof Error ? error.message : String(error),
        component: 'AdminUserAnalyticsService',
