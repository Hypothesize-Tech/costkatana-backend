...
export class MetadataEnrichmentService {
  ...
  private readonly config = METADATA_CONFIG;

  constructor(private configService: ConfigService) {
    // Override config with environment variables if available
    this.config.ENABLED = this.configService.get<boolean>(
      'METADATA_ENRICHMENT_ENABLED',
      true,
    );
    this.config.MODEL = this.configService.get<string>(
      'METADATA_ENRICHMENT_MODEL',
      METADATA_CONFIG.MODEL,
    );
    this.config.ENRICHMENT_TIMEOUT = this.configService.get<number>(
      'METADATA_ENRICHMENT_TIMEOUT',
      METADATA_CONFIG.ENRICHMENT_TIMEOUT,
    );
  }
  ...
  async enrichMetadata(
    ...
    try {
      ...
      this.logger.log('Metadata enrichment completed', {
        processingTime,
      ...
      this.logger.error('Metadata enrichment failed', {
...
Return ONLY a JSON array of topics like: ["topic1", "topic2", "topic3"]`;
      ...
      this.logger.warn('Topic extraction failed', {
        error: error instanceof Error ? error.message : String(error),
      });
      return [];
    }
...
Return ONLY the level (beginner, intermediate, or advanced).`;
      ...
      return validLevels.includes(level as TechnicalLevel)
        ? (level as TechnicalLevel)
        : 'intermediate';
    } catch (error) {
      return 'intermediate';
    }
...
Return ONLY the importance level (critical, high, medium, or low).`;
      ...
      return validLevels.includes(importance as ImportanceLevel)
...
Return ONLY the domain (ai-optimization, cost-tracking, api-usage, documentation, or general).`;
      ...
      return validDomains.includes(domain as DomainType)
  ...
  private async analyzeStructure(
    content: string,
  ): Promise<Partial<EnrichedMetadata>> {
    try {
...
Return ONLY a valid JSON object with the above properties.`;
      ...
      const response = await this.invokeLLM(prompt);
      const structure = this.parseJSONResponse<Partial<EnrichedMetadata>>(
        response,
        {},
      );
      ...
      this.logger.warn('Structure analysis failed, using fallback', {
        error: error instanceof Error ? error.message : String(error),
      });
      ...
      return {
        ...
        containsLinks:
          content.match(/https?:\/\/[^\s<>"{}|\\^`[\]]+/g)?.slice(0, 10) ?? [],
  ...
  private async invokeLLM(prompt: string): Promise<string> {
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(
          () => reject(new Error('LLM invocation timeout')),
          this.config.ENRICHMENT_TIMEOUT,
        ),
      ),
    ]);
