...
export class KbIngestionService {
  ...
  async getDocumentChunks(
    userId: string,
    docId: string,
  ): Promise<Array<{ ordinal: number; text: string }>> {
    const tenant = await this.agentDefinitionService.resolveTenant(userId);
    const kbDoc = await this.kbModel.findOne({ organizationId: tenant.organizationId });
    if (!kbDoc) throw new NotFoundException('No knowledge base for this org');

    const chunks = await this.chunkModel
      .find({ knowledgeBaseId: kbDoc._id, docId })
      .select('ordinal text')
      .sort({ ordinal: 1 })
      .lean();

    return chunks.map((c) => ({ ordinal: c.ordinal, text: c.text }));
  }
