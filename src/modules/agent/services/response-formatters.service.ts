 ...
 * Ported from Express ResponseFormattersService
 */
@Injectable()
export class ResponseFormattersService {
  private readonly logger = new Logger(ResponseFormattersService.name);

  /**
   * Format token usage response
   */
  formatTokenUsageResponse(data: any): string {
    if (data.summary) {
   ...
   */
  formatDashboardResponse(data: any): string {
    if (data.summary) {
      const summary = data.summary;
   ...
   */
  formatModelPerformanceResponse(data: any): string {
   ...
   */
  formatUsagePatternsResponse(data: any): string {
    if (data.patterns) {
   ...
   */
  formatCostTrendsResponse(data: any): string {
    if (data.trends) {
   ...
   */
  formatUserStatsResponse(data: any): string {
    if (data.stats) {
   ...
   */
  formatProjectAnalyticsResponse(data: any): string {
   ...
   */
  formatAnomalyDetectionResponse(data: any): string {
   ...
   */
  formatForecastingResponse(data: any): string {
    if (data.forecast) {
   ...
   */
  formatComparativeAnalysisResponse(data: any): string {
    if (data.comparison) {
   ...
   */
  formatResponse(toolOutput: ToolOutput): string {
    try {
      const { operation, data } = toolOutput;

      this.logger.debug('🎨 Formatting response for operation:', { operation });
