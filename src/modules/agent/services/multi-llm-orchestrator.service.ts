 ...
 * Ported from Express MultiLlmOrchestratorService with NestJS patterns
 */
@Injectable()
export class MultiLlmOrchestratorService {
  private readonly logger = new Logger(MultiLlmOrchestratorService.name);

  // Specialized LLMs for different stages
  ...
  private qualityLlm: ChatBedrockConverse; // Claude 3.5 Sonnet - Final response

  constructor() {
    this.initializeModels();
    this.logger.log(
      '🤖 Multi-LLM Orchestrator initialized with 3 specialized models',
    );
  }
  ...
  private initializeModels(): void {
    try {
      ...
      this.logger.error('Failed to initialize LLM models', {
        error: error.message,
      });
      throw error;
    }
  ...
  async analyzeQuery(query: string): Promise<QueryAnalysis> {
    try {
      this.logger.debug('📊 Stage 1: Analyzing query with fast LLM', {
...
Respond ONLY with valid JSON, no markdown or extra text.`;
      ...
      this.logger.debug('✅ Query analysis complete', {
  ...
  async selectTools(
    ...
    try {
      this.logger.debug('🔧 Stage 2: Selecting tools with smart LLM', {
        category: analysis.category,
        suggestedTools: analysis.suggestedTools,
      });
...
Respond ONLY with valid JSON, no markdown or extra text.`;
      ...
      this.logger.debug('✅ Tool selection complete', {
  ...
  async generateResponse(
    ...
    try {
      this.logger.debug('✨ Stage 3: Generating response with quality LLM', {
...
Generate a natural, conversational response (not JSON).`;
      ...
      this.logger.debug('✅ Response generation complete', {
  ...
  async orchestrate(
    ...
    try {
      this.logger.log('🚀 Starting multi-LLM orchestration pipeline', {
        query: query.substring(0, 100),
      });
      ...
      const toolSelection = await this.selectTools(
        query,
        analysis,
        availableTools,
      );
      ...
      if (toolSelection.parallelizable) {
        ...
        for (const tool of toolSelection.selectedTools.sort(
      ...
      this.logger.log('✅ Multi-LLM orchestration complete', {
        executionTime,
      ...
      this.logger.error('❌ Multi-LLM orchestration failed', {
        error: error.message,
        query: query.substring(0, 100),
      });
  ...
  getModelInfo() {
    return {
      ...
      pipeline:
        'Query Analysis → Tool Selection → Tool Execution → Response Generation',
    };
  }
}
