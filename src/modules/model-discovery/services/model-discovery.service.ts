...
export class ModelDiscoveryService {
  ...
  > = {
    openai: {
      ...
      pricingQueryTemplate:
        'OpenAI {modelName} pricing per token API 2026 cost input output',
    ...
    anthropic: {
      provider: 'anthropic',
      discoveryQuery:
        'site:platform.claude.com/docs/en/about-claude/pricing "Model pricing" Claude Opus Sonnet Haiku MTok',
      pricingQueryTemplate:
        'Anthropic {modelName} pricing per token API 2026 cost input output',
    ...
    'google-ai': {
      provider: 'google-ai',
      discoveryQuery:
        'site:ai.google.dev/gemini-api/docs/pricing "Gemini" "Pro" "Flash" pricing',
      pricingQueryTemplate:
        'Google {modelName} pricing per token API 2026 cost input output',
    ...
    'aws-bedrock': {
      ...
      discoveryQuery:
        'site:aws.amazon.com/bedrock/pricing "Amazon Titan" "Amazon Nova" pricing',
      pricingQueryTemplate:
        'AWS Bedrock {modelName} pricing per token API 2026 cost input output',
      officialDocsUrl:
    ...
    cohere: {
      provider: 'cohere',
      discoveryQuery:
        'site:cohere.com "Command A" "Command R" "Embed" "Rerank" pricing API 2026',
      pricingQueryTemplate:
        'Cohere {modelName} pricing per token API 2026 cost input output',
    ...
    mistral: {
      ...
      pricingQueryTemplate:
        'Mistral {modelName} pricing per token API 2026 cost input output',
    ...
    xai: {
      ...
      pricingQueryTemplate:
        'xAI {modelName} pricing per token API 2026 cost input output',
   ...
   */
  private normalizeModelNameForComparison(name: string): string {
    return name
      .toLowerCase()
      .trim()
      .replace(/\s+/g, ' ')
   ...
   */
  private async getExistingModelIdentifiers(
    provider: string,
  ): Promise<Set<string>> {
    const docs = await this.aiModelPricingModel
      .find({ provider, isActive: true }, { modelId: 1, modelName: 1 })
      .lean();
  ...
  async discoverModelsForProvider(
    ...
    const config = ModelDiscoveryService.PROVIDER_CONFIGS[provider];

    if (!config) {
      throw new Error(`Unknown provider: ${provider}`);
    }

    this.logger.log(`Starting model discovery for ${provider}`, { provider });
    ...
    try {
      ...
      if (modelNames.length === 0) {
        ...
        this.logger.warn(`No models discovered for ${provider}`);
      ...
      this.logger.log(
        `Discovered ${modelNames.length} models for ${provider}`,
        {
          provider,
          models: modelNames,
        },
      ...
      if (modelsSkipped > 0) {
        this.logger.log(
          `Skipping ${modelsSkipped} models already in DB for ${provider}`,
          {
            provider,
            modelsSkipped,
      ...
      if (newModels.length === 0) {
        result.duration = Date.now() - startTime;
        this.logger.log(
          `All ${modelNames.length} models already in DB for ${provider}, no pricing search needed`,
          {
            provider,
      ...
      const pricingResults = await Promise.allSettled(
        newModels.map((modelName) =>
          this.searchModelPricing(config, modelName),
        ),
      );
      ...
      for (let i = 0; i < pricingResults.length; i++) {
        ...
        if (pricingResult.status === 'fulfilled' && pricingResult.value) {
          const validationResult = this.validateAndNormalize(
            pricingResult.value,
            provider,
          );
          ...
          if (validationResult.isValid && validationResult.normalizedData) {
            ...
            result.modelsFailed++;
            result.errors.push(
              `Validation failed for ${modelName}: ${validationResult.errors.join(', ')}`,
            );
          }
        } else {
          result.modelsFailed++;
          ...
          result.errors.push(
            `Failed to get pricing for ${modelName}: ${errorMsg}`,
          );
        }
      }

      result.duration = Date.now() - startTime;
      this.logger.log(`Model discovery completed for ${provider}`, {
        provider,
      ...
      result.duration = Date.now() - startTime;
      this.logger.error(`Model discovery failed for ${provider}`, {
        provider,
        error: errorMessage,
      });
      return result;
  ...
  private async searchModelList(
    ...
    try {
      ...
      if (searchResults.length === 0) {
        this.logger.warn(
          `No search results for ${config.provider} model list, using fallback`,
        );
...
Return ONLY the JSON array:`;
      ...
      // Use Bedrock to extract model names
      const extractionResult = await BedrockService.extractModelsFromText(
        config.provider,
        extractionPrompt,
      );

      if (!extractionResult.success || !Array.isArray(extractionResult.data)) {
        this.logger.warn(
          `LLM returned invalid result for ${config.provider}, using fallback`,
        );
        return ModelDiscoveryService.FALLBACK_MODELS[config.provider] || [];
      }

      const extractedModels = extractionResult.data;

      if (extractedModels.length === 0) {
        this.logger.warn(
          `LLM returned empty array for ${config.provider}, using fallback`,
        );
        return ModelDiscoveryService.FALLBACK_MODELS[config.provider] || [];
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to search model list for ${config.provider}`, {
        error: errorMessage,
      });
   ...
   */
  private async searchModelPricing(
    config: ProviderDiscoveryConfig,
    modelName: string,
  ): Promise<RawPricingData | null> {
    try {
      const query = config.pricingQueryTemplate.replace(
        '{modelName}',
        modelName,
      );

      // Search for model pricing
      const searchResults = await this.googleSearchService.search(query, {
        maxResults: 5,
      });

      if (searchResults.length === 0) {
        this.logger.warn(
          `No pricing results for ${config.provider} ${modelName}`,
        );
        return null;
      }
      ...
      return matchingPricing;
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(
        `Error searching pricing for ${config.provider} ${modelName}`,
        {
          error: errorMessage,
        },
      );
  ...
  private validateAndNormalize(
    ...
    if (typeof data.inputPricePerMToken !== 'number')
      errors.push('Invalid input price');
    if (typeof data.outputPricePerMToken !== 'number')
    ...
    if (data.inputPricePerMToken < 0 || data.inputPricePerMToken > 1000) {
      errors.push(
        `Input price out of range for ${provider}: ${data.inputPricePerMToken}`,
      );
    }
    if (data.outputPricePerMToken < 0 || data.outputPricePerMToken > 1000) {
      errors.push(
        `Output price out of range for ${provider}: ${data.outputPricePerMToken}`,
      );
    }
    ...
    if (data.contextWindow < 1000 || data.contextWindow > 10000000) {
      warnings.push(
        `Context window seems unusual for ${provider}: ${data.contextWindow}`,
      );
    }
    ...
    const normalizedData: RawPricingData = {
      ...
      category: data.category.toLowerCase() as
        | 'text'
        | 'multimodal'
        | 'embedding'
        | 'code',
   ...
   */
  private async storeModelPricing(
    data: RawPricingData,
    provider: string,
    discoverySource: 'google_search' | 'manual' | 'fallback_scraping',
  ): Promise<AIModelPricingDocument> {
    const now = new Date();

    // Check if model already exists
    const existing = await this.aiModelPricingModel.findOne({
    ...
    if (existing) {
      ...
      await existing.save();
      this.logger.log(`Updated existing model: ${data.modelId}`);
      return existing;
    } else {
      // Create new model
      const newModel = new this.aiModelPricingModel({
        modelId: data.modelId,
      ...
      await newModel.save();
      this.logger.log(`Created new model: ${data.modelId}`);
      return newModel;
    }
  }
   ...
   */
  async getDiscoveryStatus(): Promise<DiscoveryJobStatus> {
    const providers = Object.keys(ModelDiscoveryService.PROVIDER_CONFIGS);
    const providerStats: Record<string, any> = {};

    for (const provider of providers) {
      const stats = await this.aiModelPricingModel.aggregate([
        { $match: { provider } },
        {
          $group: {
      ...
      const latest = await this.aiModelPricingModel
        .findOne({ provider })
        .sort({ lastUpdated: -1 })
        .select('lastUpdated');
    ...
    const totalModels = await this.aiModelPricingModel.countDocuments({
      isActive: true,
    });
