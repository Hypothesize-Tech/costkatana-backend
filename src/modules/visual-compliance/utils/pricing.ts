...
const logger = new Logger('VisualCompliancePricing');
...
export const MODEL_PRICING: ModelPricing[] = [
  ...
  // Amazon Titan Models
  {
    ...
    isLatest: true,
    notes:
      'Amazon Titan Text Express - advanced AI model for text generation and chat',
  },
  {
  ...
  },
  {
    ...
    isLatest: true,
    notes:
      'Claude Opus 4.5 on AWS Bedrock Global Inference Profile - Input: $5/1M, Output: $15/1M. Batch: $2.5/$12.5 per 1M. Cache (5m write): $6.25/1M, Cache (1h write): $10/1M, Cache read: $0.5/1M. Routes to best available region',
  },
  {
    ...
    isLatest: true,
    notes:
      'Claude Opus 4.6 on AWS Bedrock - next-gen flagship. Max 1M context (beta). Verify pricing on Bedrock.',
  },
  {
    ...
    isLatest: true,
    notes:
      'Claude Opus 4.7 on Bedrock — verify exact modelId when AWS publishes; list tier $5/$25 per 1M per Anthropic GA',
  },
  {
  ...
  },
  {
    ...
    isLatest: false,
    notes:
      'Claude 2.1 on AWS Bedrock (Geo/In-region) - Input: $8/1M, Output: $24/1M. Legacy model',
  },
  {
    ...
    isLatest: false,
    notes:
      'Claude 2.0 on AWS Bedrock (Geo/In-region) - Input: $8/1M, Output: $24/1M. Legacy model',
  },
  {
...
export function calculateCost(
  ...
  if (!provider || !model) {
    logger.warn('Invalid provider or model in calculateCost', {
      provider,
      model,
      inputTokens,
      outputTokens,
  ...
  if (!pricing) {
    const availableModels = MODEL_PRICING.filter(
      (p) => p.provider.toLowerCase() === provider.toLowerCase(),
    )
      .map((p) => `${p.modelId} (${p.modelName})`)
      .slice(0, 10);
    logger.warn('No pricing data found in calculateCost', {
      provider,
      model,
      inputTokens,
      outputTokens,
...
function getModelNameVariations(model: string): string[] {
