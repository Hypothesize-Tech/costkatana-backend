...
export class MetaPromptPresetsService {
  private readonly presets: MetaPromptPreset[] = [
    {
      id: 'default',
...
Return ONLY the LISP format, nothing else.`,
    },
    {
      ...
      industry: 'retail',
      description:
        'Specialized for retail shelf displays, product placement, and merchandising',
...
Return ONLY the LISP format, nothing else.`,
    },
    {
      ...
      industry: 'jewelry',
      description:
        'Specialized for jewelry displays, security, and luxury presentation',
...
Return ONLY the LISP format, nothing else.`,
    },
    {
      ...
      industry: 'grooming',
      description:
        'Specialized for salon cleanliness, equipment organization, and safety',
...
Return ONLY the LISP format, nothing else.`,
    },
    {
      ...
      industry: 'fmcg',
      description:
        'Specialized for FMCG packaging, brand visibility, and shelf placement',
...
Return ONLY the LISP format, nothing else.`,
    },
    {
      ...
      industry: 'documents',
      description:
        'Specialized for document format, text clarity, and completeness',
...
Return ONLY the LISP format, nothing else.`,
   ...
   */
  getAllPresets(): MetaPromptPreset[] {
    return this.presets;
  }
   ...
   */
  getPresetById(id: string): MetaPromptPreset | null {
   ...
   */
  getPresetsByIndustry(industry: string): MetaPromptPreset[] {
    return this.presets.filter(
      (p) => p.industry === industry || p.industry === 'default',
    );
  }
   ...
   */
  getDefaultPreset(): MetaPromptPreset {
   ...
   */
  substituteCriteria(metaPrompt: string, criteria: string[]): string {
    const formattedCriteria = criteria
      .map((criterion, index) => `Item ${index + 1}: ${criterion}`)
      .join('\n');
   ...
   */
  validateMetaPrompt(metaPrompt: string): { valid: boolean; error?: string } {
    ...
    if (metaPrompt.length > 4000) {
      return {
        valid: false,
        error: 'Meta prompt exceeds maximum length of 4000 characters',
      };
    }
    ...
    if (!metaPrompt.includes('{items_to_verify}')) {
      return {
        valid: false,
        error: 'Meta prompt must include {items_to_verify} placeholder',
      };
    }
