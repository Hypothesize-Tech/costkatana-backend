...
export class IntegrationDetector {
  private readonly logger = new Logger(IntegrationDetector.name);

  constructor(private readonly bedrockService: BedrockService) {}

  /**
   * Detect if message requires integration tools with AI-powered analysis
   */
  async detect(message: string, userId: string): Promise<IntegrationIntent> {
    try {
...
Only suggest integrations if the message clearly requires them.`;
      ...
      this.logger.error('Failed to detect integration intent with AI', {
        userId,
  ...
  /**
   * Keyword-based detection fallback with detailed tool suggestions
   */
  detectWithKeywords(message: string): IntegrationIntent {
    ...
    if (
      lowerMessage.match(/\b(deploy|deployment|vercel|hosting|build\s+log)/)
    ) {
      integrations.push('vercel');
      if (lowerMessage.includes('deploy'))
        suggestedTools.push('vercel_deploy_project');
      if (lowerMessage.includes('log'))
        suggestedTools.push('vercel_get_deployment_logs');
    }

    // Check for GitHub keywords
    if (
      lowerMessage.match(
        /\b(github|pull\s+request|pr|issue|branch|commit|repository|merge)/,
      )
    ) {
      integrations.push('github');
      if (lowerMessage.match(/\b(create|new)\s+(pull\s+request|pr)/))
        suggestedTools.push('github_create_pr');
      if (lowerMessage.match(/\b(create|new)\s+issue/))
        suggestedTools.push('github_create_issue');
      if (lowerMessage.match(/\blist\s+(pr|pull\s+request)/))
        suggestedTools.push('github_list_prs');
    }

    // Check for Google keywords
    if (
      lowerMessage.match(
        /\b(google|drive|docs|sheets|gmail|calendar|workspace)/,
      )
    ) {
      integrations.push('google');
      if (lowerMessage.includes('drive'))
        suggestedTools.push('google_drive_list_files');
      if (lowerMessage.includes('sheet'))
        suggestedTools.push('google_sheets_read');
      if (lowerMessage.includes('doc'))
        suggestedTools.push('google_docs_create');
    }

    // Check for MongoDB keywords
    if (
      lowerMessage.match(
        /\b(mongodb|database|collection|query|find|aggregate|insert|update|delete)\b/,
      )
    ) {
      integrations.push('mongodb');
      if (lowerMessage.includes('find') || lowerMessage.includes('query'))
        suggestedTools.push('mongodb_find');
      if (lowerMessage.includes('insert'))
        suggestedTools.push('mongodb_insert');
      if (lowerMessage.includes('update'))
        suggestedTools.push('mongodb_update');
      if (lowerMessage.includes('delete'))
        suggestedTools.push('mongodb_delete');
      if (lowerMessage.includes('aggregate'))
        suggestedTools.push('mongodb_aggregate');
    }
    ...
    if (lowerMessage.match(/\b(jira|ticket|epic|sprint)/)) {
      ...
      if (lowerMessage.match(/\b(create|new)\s+(ticket|issue)/))
        suggestedTools.push('jira_create_issue');
    }
    ...
    if (lowerMessage.match(/\b(linear|cycle)/)) {
      ...
      if (lowerMessage.match(/\b(create|new)\s+issue/))
        suggestedTools.push('linear_create_issue');
    }
