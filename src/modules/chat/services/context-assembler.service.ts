/**
 * Context Assembler Service (NestJS)
 * Smart context filtering with pre-LLM pipeline for RAG
 * Ported from Express ContextAssemblerService
 */

import { Injectable, Logger } from '@nestjs/common';
import { CodebaseIndex } from '../services/github-chat-agent.service';
import {
  SymbolJumpService,
  SymbolDefinition,
} from '../../../common/services/symbol-jump.service';
import {
  FindReferencesService,
  SymbolReference,
} from '../../../common/services/find-references.service';
import { VectorStoreService } from '../../agent/services/vector-store.service';
import { TreeSitterService } from '../../github/tree-sitter.service';
...
export class ContextAssemblerService {
  ...
  private readonly MAX_FILES = 30;
  private readonly TOKEN_ESTIMATE_PER_LINE = 4; // Rough estimate

  constructor(
    private readonly symbolJumpService: SymbolJumpService,
  ...
  async assembleContext(
    ...
    try {
      ...
      for (const entryPoint of codebaseIndex.structure.entryPoints.slice(
        0,
        5,
      )) {
      ...
      // Extract symbols
      context.symbols = await this.extractRelevantSymbols(
        userRequest,
        codebaseIndex,
      );
      ...
      if (context.totalTokens > this.MAX_TOKENS) {
        context.relevantFiles = this.truncateToTokenLimit(
      ...
      this.logger.log('Context assembled', {
      ...
      this.logger.error('Context assembly failed', {
   ...
   */
  private async findSymbolMatches(
    userRequest: string,
    codebaseIndex: CodebaseIndex,
    userId?: string,
    repoFullName?: string,
    ...
    if (userId && matches.length === 0) {
      for (const word of words) {
        try {
          const definition = await this.symbolJumpService.findSymbolDefinition(
            word,
            codebaseIndex,
            userId,
            repoFullName,
          ...
          this.logger.debug('SymbolJumpService search failed', {
            symbol: word,
            error: error instanceof Error ? error.message : 'Unknown',
          });
        }
  ...
  private findStructuralMatches(
    ...
    if (codebaseIndex.astMetadata) {
      for (const [filePath, ast] of codebaseIndex.astMetadata.entries()) {
        ...
        for (const imp of ast.imports) {
          const importMatches =
            requestLower.includes(imp.module.toLowerCase()) ||
            imp.imports.some((importName: string) =>
              requestLower.includes(importName.toLowerCase()),
            );
          if (importMatches) {
            matches.push({
              path: filePath,
              reason: `Contains import: ${imp.module} (via Tree-Sitter AST)`,
            });
            break;
          }
        }
    ...
    const filesWithoutAST = codebaseIndex.structure.sourceFiles.filter(
      (filePath) =>
        !codebaseIndex.astMetadata?.has(filePath) && fileContents[filePath],
    );

    for (const filePath of filesWithoutAST.slice(0, 10)) {
      ...
      try {
        ...
        const ast = this.treeSitterService.parseCode(
          content,
          fileInfo.language,
          filePath,
        );
        ...
        for (const imp of ast.imports) {
          ...
          if (importMatches) {
            matches.push({
              path: filePath,
              reason: `Contains import: ${imp.module} (parsed on-the-fly via Tree-Sitter)`,
            });
            break;
          }
        }
      } catch (error) {
        this.logger.debug('Failed to parse file with TreeSitterService', {
          filePath,
          error: error instanceof Error ? error.message : 'Unknown',
        });
  ...
  private async findSemanticMatches(
    ...
    try {
      ...
      const results = await this.vectorStoreService.search(userRequest, 20); // Get more results to filter

      // Extract and validate paths using codebase index
      const validPaths = results
        .map((r) => {
          const result = r as {
            metadata?: { source?: string; filePath?: string };
          };
        ...
        .filter((path) => {
          ...
          const isSourceFile =
            codebaseIndex.structure.sourceFiles.includes(path);
      ...
      this.logger.warn('Semantic search failed in context assembler', {
        error: error instanceof Error ? error.message : 'Unknown',
      });
      return [];
    }
   ...
   */
  private async extractRelevantSymbols(
    userRequest: string,
    codebaseIndex: CodebaseIndex,
    ...
    for (const word of words) {
      if (codebaseIndex.symbolTable.has(word)) {
        ...
        // Get reference count
        const references = await this.findReferencesService.findReferences(
          word,
          codebaseIndex,
        );
   ...
   */
  private truncateFileContent(content: string, focusLine?: number): string {
    ...
    for (const file of files) {
      ...
      if (currentTokens + fileTokens <= maxTokens) {
        ...
        const remainingLines = Math.floor(
          remainingTokens / this.TOKEN_ESTIMATE_PER_LINE,
        );
