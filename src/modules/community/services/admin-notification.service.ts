...
export class AdminNotificationService {
  ...
  private notificationClients: Map<
    string,
    Set<(data: NotificationData) => void>
  > = new Map();
    ...
    if (clients) {
      clients.forEach((callback) => {
        try {
          ...
          this.logger.error('Admin notification error', { adminId, error });
        }
      });
    }
  }
  ...
  isAdminOnline(adminId: string): boolean {
    return (
      this.notificationClients.has(adminId) &&
