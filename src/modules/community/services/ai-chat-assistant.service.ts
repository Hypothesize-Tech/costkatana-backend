...
export class AiChatAssistantService {
  ...
  private initializeModel() {
    ...
    const chatModel =
      process.env.AWS_BEDROCK_CHAT_MODEL_ID ?? 'amazon.nova-micro-v1:0';
    ...
    if (!accessKeyId || !secretAccessKey) {
      this.logger.warn(
        'AWS credentials not configured for AI Chat Assistant - AI responses will be disabled',
        {
    ...
    try {
      ...
      this.logger.log('🤖 AI Chat Assistant Service initialized', {
        model: chatModel,
        region,
      });
    } catch (error) {
      this.logger.error('Failed to initialize AI Chat Assistant Service', {
  ...
  async generateResponse(
    ...
    if (!this.isInitialized || !this.model) {
      this.logger.warn(
        'AI Chat Assistant not initialized, returning fallback response',
        {
    ...
    try {
      const userGreeting = userName
        ? `The user you are helping is ${userName}.`
        : '';
...
Remember: You're here to help users get the most out of Cost Katana. Be professional, friendly, and solution-oriented.`;
      ...
      const messages = [
        ...
        ...recentMessages.map((msg) => {
          if (msg.senderType === 'user') {
            ...
            // For AI messages, format as assistant messages for proper multi-turn coherence
            return new AIMessage(msg.content);
          } else {
            // For support/system messages, format as system messages
      ...
      if (!aiResponse || aiResponse.trim().length === 0) {
        this.logger.warn('Empty response from AI model', { sessionSubject });
