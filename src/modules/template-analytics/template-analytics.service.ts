...
export class TemplateAnalyticsService {
  ...
  async getTemplateUsageStats(
    ...
    try {
      ...
      if (filters.templateId) {
        matchStage['templateUsage.templateId'] = new mongoose.Types.ObjectId(
          filters.templateId,
        );
      }
      ...
      return {
        ...
        averageTokenReduction:
          totalTokensSaved / (overview.totalUsageCount || 1),
      ...
      this.logger.error('Error getting template usage stats:', error);
      throw error;
    }
  }
  ...
  async getTemplateBreakdown(
    ...
    try {
            ...
            summary: [
              {
                $group: {
                  ...
                  templateCategory: {
                    $first: '$templateUsage.templateCategory',
                  },
                },
              },
            ],
            ...
            variablesUsage: [
              { $unwind: '$templateUsage.variablesResolved' },
              {
                $group: {
                  ...
                  usageCount: { $sum: 1 },
                  values: {
                    $addToSet: '$templateUsage.variablesResolved.value',
                  },
                },
              },
              { $sort: { usageCount: -1 } },
      ...
      return {
        ...
        averageCost:
          summary.usageCount > 0 ? summary.totalCost / summary.usageCount : 0,
        averageTokens:
          summary.usageCount > 0 ? summary.totalTokens / summary.usageCount : 0,
        contextUsage,
        recentUsages,
        variablesUsage,
      };
    } catch (error) {
      this.logger.error('Error getting template breakdown:', error);
      throw error;
    }
  }
  ...
  async getTopTemplates(
    ...
    try {
      ...
      const topTemplates = await this.usageModel.aggregate([
        {
          $match: {
            userId: new mongoose.Types.ObjectId(userId),
            'templateUsage.templateId': { $exists: true },
      ...
      return topTemplates.map((template, index) => ({
        ...
        averageCost:
          template.usageCount > 0
            ? template.totalCost / template.usageCount
            : 0,
        lastUsed: template.lastUsed,
        costSavingsEstimate: template.totalCost * 0.1, // 10% estimated savings
      }));
    } catch (error) {
      this.logger.error('Error getting top templates:', error);
      throw error;
    }
  }
  ...
  async getTemplateCostSavings(
    ...
    try {
      ...
      const savingsByContext = savings.byContext.map((ctx: any) => ({
        ...
        percentage:
          totalContextCost > 0 ? (ctx.totalCost / totalContextCost) * 100 : 0,
      }));
      ...
      this.logger.error('Error getting template cost savings:', error);
      throw error;
    }
  }
  ...
  > {
    try {
      ...
      const templates = await this.usageModel.aggregate([
        { $match: matchStage },
        {
          $group: {
            _id: '$templateUsage.templateId',
      ...
      this.logger.error('Error getting templates by context:', error);
      throw error;
    }
  }
}
