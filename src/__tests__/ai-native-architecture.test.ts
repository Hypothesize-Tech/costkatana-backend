...
describe('Latency Routing', () => {
  ...
  it('should track model latency', async () => {
    ...
    const p95 = await latencyRouterService.getModelLatencyP95(
      'openai',
      'gpt-4',
    );
  ...
  it('should calculate P95 latency correctly', async () => {
    ...
    for (const latency of latencies) {
      await latencyRouterService.trackModelLatency(
        'anthropic',
        'claude-3',
        latency,
        true,
    ...
    const stats = await latencyRouterService.getModelLatencyStats(
      'anthropic',
      'claude-3',
    );
  ...
  it('should select model based on latency requirements', async () => {
    ...
    const decision = await latencyRouterService.selectModelByLatency(
      1000,
      modelOptions,
    );
  ...
  it('should open circuit breaker after failures', async () => {
    ...
    const isAvailable = latencyRouterService.isModelAvailable(
      'openai',
      'gpt-4',
    );
...
describe('Budget Estimation', () => {
  ...
  it('should release budget on failure', async () => {
    const reservationId = await BudgetService.reserveBudget('test-user', 0.1);
...
describe('Priority Queue', () => {
  it('should enqueue requests with priority', async () => {
    ...
    const requestId = await priorityQueueService.enqueueRequest(
      mockRequest,
      75,
    );
...
describe('Performance Tests', () => {
  it('should handle high-volume latency tracking', async () => {
    ...
    for (let i = 0; i < 100; i++) {
      promises.push(
        latencyRouterService.trackModelLatency(
          'openai',
          'gpt-4',
