...
import { Session } from '../schemas/misc/session.schema';
import { Telemetry } from '../schemas/core/telemetry.schema';

// Mock the models
jest.mock('../schemas/misc/session.schema');
jest.mock('../schemas/core/telemetry.schema');
...
describe('SessionReplayService', () => {
  ...
  describe('getOrCreateActiveSession', () => {
    it('should return existing active session if found', async () => {
      ...
      const result = await sessionReplayService.getOrCreateActiveSession(
        'user-123',
        {
          workspaceId: 'workspace-123',
        },
  ...
  describe('listSessionReplays', () => {
    ...
    it('should filter by source', async () => {
      const mockSessions = [{ sessionId: 'session-1', source: 'unified' }];
